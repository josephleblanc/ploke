...
pub fn parse_source_file_cst(text: &str, edition: Edition) -> Parse<SyntaxNode> {
    let (green, errors) = parsing::parse_text(text, edition);
    Parse::new(green, errors)
}

pub fn parse_text_to_cst(text: &str, entry: TopEntryPoint, edition: Edition) -> Parse<SyntaxNode> {
    let (green, errors) = parsing::parse_text_at(text, entry, edition);
    Parse::new(green, errors)
}

pub fn parse_expr_cst(text: &str, edition: Edition) -> Parse<SyntaxNode> {
    parse_text_to_cst(text, TopEntryPoint::Expr, edition)
}
