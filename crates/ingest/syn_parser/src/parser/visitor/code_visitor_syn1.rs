...
// visit_item_* methods,
// e.g.
// fn visit_trait_item_fn(&mut self, i: &'ast syn1::TraitItemFn)
// fn visit_trait_item_const(&mut self, i: &'ast syn1::TraitItemConst)
...
// Implement ExtractSpan for syn1 types (syn1 uses the same spanned::Spanned trait)
pub trait ExtractSpan {
    fn extract_span_bytes(&self) -> (usize, usize);
}
impl<T: syn1::spanned::Spanned> ExtractSpan for T {
    fn extract_span_bytes(&self) -> (usize, usize) {
...
// Helper function to convert syn1 visibility to our VisibilityKind
fn convert_visibility_syn1(vis: &syn1::Visibility) -> VisibilityKind {
    use syn1::Visibility;
    match vis {
        Visibility::Public(_) => VisibilityKind::Public,
        Visibility::Crate(_) => VisibilityKind::Crate,
...
impl<'a> CodeVisitor<'a> {
    ...
    /// Recursively processes a `syn1::UseTree` to extract `ImportNode`s.
    ///
...
impl<'a, 'ast> Visit<'ast> for CodeVisitor<'a> {
    ...
    fn visit_item_fn(&mut self, func: &'ast ItemFn) {
        ...
        if is_proc_macro {
            ...
            let scope_cfgs = self.state.current_scope_cfgs.clone();
            let item_cfgs = super::attribute_processing_syn1::extract_cfg_strings(&func.attrs);
            let provisional_effective_cfgs: Vec<String> = scope_cfgs
                .iter()
                .cloned()
                .chain(item_cfgs.iter().cloned())
            ...
            for arg in &func.sig.inputs {
                if let Some(param) = self.process_fn_arg_syn1(arg) {
    ...
    fn visit_item_struct(&mut self, item_struct: &'ast ItemStruct) {
        ...
        for (field, i) in item_struct.fields.iter().zip(u8::MIN..u8::MAX) {
            ...
            let field_node = FieldNode {
                ...
                name: field_name,
                type_id,
                visibility: convert_visibility_syn1(&field.vis),
    ...
    fn visit_item_type(&mut self, item_type: &'ast syn1::ItemType) {
        #[cfg(feature = "cfg_eval")]
        {
            use crate::parser::visitor::attribute_processing_syn1::should_include_item;
            let active_cfg = &self.state.active_cfg;
        ...
        // --- CFG Handling (Raw Strings) ---
        let scope_cfgs = self.state.current_scope_cfgs.clone();
        let item_cfgs = super::attribute_processing_syn1::extract_cfg_strings(&item_type.attrs);
        let provisional_effective_cfgs: Vec<String> = scope_cfgs
            .iter()
            .cloned()
            .chain(item_cfgs.iter().cloned())
    ...
    fn visit_item_union(&mut self, item_union: &'ast syn1::ItemUnion) {
        #[cfg(feature = "cfg_eval")]
        {
            use crate::parser::visitor::attribute_processing_syn1::should_include_item;
            let active_cfg = &self.state.active_cfg;
        ...
        // --- CFG Handling (Raw Strings) ---
        let scope_cfgs = self.state.current_scope_cfgs.clone();
        let item_cfgs = super::attribute_processing_syn1::extract_cfg_strings(&item_union.attrs);
        let provisional_effective_cfgs: Vec<String> = scope_cfgs
            .iter()
            .cloned()
            .chain(item_cfgs.iter().cloned())
    ...
    fn visit_item_enum(&mut self, item_enum: &'ast ItemEnum) {
        ...
        // --- CFG Handling (Raw Strings) ---
        let scope_cfgs = self.state.current_scope_cfgs.clone();
        let item_cfgs = super::attribute_processing_syn1::extract_cfg_strings(&item_enum.attrs);
        let provisional_effective_cfgs: Vec<String> = scope_cfgs
            .iter()
            .cloned()
            .chain(item_cfgs.iter().cloned())
        ...
        for variant in &item_enum.variants {
            ...
            let variant_item_cfgs =
                super::attribute_processing_syn1::extract_cfg_strings(&variant.attrs);
            ...
            match &variant.fields {
                syn1::Fields::Named(fields_named) => {
                ...
                syn1::Fields::Unnamed(fields_unnamed) => {
    ...
    fn visit_item_impl(&mut self, item_impl: &'ast ItemImpl) {
        ...
        // --- CFG Handling (Raw Strings) ---
        let scope_cfgs = self.state.current_scope_cfgs.clone();
        let item_cfgs = super::attribute_processing_syn1::extract_cfg_strings(&item_impl.attrs);
        let provisional_effective_cfgs: Vec<String> = scope_cfgs
            .iter()
            .cloned()
            .chain(item_cfgs.iter().cloned())
        ...
        for item in &item_impl.items {
            ...
            //     //       to handle syn1::ImplItem::Const or syn1::ImplItem::Type
            // Note: In syn1, ImplItem::Method is the equivalent of syn2's ImplItem::Fn
            if let syn1::ImplItem::Method(method) = item {
                ...
                let method_item_cfgs =
                    super::attribute_processing_syn1::extract_cfg_strings(&method.attrs);
            ...
            // TODO: Handle syn1::ImplItem::Const and syn1::ImplItem::Type here
    ...
    fn visit_item_trait(&mut self, item_trait: &'ast ItemTrait) {
        ...
        // --- CFG Handling (Raw Strings) ---
        let scope_cfgs = self.state.current_scope_cfgs.clone();
        let item_cfgs = super::attribute_processing_syn1::extract_cfg_strings(&item_trait.attrs);
        let provisional_effective_cfgs: Vec<String> = scope_cfgs
            .iter()
            .cloned()
            .chain(item_cfgs.iter().cloned())
        ...
        for item in &item_trait.items {
            if let syn1::TraitItem::Method(method) = item {
                let method_name = method.sig.ident.to_string();
            ...
            // TODO: Handle syn1::TraitItem::Const and syn1::TraitItem::Type here
            // 1. Generate base ID for ConstNode/TypeAliasNode
            // 2. Create the ConstNode/TypeAliasNode
            // 3. Store the node
    ...
    fn visit_item_mod(&mut self, module: &'ast syn1::ItemMod) {
        #[cfg(feature = "cfg_eval")]
        {
            use crate::parser::visitor::attribute_processing_syn1::should_include_item;
            let active_cfg = &self.state.active_cfg;
        ...
        // --- CFG Handling (Raw Strings) ---
        let scope_cfgs = self.state.current_scope_cfgs.clone();
        let item_cfgs = super::attribute_processing_syn1::extract_cfg_strings(&module.attrs);
        let provisional_effective_cfgs: Vec<String> = scope_cfgs
            .iter()
            .cloned()
            .chain(item_cfgs.iter().cloned())
        ...
        let module_node = ModuleNode {
            ...
            visibility: convert_visibility_syn1(&module.vis),
    ...
    fn visit_item_use(&mut self, use_item: &'ast syn1::ItemUse) {
...
", self.state.current_primary_defn_scope, use_item.tree.to_token_stream().to_string());
        ...
        // --- CFG Handling (Raw Strings) ---
        let scope_cfgs = self.state.current_scope_cfgs.clone();
        let item_cfgs = super::attribute_processing_syn1::extract_cfg_strings(&use_item.attrs);
        let provisional_effective_cfgs: Vec<String> = scope_cfgs
            .iter()
            .cloned()
            .chain(item_cfgs.iter().cloned())
        ...
        let vis_kind = convert_visibility_syn1(&use_item.vis);
    ...
    fn visit_item_extern_crate(&mut self, extern_crate: &'ast syn1::ItemExternCrate) {
        #[cfg(feature = "cfg_eval")]
        {
            use crate::parser::visitor::attribute_processing_syn1::should_include_item;
            let active_cfg = &self.state.active_cfg;
        ...
        // --- CFG Handling (Raw Strings) ---
        let scope_cfgs = self.state.current_scope_cfgs.clone();
        let item_cfgs = super::attribute_processing_syn1::extract_cfg_strings(&extern_crate.attrs);
        let provisional_effective_cfgs: Vec<String> = scope_cfgs
            .iter()
            .cloned()
            .chain(item_cfgs.iter().cloned())
    ...
    fn visit_item_const(&mut self, item_const: &'ast syn1::ItemConst) {
        ...
        // --- CFG Handling (Raw Strings) ---
        let scope_cfgs = self.state.current_scope_cfgs.clone();
        let item_cfgs = super::attribute_processing_syn1::extract_cfg_strings(&item_const.attrs);
        let provisional_effective_cfgs: Vec<String> = scope_cfgs
            .iter()
            .cloned()
            .chain(item_cfgs.iter().cloned())
        ...
        let const_node = ConstNode {
            ...
            span,
            visibility: convert_visibility_syn1(&item_const.vis),
            type_id,
            value,
            attributes,
            docstring,
    ...
    // Visit static items
    fn visit_item_static(&mut self, item_static: &'ast syn1::ItemStatic) {
        #[cfg(feature = "cfg_eval")]
        {
            use crate::parser::visitor::attribute_processing_syn1::should_include_item;
            let active_cfg = &self.state.active_cfg;
        ...
        // --- CFG Handling (Raw Strings) ---
        let scope_cfgs = self.state.current_scope_cfgs.clone();
        let item_cfgs = super::attribute_processing_syn1::extract_cfg_strings(&item_static.attrs);
        let provisional_effective_cfgs: Vec<String> = scope_cfgs
            .iter()
            .cloned()
            .chain(item_cfgs.iter().cloned())
        ...
        let static_node = StaticNode {
            ...
            span,
            visibility: convert_visibility_syn1(&item_static.vis),
            type_id,
    ...
    fn visit_item_macro(&mut self, item_macro: &'ast syn1::ItemMacro) {
        ...
        // --- CFG Handling (Raw Strings) ---
        let scope_cfgs = self.state.current_scope_cfgs.clone();
        let item_cfgs = super::attribute_processing_syn1::extract_cfg_strings(&item_macro.attrs);
        let provisional_effective_cfgs: Vec<String> = scope_cfgs
            .iter()
            .cloned()
            .chain(item_cfgs.iter().cloned())
...
fn format_generics_for_name(generics: &syn1::Generics) -> String {
