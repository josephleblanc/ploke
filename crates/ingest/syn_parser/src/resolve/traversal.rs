...
pub fn traverse_tree(
    ...
    let first_rel = tree.get_relations_to(item_pid.as_any()).map(|rels| {
        rels.iter()
            .find(|&tr| tr.rel().is_contains())
...
// ..
pub fn get_relations_to_primary(&self, target_id: &PrimaryNodeId) -> Option<&TreeRelation> {
    self.relations_by_target
        .get(target_id.as_any())
        .map(|indices| {
            // Changed: Use AnyNodeId key
            indices
                .iter()
                .find_map(|&index| self.tree_relations.get(index))
        })
}
// .. other methods
