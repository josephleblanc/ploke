...
fn test_simple_imports() {
    ...
    let has_map = graph.use_statements.iter()
        .find(|u| u.path == vec!["std", "collections", "HashMap"]);
    assert!(has_map.is_some(), "Should find std::collections::HashMap import");
}
...
fn test_aliases() {
    ...
    let fmt_alias = graph.use_statements.iter()
...
fn test_nested_groups() {
    ...
    let atomic_bool = graph.use_statements.iter()
...
fn test_glob_imports() {
    ...
    let glob = graph.use_statements.iter()
