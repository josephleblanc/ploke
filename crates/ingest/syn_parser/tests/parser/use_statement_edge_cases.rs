...
fn test_edge_case_imports() {
    ...
    #[cfg(feature = "use_statement_tracking")]
    {
        ...
        assert!(uses.iter().any(|u| 
            u.path == vec!["a", "b", "c", "d", "e", "f"] &&
            u.visible_name == "f"
        ...
        assert!(uses.iter().any(|u| 
            u.path == vec!["r#mod", "r#type"] &&
            u.visible_name == "r#var"
...
fn test_invalid_use_statements() {
    ...
    #[cfg(feature = "use_statement_tracking")]
    {
        assert!(graph.unwrap().use_statements.is_empty(), 
