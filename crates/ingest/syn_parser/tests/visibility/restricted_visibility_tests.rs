...
use syn_parser::parser::nodes::{OutOfScopeReason, TypeDefNode, VisibilityResult};
...
fn test_pub_in_path_restricted_function() {
    ...
    let restricted_fn =
        find_function_by_name(&graph, "restricted_fn").expect("restricted_fn not found in fixture");

    // Test allowed access from within the specified path
    let allowed_result =
        graph.resolve_visibility(restricted_fn.id, &test_module_path(&["crate", "outer"]));
    assert!(
    ...
    let denied_result =
        graph.resolve_visibility(restricted_fn.id, &test_module_path(&["crate", "unrelated"]));
    assert!(
        matches!(
            denied_result,
...
fn test_pub_in_path_restricted_struct() {
    ...
    let restricted_struct = find_struct_by_name(&graph, "RestrictedStruct").unwrap_or_else(|| {
        #[cfg(feature = "verbose_debug")]
        {
    ...
    // Test allowed access
    let allowed_result =
        graph.resolve_visibility(restricted_struct.id, &test_module_path(&["crate", "outer"]));
    assert!(
        matches!(allowed_result, VisibilityResult::Direct),
