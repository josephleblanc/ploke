...
pub fn find_enum_node_paranoid<'a>(
    ...
    let item_cfgs = enum_node.cfgs(); // Get the enum's own CFGs
    ...
    assert_eq!(
        ...
        "Mismatch between node's actual ID ({}) and regenerated ID ({}) for enum '{}' in module {:?} file '{}'.\nItemKind: {:?}\nParentScope: {:?}\nScope CFGs: {:?}\nItem CFGs: {:?}\nCombined CFGs: {:?}",
        enum_id, regenerated_id, enum_name, expected_module_path, file_path.display(), ItemKind::Enum, Some(module_node.id), scope_cfgs, item_cfgs, provisional_effective_cfgs
    );
