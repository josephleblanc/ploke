...
fn test_parser_robustness_against_malformed_input() {
    ...
    // 1. Run Discovery
    let discovery = run_discovery_phase(Some(&fixture_path), &[fixture_path.clone()]).expect(
    ...
    assert!(
        results[0].is_err(),
        "Expected the malformed file to return an error, but it parsed successfully?"
    );
}
