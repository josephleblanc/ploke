...
mod tests {
    ...
    fn test_concurrent_access_to_shared_graph() {
        ...
        for i in 0..NUM_THREADS {
            output_paths.push(PathBuf::from(format!(
                "tests/data/concurrent_test_{}.ron",
                i
            )));
        }
        ...
        for (i, path) in output_paths.iter().enumerate() {
            ...
            let handle = thread::spawn(move || {
                ...
                save_to_ron_threadsafe(graph_clone, &path_clone)
                    .unwrap_or_else(|_| panic!("Thread {} failed to save graph", i));

                // Verify the file exists
                assert!(
                    path_clone.exists(),
                    "Output file was not created by thread {}",
                    i
                );
            });
