...
fn test_union_node_int_or_float_paranoid() {
    ...
    // Field i
    let field_i = union_node.fields.iter().find(|f| f.name.as_deref() == Some("i")).expect("Field 'i' not found");
    ...
    // Field f
    let field_f = union_node.fields.iter().find(|f| f.name.as_deref() == Some("f")).expect("Field 'f' not found");
...
fn test_union_node_generic_union_paranoid() {
    ...
    // Field value
    let field_value = union_node.fields.iter().find(|f| f.name.as_deref() == Some("value")).expect("Field 'value' not found");
    ...
    assert!(matches!(&type_value.kind, TypeKind::Named { path, .. } if path.ends_with(&["ManuallyDrop".to_string()])));
    ...
    // Field raw
    let field_raw = union_node.fields.iter().find(|f| f.name.as_deref() == Some("raw")).expect("Field 'raw' not found");
    ...
    // 1. Module Contains Union
    assert_relation_exists(graph, GraphId::Node(module_id), GraphId::Node(union_node.id()), RelationKind::Contains, "Module->Union");

    // 2. Union Contains Fields
    assert_relation_exists(graph, GraphId::Node(union_node.id()), GraphId::Node(field_value.id), RelationKind::StructField, "Union->Field value");
    assert_relation_exists(graph, GraphId::Node(union_node.id()), GraphId::Node(field_raw.id), RelationKind::StructField, "Union->Field raw");
}
...
fn test_other_union_nodes() {
    ...
    let module_path = vec!["crate".to_string()];
    let node = find_union_node_paranoid(&results, fixture_name, relative_file_path, &module_path, union_name);
    assert_eq!(node.visibility(), VisibilityKind::Inherited);
    assert_eq!(node.fields.len(), 2);
    assert_relation_exists(graph, GraphId::Node(module_id_crate), GraphId::Node(node.id()), RelationKind::Contains, union_name);
    ...
    let field_always = node.fields.iter().find(|f| f.name.as_deref() == Some("always_present")).expect("Field 'always_present' not found");
    ...
    let has_cfg_attr = node.fields.iter().any(|f| f.name.as_deref() != Some("always_present") && !f.attributes.is_empty());
    assert!(has_cfg_attr, "Expected at least one cfg'd field to have attributes");
    assert_relation_exists(graph, GraphId::Node(module_id_crate), GraphId::Node(node.id()), RelationKind::Contains, union_name);
    ...
    let node = find_union_node_paranoid(&results, fixture_name, relative_file_path, &module_path_inner, union_name);
    assert_eq!(node.visibility(), VisibilityKind::Inherited);
    assert_eq!(node.fields.len(), 2);
    assert_relation_exists(graph, GraphId::Node(module_id_inner), GraphId::Node(node.id()), RelationKind::Contains, union_name);
