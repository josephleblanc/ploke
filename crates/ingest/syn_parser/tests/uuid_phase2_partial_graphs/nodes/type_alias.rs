...
fn test_type_alias_node_simple_id_paranoid() {
    ...
    assert!(matches!(type_alias_node.type_id, TypeId::Synthetic(_)), "Aliased TypeId should be Synthetic");
    ...
    assert!(
        ...
        "Expected aliased type 'u64', found {:?}", aliased_type_node.kind
    );
...
fn test_type_alias_node_displayable_container_paranoid() {
    ...
    match &generic_param.kind {
        GenericParamKind::Type { name, bounds, default } => {
            ...
            assert!(
                ...
                "Expected bound type 'Display', found {:?}", bound_type_node.kind
            );
    ...
    assert!(
        ...
        "Expected aliased type 'Vec<T>', found {:?}", aliased_type_node.kind
    );
    // Check the related type (T)
    assert_eq!(aliased_type_node.related_types.len(), 1, "Vec<T> should have one related type (T)");
    ...
    assert!(
        ...
        "Expected related type 'T', found {:?}", related_type_node.kind
    );
...
fn test_other_type_alias_nodes() {
    ...
    let module_path = vec!["crate".to_string()];
    let node = find_type_alias_node_paranoid(&results, fixture_name, relative_file_path, &module_path, alias_name);
    ...
    assert_relation_exists(graph, GraphId::Node(module_id_crate), GraphId::Node(node.id()), RelationKind::Contains, alias_name);
    ...
    assert_eq!(node.docstring.as_deref(), Some(" Documented public alias for a tuple type"));
    let aliased_type = find_type_node(graph, node.type_id);
    assert!(matches!(&aliased_type.kind, TypeKind::Unknown { type_str } if type_str == "(i32 , i32)")); // Tuple not implemented
    ...
    let aliased_type = find_type_node(graph, node.type_id);
    assert!(matches!(&aliased_type.kind, TypeKind::Named { path, .. } if path.ends_with(&["HashMap".to_string()]))); // std::collections::HashMap<K, V>
    ...
    assert_eq!(node.visibility(), VisibilityKind::Inherited);
    let aliased_type = find_type_node(graph, node.type_id);
    assert!(matches!(&aliased_type.kind, TypeKind::Unknown { type_str } if type_str == "fn (i32 , i32) -> i32")); // Fn Ptr not implemented
    ...
    let node = find_type_alias_node_paranoid(&results, fixture_name, relative_file_path, &module_path_inner, alias_name);
    assert_eq!(node.visibility(), VisibilityKind::Inherited);
    let aliased_type = find_type_node(graph, node.type_id);
    assert!(matches!(&aliased_type.kind, TypeKind::Named { path, .. } if path == &["bool"]));
    assert_relation_exists(graph, GraphId::Node(module_id_inner), GraphId::Node(node.id()), RelationKind::Contains, alias_name);
    ...
    let aliased_type = find_type_node(graph, node.type_id);
    assert!(matches!(&aliased_type.kind, TypeKind::Named { path, .. } if path.ends_with(&["Point".to_string()]))); // super::Point
    assert_relation_exists(graph, GraphId::Node(module_id_inner), GraphId::Node(node.id()), RelationKind::Contains, alias_name);
    ...
    assert_eq!(node.visibility(), VisibilityKind::Inherited);
    let aliased_type = find_type_node(graph, node.type_id);
    assert!(matches!(&aliased_type.kind, TypeKind::Named { path, .. } if path.ends_with(&["InnerPublic".to_string()]))); // inner::InnerPublic
    assert_relation_exists(graph, GraphId::Node(module_id_crate), GraphId::Node(node.id()), RelationKind::Contains, alias_name);
    ...
    assert_eq!(node.visibility(), VisibilityKind::Inherited);
    let aliased_type = find_type_node(graph, node.type_id);
    assert!(matches!(&aliased_type.kind, TypeKind::Named { path, .. } if path.ends_with(&["OuterPoint".to_string()]))); // inner::OuterPoint
    assert_relation_exists(graph, GraphId::Node(module_id_crate), GraphId::Node(node.id()), RelationKind::Contains, alias_name);
    ...
    let aliased_type = find_type_node(graph, node.type_id);
    assert!(matches!(&aliased_type.kind, TypeKind::Reference { is_mutable: false, .. })); // &'a str
    ...
    assert_eq!(node.generic_params.len(), 1); // Lifetime 'a
    let aliased_type = find_type_node(graph, node.type_id);
    assert!(matches!(&aliased_type.kind, TypeKind::Reference { is_mutable: true, .. })); // &'a mut str
    assert_eq!(aliased_type.related_types.len(), 1); // str
    assert_relation_exists(graph, GraphId::Node(module_id_crate), GraphId::Node(node.id()), RelationKind::Contains, alias_name);
    ...
    assert_eq!(node.visibility(), VisibilityKind::Inherited);
    let aliased_type = find_type_node(graph, node.type_id);
    assert!(matches!(&aliased_type.kind, TypeKind::Unknown { type_str } if type_str == "* const u8")); // Ptr not implemented
    ...
    assert_eq!(node.visibility(), VisibilityKind::Inherited);
    let aliased_type = find_type_node(graph, node.type_id);
    assert!(matches!(&aliased_type.kind, TypeKind::Unknown { type_str } if type_str == "* mut u8")); // Ptr not implemented
    #[ignore = "TypeKind::Ptr not yet handled"] {}
    assert_relation_exists(graph, GraphId::Node(module_id_crate), GraphId::Node(node.id()), RelationKind::Contains, alias_name);
    ...
    let node = find_type_alias_node_paranoid(&results, fixture_name, relative_file_path, &module_path, alias_name);
    let aliased_type = find_type_node(graph, node.type_id);
    assert!(matches!(&aliased_type.kind, TypeKind::Unknown { type_str } if type_str == "[u8 ; 256]")); // Array not implemented
    ...
    let node = find_type_alias_node_paranoid(&results, fixture_name, relative_file_path, &module_path, alias_name);
    let aliased_type = find_type_node(graph, node.type_id);
    assert!(matches!(&aliased_type.kind, TypeKind::Unknown { type_str } if type_str == "dyn Debug")); // TraitObject not implemented
