...
pub const LOG_TEST_CONST: &str = "log_test_node"; // Use generic log target name

// Remove manual definitions - they will be generated by the (not-yet-applied) derive macro
// pub trait ExpectedNodeData<N: GraphNode + HasAttributes + TestIds> { ... }

// Remove manual struct definition
// #[derive(Debug, Clone, PartialEq)]
// pub struct ExpectedConstData { ... }

// Remove manual impl block
...
// Keep the lazy_static maps, but they now initialize the *derived* structs
// (which happen to have the same field names for now).
// The types `ExpectedConstData` and `ExpectedStaticData` are assumed to be
// generated by a derive macro applied elsewhere (or manually defined matching the derived structure).
// For this step, we keep the manual definitions but remove the trait and methods.

#[derive(Debug, Clone, PartialEq)]
pub struct ExpectedConstData { // Keep manual struct def for now
    name: &'static str,
...
// Remove the manual impl block for ExpectedConstData
// impl ExpectedConstData { ... }


#[derive(Debug, Clone, PartialEq)]
struct ExpectedStaticData { // Keep manual struct def for now
    name: &'static str,
    visibility: VisibilityKind,
    type_id_check: bool,
    is_mutable: bool,
