...
async fn call_context_expansion_adds_incoming_fixture_method_callers() -> Result<(), Error> {
    ...
    let borrowed_alias_owner = one_uuid(
        &db,
        &function_in_module_query(&["crate"], "call_borrowed_param_alias_instance_method"),
    )?;
    ...
    let borrowed_param_owner = one_uuid(
        &db,
        &function_in_module_query(&["crate"], "call_borrowed_value_param_instance_method"),
    )?;
    let borrowed_result_owner = one_uuid(
        &db,
        &function_in_module_query(
            &["crate"],
    ...
    let method_callers = [
        ...
        Case {
            owner: borrowed_alias_owner,
            label: "borrowed parameter-alias method owner",
            callee: CallCalleeInfo::Method {
                name: "instance_value".to_string(),
                receiver: Some(CallReceiverInfo::AliasedLocalBinding {
                    name: "alias".to_string(),
        ...
        Case {
            owner: borrowed_param_owner,
            label: "borrowed value-parameter method owner",
            callee: CallCalleeInfo::Method {
                name: "instance_value".to_string(),
                receiver: Some(CallReceiverInfo::BorrowedLocalBinding {
                    name: "value".to_string(),
                }),
            },
        },
        Case {
            owner: borrowed_result_owner,
            label: "borrowed value-parameter method-result owner",
            callee: CallCalleeInfo::Method {
                name: "instance_value".to_string(),
