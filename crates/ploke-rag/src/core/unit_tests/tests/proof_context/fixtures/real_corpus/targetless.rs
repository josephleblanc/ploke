...
#[tokio::test]
async fn proof_context_collection_preserves_axum_request_parts_external_return_blocker()
-> Result<(), Error> {
    init_tracing_once();
    let db = axum_db()?;
    ...
    assert!(
        projected >= 2,
        "request_parts.rs:164 should project targetless call-site proof rows"
    );
    ...
    let site_id = context
        ...
        .unwrap_or_else(|| {
            panic!("request_parts.rs:164 should expose the unsupported turbofish row: {context:#?}")
        })
        .site
        .id;
    db.upsert_proof_fact_values(&[ploke_test_utils::axum_parts_blocker(site_id)])?;

    let rag = init_test_rag_mock(Arc::clone(&db));
    assert!(
        !rag.proof_context_degraded(),
        "projected axum request-parts facts should enable RAG proof context"
    );
    ...
    let observed = targetless_method_site_with_status(
        ...
        "request_parts.rs:164 extract_with_state turbofish",
    );
    assert_eq!(observed, site_id);
    ...
    assert!(
        ...
        "request_parts.rs:164 proof context should expose the explicit missing external return summary blocker: {rows:#?}"
    );
    assert!(
        rows.iter().any(|proof| {
        ...
        "request_parts.rs:164 projected call_resolution should remain fail-closed with the derived type-resolution blocker: {rows:#?}"
    );
