...
#[test]
fn fixture_projection_marks_real_unsupported_receiver_method_without_edges() -> Result<(), DbError>
{
    let db = setup_call_graph_fixture_db("fixture_call_graph")?;
    ...
    assert_eq!(row.site.receiver, Some(CallReceiver::Unsupported));
    assert_eq!(row.status.status, CallStatusKind::Unsupported);
    assert_eq!(row.status.resolution, None);
    assert!(
        row.targets.is_empty(),
        "unsupported receiver proof setup must be targetless: {row:#?}"
    );
