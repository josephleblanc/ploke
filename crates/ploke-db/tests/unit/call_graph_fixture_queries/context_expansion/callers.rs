...
fn fixture_callers_for_target_reads_real_incoming_callers() -> Result<(), DbError> {
    ...
    let boxed_path = caller_by_owner_kind_path(
        &callers,
        boxed_path_owner,
        CallSiteKind::Path,
        &["boxed_fn"],
    );
    assert_eq!(boxed_path.status.status, CallStatusKind::Resolved);
    assert_eq!(
        boxed_path.status.resolution,
        Some(CallResolutionKind::LocalExact)
    );
    assert_eq!(boxed_path.target.relation, CallRelationKind::Function);
    assert_eq!(boxed_path.target.source_kind, CallSiteKind::Path);
    assert_eq!(boxed_path.target.target_kind, CallTargetKind::Function);

    let boxed_dynamic = caller_by_owner_kind_path(
        &callers,
        boxed_dynamic_owner,
        CallSiteKind::Dynamic,
        &["boxed_fn"],
    );
    assert_eq!(boxed_dynamic.status.status, CallStatusKind::Resolved);
    assert_eq!(
        boxed_dynamic.status.resolution,
        Some(CallResolutionKind::LocalExact)
    );
    assert_eq!(
        boxed_dynamic.target.relation,
...
fn fixture_callers_for_target_reads_method_and_associated_callers() -> Result<(), DbError> {
    ...
    let callers = db.callers_for_target(target)?;
    assert_resolved_callers_for_target(&callers, target, 11, "LocalAssoc::instance_value");
    ...
    let owner = function_id_by_name(&db, "call_borrowed_value_param_instance_method")?;
    let caller = caller_by_owner_method_receiver(
        &callers,
        owner,
        "instance_value",
