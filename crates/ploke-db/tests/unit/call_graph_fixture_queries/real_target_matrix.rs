...
//! | Trait associated extraction paths | `axum-core/src/ext_traits/{request.rs:279,305}` and `request_parts.rs:133` call `E::from_request*` | bounded type-parameter associated paths traverse to the `FromRequest` / `FromRequestParts` trait method bindings in one call edge each. |
//! | Bounded `FromRef` paths | `axum-core/src/ext_traits/mod.rs:{25,45}`, `axum/src/extract/state.rs:309`, and `middleware/from_extractor.rs:328` call `*::from_ref(...)` | currently unsupported: bounded associated paths are visible but targetless. |
//! | Listener `Self::accept` paths | `axum/src/serve/listener.rs:{41,61}` call `Self::accept(self).await` | currently unsupported and targetless; the projected line-41 row must not be modeled as recursive trait dispatch, while line 61 remains a body-owner completeness gap. |
//! | `HeaderValue::from_static` external paths | response conversion bodies plus `axum/src/routing/route.rs:202` call `HeaderValue::from_static(...)` | all projected rows are external and targetless; the route local-const call is still owned by `set_content_length`, not `CallBodyOwnerId::Const`. |
...
//! | Receiver forwarding gaps | `axum/src/extension.rs:180`, `routing/route.rs:51`, and `middleware/from_fn.rs:411` exercise field/result receivers | unsupported receiver shapes remain visible and targetless; `Router::new` currently has 144 caller rows and 123 incoming expansion candidates. |
