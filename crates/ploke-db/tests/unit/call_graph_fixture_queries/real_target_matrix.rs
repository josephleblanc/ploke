...
//! | Bounded `FromRef` paths | `axum-core/src/ext_traits/mod.rs:{25,45}`, `axum/src/extract/state.rs:309`, and `middleware/from_extractor.rs:328` call `*::from_ref(...)` | same-crate axum-core bounds traverse to `FromRef::from_ref`; dependency-root `axum_core::extract::FromRef` bounds remain visible but targetless. |
...
//! | Receiver forwarding gaps | `axum/src/extension.rs:180`, `routing/route.rs:51`, `serve/mod.rs:{574,575,577,581,585,590,595,601,692,704}`, and `middleware/from_fn.rs:411` exercise field/result receivers | unsupported receiver shapes remain visible and targetless; `Router::new` currently has 144 caller rows and 123 incoming expansion candidates. |
