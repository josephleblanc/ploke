...
fn fixture_context_reads_projected_result_receiver_method_chains() -> Result<(), DbError> {
    ...
    assert_eq!(
        context.len(),
        2,
        "borrowed-parameter method-result context rows: {context:#?}"
    );

    let clone_receiver = CallReceiver::BorrowedLocalBinding {
        name: "value".to_string(),
    };
    let row = row_by_method_receiver(&context, "clone_assoc", &clone_receiver);
    assert_resolved_target(
    ...
    assert_eq!(
        context.len(),
        2,
        "self-field method-result context rows: {context:#?}"
    );

    let clone_receiver = CallReceiver::SelfField {
        path: path(&["value"]),
    };
    let row = row_by_method_receiver(&context, "clone_assoc", &clone_receiver);
    assert_resolved_target(
