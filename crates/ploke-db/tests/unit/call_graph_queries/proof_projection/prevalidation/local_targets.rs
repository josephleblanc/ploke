...
const CRATE_TARGET_PATH: &[&str] = &["crate", "target"];
...
fn assert_owner_projection_rejects(row: LocalTargetRow<'_>) -> Result<(), DbError> {
    let db =
...
fn assert_target_projection_rejects(
    target: Uuid,
