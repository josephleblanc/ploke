# Prototype 1

Prototype 1 is the self-propagating eval loop currently implemented inside
`ploke-eval`.

Use this section for current operator and implementation truth.

| Need | Page |
| --- | --- |
| Command, file, and implementation overview | [Operator Map](operator-map.md) |
| What phase am I in? | [Phase Map](phase-map.md) |
| How do I inspect a stalled run? | [Debugging Playbook](debugging-playbook.md) |
| How do I recover without weakening authority? | [Recovery](recovery.md) |
| How do I use the typestate debugger? | [Loop Walk](loop-walk.md) |
| What fields are in the admitted profile? | [Run Profile](run-profile.md) |
| What does typestate prove? | [Typestate](typestate.md) |
| What invariants matter? | [Typestate Invariants](typestate-invariants.md) |
| How does child-plan admission work? | [Child Plan Authority](child-plan-authority/index.md) |

For conceptual rationale, see the Evalnomicon
[Prototype 1 Architecture](../../../../docs/workflow/evalnomicon/src/prototype1/index.md).
