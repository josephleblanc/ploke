...
4. If there is no `?graph=`, fetches the default fixture URL `/benchmark-fixtures/trajectory-multi-gen.json`.
...
|------------------|-------|
| Default fetch URL | `/benchmark-fixtures/trajectory-multi-gen.json` |
| Eval/protocol fixture | `/benchmark-fixtures/protocol-graph.json` |
| Basename alias | `?graph=protocol-graph.json` → protocol fixture URL |
...
| Method | How |
|--------|-----|
| Default (no query) | Open `http://127.0.0.1:8080/` — startup fetches the trajectory multi-gen fixture. |
| Query param | `http://127.0.0.1:8080/?graph=/benchmark-fixtures/protocol-graph.json` |
| Alias basename | `http://127.0.0.1:8080/?graph=protocol-graph.json` (eval/protocol export) |
...
The browser only fetches URLs Trunk serves under `.dist/` (default build: `.dist/default/`). The default startup fixture is `trajectory-multi-gen.json` under `benchmark-fixtures/` (tracked; same bytes as `standard-prototype1-graph-snapshot.json` when aligned). For eval/protocol exports, symlink `protocol-graph.json` from `.temp/` if needed. For other exports, pick one:
...
| Goal | Command | URL |
|------|---------|-----|
| Multi-gen trajectory at startup | `trunk serve --config crates/ploke-egui/Trunk.toml` | `http://127.0.0.1:8080/` |
| Eval/protocol export | same `trunk serve` | `http://127.0.0.1:8080/?graph=/benchmark-fixtures/protocol-graph.json` |
