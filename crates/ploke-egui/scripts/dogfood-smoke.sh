#!/usr/bin/env bash
# Non-interactive WASM dogfood: wasm check, trunk build, curl smoke on dist/.
set -euo pipefail
...
echo "==> curl smoke (dist/)"
test -f "${DIST}/index.html"
curl -sf "file://${DIST}/index.html" | head -c 200 >/dev/null

FIXTURE="${DIST}/benchmark-fixtures/standard-prototype1-graph-snapshot.json"
