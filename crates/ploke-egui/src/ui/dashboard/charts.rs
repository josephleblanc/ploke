...
pub fn horizontal_bar_chart(ui: &mut egui::Ui, title: &str, data: &[(&str, f32)]) {
    ui.vertical(|ui| {
        ui.heading(title);
        ui.add_space(8.0);

        let max_val = data.iter().map(|(_, v)| *v).fold(1.0, f32::max);
            ...
            .show(ui, |ui| {
                for (label, value) in data {
                    ...
                    let (rect, _response) = ui.allocate_at_least(
                        egui::vec2(ui.available_width().max(100.0), 16.0),
                        egui::Sense::hover(),
                    );
