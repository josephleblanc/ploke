//! Diagnostic snapshots and default-view contract reporting.
//!
//! Render-only observations from the live egui view. Layout is not recomputed
//! here. Native targets may persist snapshots via [`io::SnapshotSink`]; WASM
//! builds expose the same contract types for read-only reporting.
...
#[cfg(not(target_arch = "wasm32"))]
pub use io::{SnapshotSink, write_manual_snapshot};
