...
use ploke_tui::app::view::components::config_overlay::{ConfigOverlayState, render_config_overlay};
...
fn footer_rect(area: Rect, help_visible: bool) -> Rect {
    ...
    let footer_height = if help_visible { 7 } else { 4 };
...
fn config_overlay_footer_shows_help_when_enabled() {
    ...
    assert!(
        lines
            .iter()
            .any(|line| line.contains("Keys: tab/shift+tab")),
