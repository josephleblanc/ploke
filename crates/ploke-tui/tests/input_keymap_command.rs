...
fn command_mode_enter_executes_and_esc_exits() {
    let exec = to_action(Mode::Command, KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE), CommandStyle::NeoVim);
    assert!(matches!(exec, Some(Action::ExecuteCommand)));

    let esc = to_action(Mode::Command, KeyEvent::new(KeyCode::Esc, KeyModifiers::NONE), CommandStyle::NeoVim);
    assert!(matches!(esc, Some(Action::SwitchMode(Mode::Normal))));
}

#[test]
fn command_mode_backspace_is_forwarded() {
    let back = to_action(Mode::Command, KeyEvent::new(KeyCode::Backspace, KeyModifiers::NONE), CommandStyle::NeoVim);
