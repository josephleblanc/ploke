...
#[tokio::test]
async fn code_item_lookup_marks_async_function_targets_in_call_impact() {
    let fixture = CallGraphToolFixture::new().await;
    let params = LookupParams {
    ...
    let result = CodeItemLookup::execute(params, fixture.ctx("async-target-lookup"))
        .await
        .expect("make_ready_local_assoc lookup");
    let payload: serde_json::Value =
        serde_json::from_str(&result.content).expect("deserialize ConciseContext");
    ...
    assert_eq!(
        ...
        Some(true),
        "code_item_lookup should serialize async function item metadata: {impact:#?}"
    );
    ...
    assert!(
        ...
        "code_item_lookup should preserve async direct caller metadata: {direct_callers:#?}"
    );
}
...
#[tokio::test]
async fn code_item_lookup_returns_local_binding_payload_for_initialized_callable() {
    let fixture =
    ...
    let result = CodeItemLookup::execute(params, fixture.ctx("local-binding-lookup"))
        .await
        .expect("initialized callable binding lookup");
    let payload: serde_json::Value =
        serde_json::from_str(&result.content).expect("deserialize ConciseContext");
    ...
    //   edge; lookup should also expose the durable local-binding evidence.
...
#[tokio::test]
async fn code_item_lookup_returns_local_binding_payload_for_local_function_binding() {
    ...
    let result = CodeItemLookup::execute(params, fixture.ctx("local-function-binding-lookup"))
        .await
        .expect("local function binding lookup");
    let payload: serde_json::Value =
        serde_json::from_str(&result.content).expect("deserialize ConciseContext");
    let bindings = payload
        .get("local_bindings")
    ...
    //   `fn inner() { ... } inner()` resolves to a LocalItem target and should
    //   expose the durable binding-to-local-item proof at the tool boundary.
    assert_local_function_binding_payload(
        bindings,
        edges,
        fixture.owner,
...
#[tokio::test]
async fn code_item_lookup_returns_local_binding_payload_for_method_callable_argument() {
    ...
    let result = CodeItemLookup::execute(params, fixture.ctx("method-argument-binding-lookup"))
        .await
        .expect("method callable argument binding lookup");
    let payload: serde_json::Value =
        serde_json::from_str(&result.content).expect("deserialize ConciseContext");
    let bindings = payload
        .get("local_bindings")
...
#[tokio::test]
async fn code_item_lookup_returns_local_binding_payload_for_aliased_field_parameter() {
    let fixture = FixtureDynamicCallableToolFixture::new_for_owner(
    ...
    let result = CodeItemLookup::execute(params, fixture.ctx("alias-binding-lookup"))
        .await
        .expect("aliased field parameter binding lookup");
    let payload: serde_json::Value =
        serde_json::from_str(&result.content).expect("deserialize ConciseContext");
    let bindings = payload
        .get("local_bindings")
    ...
    //   `let alias = holder; (alias.callback)()` resolves through private
    //   complete-caller proof and should expose the alias carrier.
...
#[tokio::test]
async fn code_item_lookup_returns_memchr_runner_setter_assignment_payload() {
    let fixture = MemchrRunnerSetterToolFixture::new_fwd().await;
    let params = LookupParams {
        item_name: Cow::Borrowed("fwd"),
        file_path: Cow::Owned(fixture.file_path.display().to_string()),
        node_kind: Cow::Borrowed("method"),
        module_path: Cow::Owned(fixture.module_path_arg()),
        owner_trait: None,
        ...
        parent_name: None,
        body_contains: Some(Cow::Borrowed("self.fwd = Some(Box::new(search));")),
        allowed_effects: Vec::new(),
    };

    let result = CodeItemLookup::execute(params, fixture.ctx("memchr-setter-binding-lookup"))
        .await
        .expect("memchr Runner::fwd setter binding lookup");
    let payload: serde_json::Value =
        serde_json::from_str(&result.content).expect("deserialize ConciseContext");
    let bindings = payload
        .get("local_bindings")
    ...
    // Source oracle:
    //   memchr/src/tests/substring/mod.rs:133-138
    //   `Runner::fwd(search)` stores `search` into `self.fwd` through
    //   `Some(Box::new(search))`. This is field-assignment evidence only; the
    //   later boxed `dyn FnMut` call in `Runner::run` remains targetless.
    assert_memchr_runner_setter_local_binding_payload(
        bindings,
        edges,
        fixture.owner,
...
#[tokio::test]
async fn code_item_lookup_returns_axum_tap_io_accept_self_field_parameter_flow() {
    ...
    let result = CodeItemLookup::execute(params, fixture.ctx("tap-io-accept-flow-lookup"))
        .await
        .expect("TapIo::accept self-field parameter flow lookup");
    let payload: serde_json::Value =
        serde_json::from_str(&result.content).expect("deserialize ConciseContext");
    let flows = payload
        .get("self_field_parameter_flows")
        .and_then(serde_json::Value::as_array)
        .expect("self_field_parameter_flows array");
...
#[tokio::test]
async fn code_item_lookup_returns_private_referenced_dyn_fn_param_callable_context() {
    // Fixture source:
    ...
    assert_resolved_dynamic_callable_lookup("call_single_parenthesized_referenced_dyn_fn_param")
        .await;
}

#[tokio::test]
async fn code_item_lookup_returns_private_boxed_dyn_fn_param_callable_context() {
    // Fixture source:
    ...
    assert_callable_param_lookup(
        CallableParamResolvedFixture::single_boxed_dyn_fn_param().await,
        "private boxed dyn Fn parameter",
    )
    .await;
    assert_resolved_dynamic_callable_lookup("call_single_parenthesized_boxed_dyn_fn_param").await;
}
...
#[tokio::test]
async fn code_item_lookup_returns_two_hop_forwarded_named_field_dynamic_callable_context() {
    // Fixture source:
    //   tests/fixture_crates/fixture_call_graph/src/lib.rs EOF
    ...
    // This pins the bounded two-hop holder-field proof at the lookup boundary
    // without broadening to arbitrary callable value-flow.
    assert_resolved_dynamic_callable_lookup("call_two_hop_forwarded_named_field_leaf").await;
}
...
#[tokio::test]
async fn code_item_lookup_returns_direct_self_field_dispatch_candidates() {
    // Source oracle:
    ...
    // The exact lookup payload must expose the finite candidates while keeping
    // the dynamic call candidate-only and edge-free.
    ...
    let result = CodeItemLookup::execute(params, fixture.ctx("direct-self-field-lookup"))
        .await
        .expect("direct self-field lookup");
    let payload: serde_json::Value =
        serde_json::from_str(&result.content).expect("deserialize ConciseContext");
    let call_context = payload
        .get("call_context")
    ...
    assert!(
        ui_field(ui, "call_context_outgoing")
            ...
            .expect("outgoing count")
            >= 1,
        "code_item_lookup should surface outgoing direct self-field call context"
    );
    assert_eq!(
        ui_field(ui, "proof_context"),
        proof_context.len().to_string()
...
async fn code_item_lookup_returns_multi_caller_function_pointer_param_target() {
    ...
    for fixture in [
        ...
        assert_callable_param_lookup(fixture, "multi-caller callable parameter").await;
    }
}

#[tokio::test]
async fn code_item_lookup_returns_forwarded_referenced_dyn_fn_param_targets() {
    // Source oracle:
    //   tests/fixture_crates/fixture_call_graph/src/lib.rs EOF
    ...
    for fixture in [
        ...
        assert_callable_param_lookup(fixture, "forwarded referenced dyn Fn parameter").await;
    }
}

#[tokio::test]
async fn code_item_lookup_returns_forwarded_boxed_dyn_fn_param_targets() {
    // Source oracle:
    //   tests/fixture_crates/fixture_call_graph/src/lib.rs EOF
...
async fn assert_callable_param_lookup(fixture: CallableParamResolvedFixture, label: &str) {
    ...
    let result = CodeItemLookup::execute(params, fixture.ctx("callable-param-lookup"))
        .await
    ...
    assert!(
        ui_field(ui, "call_context_outgoing")
            ...
            .expect("outgoing count")
            >= 1,
        "code_item_lookup should surface the resolved {label} row"
    );
    assert_eq!(
        ui_field(ui, "proof_context"),
        proof_context.len().to_string()
...
#[tokio::test]
async fn code_item_lookup_returns_real_corpus_body_new_generated_callers() {
    ...
    let result = CodeItemLookup::execute(params, fixture.ctx("axum-body-new-lookup"))
        .await
        .expect("tool execution");
    let payload: serde_json::Value =
        serde_json::from_str(&result.content).expect("deserialize ConciseContext");
    ...
    // Expected tool traversal: exact lookup of `Body::new` exposes the full
    // target-centered caller set and includes the seven generated
...
#[tokio::test]
async fn code_item_lookup_returns_real_corpus_generated_rejection_self_methods() {
    ...
    let result = CodeItemLookup::execute(params, fixture.ctx("axum-rejection-lookup"))
        .await
        .expect("tool execution");
    let payload: serde_json::Value =
        serde_json::from_str(&result.content).expect("deserialize ConciseContext");
    ...
    assert_eq!(
        payload.get("id").and_then(serde_json::Value::as_str),
        Some(owner.as_str()),
        "code_item_lookup should resolve the generated MissingExtension::into_response owner"
    );
    ...
    // Expected tool traversal: exact lookup of the generated
    // `IntoResponse for MissingExtension` method exposes outgoing one-hop
    // method edges for `self.status()` and `self.body_text()`.
    assert_generated_rejection_outgoing_context(call_context, &fixture.calls, "code_item_lookup");
    for call in &fixture.calls {
        assert_target_proof(proof_context, call.owner, call.target, "code_item_lookup");
    }
    ...
    assert!(
        ui_field(ui, "call_context_outgoing")
            ...
            .expect("outgoing count")
            >= fixture.calls.len(),
        "code_item_lookup should surface generated rejection outgoing call rows"
    );
    assert!(
        ui_field(ui, "proof_context")
            .parse::<usize>()
            .expect("proof count")
            >= fixture.calls.len(),
        "code_item_lookup should surface generated rejection proof rows"
    );
}

#[tokio::test]
async fn code_item_lookup_returns_real_corpus_composite_rejection_delegate() {
    ...
    let result = CodeItemLookup::execute(params, fixture.ctx("axum-composite-rejection-lookup"))
        .await
        .expect("tool execution");
    let payload: serde_json::Value =
        serde_json::from_str(&result.content).expect("deserialize ConciseContext");
    ...
    assert_eq!(
        payload.get("id").and_then(serde_json::Value::as_str),
        Some(owner.as_str()),
        "code_item_lookup should resolve the generated QueryRejection::into_response owner"
    );
    ...
    assert!(
        ui_field(ui, "call_context_outgoing")
            ...
            .expect("outgoing count")
            >= 1,
        "code_item_lookup should surface composite rejection outgoing call rows"
    );
    assert!(
        ui_field(ui, "proof_context")
            .parse::<usize>()
            .expect("proof count")
            >= 1,
        "code_item_lookup should surface composite rejection proof rows"
    );
}
...
async fn code_item_lookup_returns_chrono_parse_internal_typed_setter_binding_payload() {
    ...
    assert!(
        ui_field(ui, "call_context_outgoing")
            ...
            .expect("outgoing count")
            >= 1,
        "code_item_lookup should surface outgoing chrono setter candidate context"
    );
    assert_eq!(
        ui_field(ui, "proof_context"),
        proof_context.len().to_string()
