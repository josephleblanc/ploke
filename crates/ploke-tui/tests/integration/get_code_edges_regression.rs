...
#[tokio::test]
async fn code_item_edges_returns_local_binding_payload_for_method_callable_argument() {
    ...
    let result = CodeItemEdges::execute(params, fixture.ctx("method-argument-binding-edges"))
        .await
        .expect("method callable argument binding edges");
    let payload: serde_json::Value =
        serde_json::from_str(&result.content).expect("deserialize NodeEdgeInfo");
    let node_info = payload.get("node_info").expect("node_info");
    let bindings = node_info
    ...
    // Same oracle as lookup: the private method's `f()` resolves because its
    // caller supplies `local_target` through a method-call argument.
    assert_method_argument_parameter_local_binding_payload(
        bindings,
        edges,
        fixture.owner,
...
#[tokio::test]
async fn code_item_edges_returns_axum_tap_io_constructor_frontier_payload() {
    ...
    let result = CodeItemEdges::execute(params, fixture.ctx("tap-io-binding-edges"))
        .await
        .expect("tap_io constructor binding edges");
    let payload: serde_json::Value =
        serde_json::from_str(&result.content).expect("deserialize NodeEdgeInfo");
    let node_info = payload.get("node_info").expect("node_info");
    let bindings = node_info
...
#[tokio::test]
async fn code_item_edges_returns_memchr_runner_setter_assignment_payload() {
    ...
    let result = CodeItemEdges::execute(params, fixture.ctx("memchr-setter-binding-edges"))
        .await
        .expect("memchr Runner::fwd setter binding edges");
    let payload: serde_json::Value =
        serde_json::from_str(&result.content).expect("deserialize NodeEdgeInfo");
    let node_info = payload.get("node_info").expect("node_info");
    let bindings = node_info
    ...
    // Same oracle as lookup: the setter stores `search` into `self.fwd`, but
    // the boxed dyn call in `Runner::run` remains a runtime-dispatch frontier.
    assert_memchr_runner_setter_local_binding_payload(
        bindings,
        edges,
        fixture.owner,
...
#[tokio::test]
async fn code_item_edges_returns_memchr_runner_run_assignment_flow_payload() {
    ...
    let result = CodeItemEdges::execute(params, fixture.ctx("memchr-run-assignment-flow-edges"))
        .await
        .expect("memchr Runner::run assignment flow edges");
    let payload: serde_json::Value =
        serde_json::from_str(&result.content).expect("deserialize NodeEdgeInfo");
    let flows = payload
        .get("node_info")
        .and_then(|node| node.get("self_field_assignment_flows"))
        .and_then(serde_json::Value::as_array)
        .expect("node_info.self_field_assignment_flows array");
    let argument_flows = payload
        .get("node_info")
        .and_then(|node| node.get("self_field_assignment_argument_flows"))
        .and_then(serde_json::Value::as_array)
        .expect("node_info.self_field_assignment_argument_flows array");

    // Same source oracle as lookup: the run method reaches targetless boxed
    // callable path rows while setter-side assignment evidence remains
    // proof-only and does not create traversal edges.
    assert_memchr_runner_run_assignment_flow_payload(flows, fixture.owner, "code_item_edges");
    assert_memchr_runner_run_assignment_argument_flow_payload(
        argument_flows,
        fixture.owner,
        "code_item_edges",
...
#[tokio::test]
async fn code_item_edges_returns_chrono_parse_internal_typed_setter_binding_payload() {
    ...
    let result = CodeItemEdges::execute(params, fixture.ctx("chrono-parse-internal-edges"))
        .await
        .expect("chrono parse_internal typed setter binding edges");
    let payload: serde_json::Value =
        serde_json::from_str(&result.content).expect("deserialize NodeEdgeInfo");
    let node_info = payload.get("node_info").expect("node_info");
    let bindings = node_info
