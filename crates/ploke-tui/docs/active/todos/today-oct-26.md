...
- create a test project with a Ratzilla toy program with the goal to
understand:
  - how similar it seems to be to ratatui
  - how to access local files
  - how accessing user-uploaded files is different from accessing server-local
  files.
  - general interaction patterns (mouse, keyboard, capture)
- evaluate which changes need to be made to implement a webassembly build of
Ploke
- Q: is it as simple as replacing the ratatui rendering with ratzilla-based
rendering?
- Q: are there other crates that will need to be changed or that won't work in
a webassembly context?
- Q: is there anything extra that needs to be considered regarding file access
in the webassembly build?
- Q: should the webassembly build be its own crate or should it be a build flag
within ploke-tui?
...
- make the selection of an item circle back around after moving past the end of
the list (current poor functionality allows user to select non-existent items)
- stretch:
  - better previews: Add a way to get the changed code snippet area, instead of
  the file/item as a whole
  - better selection: add fuzzy search (probably should be its own todo item,
  see (7) below)
...
- must have basic editing functionality, e.g.
  ...
  - read file (max chars)
  - search + replace: try a basic search + replace w/ fallbacks, if this fails
  see how other code editors handle this (e.g. aider seems to have different
  edits modes for different models)

7. Start work on internal represenation of cargo commands:
- cargo seems to have a json flag for returning values, research this to see if
it can work for us
- consider what information is most useful
- add a way to include the cargo feedback in the database + link to node edges
in code graph
...
- write a short doc on how to add a new model to the list of usable models (if
needed, I just forget how this works)
- expand tools
...
- expand database functionality, esp. for tools
  - add more logging by default: 
    ...
    - statistics on usage: % LLM-submitted code that lives for X period of
    time, accepted code edits
  - add beginnings of user profile:
    - start experimenting with LLM-authored notes on user behavior, check
    ...
    - revisit research on user behavior embeddings to see how it might be
...
- Improve the parser for the code graph, in particular:
  ...
  - call graph: start getting a basic approach working w/out handling edge
  cases + do research
...
- Consider advanced functionality like detecting code patters + antipatterns
