...
Each slice below must link evidence (tests, artifacts, docs) back to this plan when it lands.
...
- **Tests & fixtures.**
  - Unit tests for schema modules proving the new relations create + insert data at least for functions; align with `multi_embedding_experiment.rs` expectations.
  - Extend `cargo xtask verify-fixtures` to assert the new relations are present in fixture dumps.
...
- **Goal.** Remove legacy single-embedding columns, flip feature flags ON by default, and finalize CLI/telemetry surfaces.
- **Touch points.**
...
- **Feature flags.** Retire slice-specific flags once validation is complete; keep a kill-switch flag documented in case we need to revert in production.
- **Tests & evidence.**
...
## Tracking & Reporting
- Implementation logs live under `crates/ploke-tui/docs/agent-system/impl-log/` (see Slice 0 governance requirements). Every slice gets its own entry referencing this execution plan.
- Decisions requiring user approval go into `crates/ploke-tui/docs/decisions_required.md`.
