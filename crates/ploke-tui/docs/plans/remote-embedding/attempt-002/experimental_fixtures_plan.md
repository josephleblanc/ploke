...
5. Keep the legacy single-column `embedding` data populated so helper tests never skip: the new `ploke_test_utils::seed_default_legacy_embeddings` helper (32 rows @ 384 dims) now runs from the `regenerate_fixture` binary for both the legacy and multi-embedding backups before `cargo xtask verify-fixtures --multi-embedding` executes.
