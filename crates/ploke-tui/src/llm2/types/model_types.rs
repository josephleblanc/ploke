...
use crate::llm2::{Author, IdError, ModelSlug};
...
pub(crate) enum ModelVariant {
    Free, Beta, Extended, Thinking, Online, Nitro, Floor,
    Other(ArcStr),
}
...
impl fmt::Display for ModelId {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match &self.variant {
            ...
            Some(v) => write!(f, "{}/{}:{}", self.key.author.as_str(), self.key.slug.as_str(), v.as_str()),
        }
    }
}
...
impl FromStr for ModelId {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let (author, rest) = s.trim().split_once('/').ok_or(IdError::Invalid("missing '/'"))?;
        ...
        Ok(Self {
            key: ModelKey { author: Author::new(author)?, slug: ModelSlug::new(slug)? },
