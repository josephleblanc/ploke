use super::App;
use crate::{app_state::StateCommand, chat_history::MessageKind, user_config::CommandStyle};
use itertools::Itertools;
use std::path::PathBuf;
use tokio::sync::oneshot;
use uuid::Uuid;

/// Centralized command handling for the TUI.
/// Parses the current input buffer and dispatches corresponding StateCommands,
/// plus makes small UI-local mutations (e.g., toggling preview).
pub(crate) fn execute_command(app: &mut App) {
    let cmd = app.input_buffer.clone();
    let cmd_str = match app.command_style {
    ...
    match cmd_str {
        ...
        cmd if cmd.starts_with("index start") => {
            ...
            match std::fs::metadata(&workspace) {
                Ok(metadata) if metadata.is_dir() => {
                    app.send_cmd(StateCommand::IndexWorkspace { workspace, needs_parse: true });
                }
                Ok(_) => {
        ...
        cmd if cmd.starts_with("bm25 save ") => {
            ...
            if path_str.is_empty() {
                ...
                app.send_cmd(StateCommand::RagBm25Save { path: PathBuf::from(path_str) });
            }
        }
        cmd if cmd.starts_with("bm25 load ") => {
            ...
            if path_str.is_empty() {
                ...
                app.send_cmd(StateCommand::RagBm25Load { path: PathBuf::from(path_str) });
            }
        }
...
fn list_models(app: &App) {
    let cfg = tokio::task::block_in_place(|| {
        tokio::runtime::Handle::current().block_on(async { app.state.config.read().await })
    });
