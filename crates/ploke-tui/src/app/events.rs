        ...
        AppEvent::System(system_event) => {
            match system_event {
                system::SystemEvent::ModelSwitched(new_model) => {
                    ...
                    app.send_cmd(StateCommand::AddMessageImmediate {
                        msg: format!("model changed from {} to {}", app.active_model_id, new_model),
                        kind: MessageKind::SysInfo,
                        new_msg_id: Uuid::new_v4(),
                    });
                ...
                system::SystemEvent::ReadQuery { file_name, query_name } => {
                ...
                system::SystemEvent::WriteQuery { query_name, query_content } => {
                ...
                system::SystemEvent::BackupDb { file_dir, is_success, .. } if is_success => {
                ...
                system::SystemEvent::BackupDb { file_dir, is_success, error } if !is_success => {
                ...
                system::SystemEvent::LoadDb { crate_name, file_dir, is_success, .. } if is_success => {
                ...
                system::SystemEvent::LoadDb { crate_name, file_dir, is_success, error } if !is_success => {
                ...
                system::SystemEvent::ReIndex { workspace } => {
                    app.send_cmd(StateCommand::IndexWorkspace { workspace, needs_parse: false });
                }
                other => {
