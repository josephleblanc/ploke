use super::*;
use crate::{EventBus, AppEvent, system::SystemEvent};
...
mod tests {
    ...
    async fn run_happy_path_tempfile() {
        ...
        let event_bus = Arc::new(crate::EventBus::new(crate::event_bus::EventBusCaps::default()));

        let tmp = std::env::temp_dir().join(format!("get_meta_{}.txt", Uuid::new_v4()));
        fs::write(&tmp, b"hello").expect("write tmp");

        let tool = GetFileMetadataTool { state, event_bus, parent_id: Uuid::new_v4() };
        let res = super::Tool::run(tool, GetFileMetadataInput { file_path: tmp.display().to_string() })
            .await
            .expect("run ok");
        assert!(res.ok);
        assert!(res.exists);
...
impl super::GatTool for GetFileMetadataTool {
    ...
    fn name() -> ToolName { ToolName::GetFileMetadata }
    fn description() -> ToolDescr { ToolDescr::GetFileMetadata }
    fn schema() -> &'static serde_json::Value { &GET_FILE_METADATA_PARAMETERS }
    ...
    fn into_owned<'a>(params: &Self::Params<'a>) -> Self::OwnedParams {
        GetFileMetadataParamsOwned { file_path: params.file_path.clone().into_owned() }
    }
    ...
    async fn run<'a>(self, params: &Self::Params<'a>, _ctx: super::Ctx) -> Result<Self::Output, ploke_error::Error> {
        ...
        match tokio::fs::read(&path).await {
                ...
                let (byte_len, modified_ms) = match tokio::fs::metadata(&path).await {
                    Ok(md) => {
                        ...
                        let modified_ms = md.modified().ok().and_then(|mtime| {
                            mtime.duration_since(std::time::UNIX_EPOCH).ok().map(|d| d.as_millis() as i64)
                        });
                        (len, modified_ms)
                    }
            ...
            Err(e) => {
                Err(ploke_error::Error::Internal(ploke_error::InternalError::CompilerError(format!(
