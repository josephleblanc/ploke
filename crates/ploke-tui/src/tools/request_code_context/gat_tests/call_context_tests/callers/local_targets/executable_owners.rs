...
#[tokio::test]
async fn request_code_context_keeps_non_awaited_async_closure_binding_targetless()
-> color_eyre::Result<()> {
    let db = Arc::new(Database::new(setup_db_full_multi_embedding(
        "fixture_call_graph",
    )?));
    ...
    let result = execute_fixture_request(
        ...
        "call_async_closure_binding_without_await_with_body_call",
        1,
        "non_awaited_async_closure_binding_call_context",
    )
    .await?;
    assert_result_ok(
        &result,
    ...
    // `closure()` creates the async-closure future but does not poll it. Tool
    // context should keep the outer path call targetless and should not expand
    // into the async-closure body.
    let outer_part = result.context.iter().find(|part| part.id == outer).expect(
        "request_code_context should preserve the non-awaited async-closure outer function",
    );
    let closure_call = outer_part
        ...
        .expect("outer function should retain the non-awaited closure() path call");
    ...
    assert!(
        result.context.iter().all(|part| part.id != async_closure),
        "non-awaited async closure binding should not expand into the closure body: {result:#?}"
    );
...
#[tokio::test]
async fn request_code_context_returns_awaited_async_closure_binding_owner_call_context()
-> color_eyre::Result<()> {
    let db = Arc::new(Database::new(setup_db_full_multi_embedding(
        "fixture_call_graph",
    )?));
    ...
    let result = execute_fixture_request(
        ...
        "call_awaited_async_closure_binding_with_body_call",
        1,
        "awaited_async_closure_binding_call_context",
    )
    .await?;
    assert_result_ok(
        &result,
    ...
    // `closure().await` immediately polls the async-closure binding. Tool
    // context should expose the outer path call to the async-closure owner, then
    // the closure-owned body call to `local_target()`.
    let outer_part =
        result.context.iter().find(|part| part.id == outer).expect(
            "request_code_context should preserve the awaited async-closure outer function",
        );
    let closure_call = outer_part
        ...
        .expect("outer function should retain the awaited closure() path call");
    ...
    let async_part = result
        ...
        .iter()
        .find(|part| part.id == async_closure)
        .expect("request_code_context should materialize the awaited async-closure binding owner");
    assert_expansion(
        async_part,
        outer,
        async_closure,
    ...
    let body_call = async_part
        ...
        .expect("awaited async-closure binding owner should retain local_target() call context");
    assert_resolved_target(body_call, target, CallTargetKind::Function);
...
#[tokio::test]
async fn request_code_context_keeps_non_awaited_async_closure_future_binding_targetless()
-> color_eyre::Result<()> {
    let db = Arc::new(Database::new(setup_db_full_multi_embedding(
        "fixture_call_graph",
    )?));
    ...
    let result = execute_fixture_request(
        ...
        "call_async_closure_future_binding_without_await_with_body_call",
        1,
        "non_awaited_async_closure_future_binding_call_context",
    )
    .await?;
    assert_result_ok(
        &result,
    ...
    // Tool context should keep the original path call targetless and should not
    // expand into the async-closure body.
    let outer_part =
        result.context.iter().find(|part| part.id == outer).expect(
            "request_code_context should preserve the unawaited future-binding outer function",
        );
    let closure_call = outer_part
        .call_context
        .iter()
        ...
        .expect("outer function should retain the unawaited closure() future-binding path call");
    assert_eq!(closure_call.status, CallStatusKind::Unsupported);
    assert!(
        closure_call.targets.is_empty(),
    ...
    assert!(
        result.context.iter().all(|part| part.id != async_closure),
        "unawaited async closure future binding should not expand into the closure body: {result:#?}"
    );
...
#[tokio::test]
async fn request_code_context_returns_awaited_async_closure_future_binding_owner_call_context()
-> color_eyre::Result<()> {
    assert_awaited_async_closure_future_owner_context(
        "call_awaited_async_closure_future_binding_with_body_call",
        "awaited_async_closure_future_binding_call_context",
    )
    .await
}
...
#[tokio::test]
async fn request_code_context_returns_awaited_async_closure_future_alias_chain_owner_call_context()
-> color_eyre::Result<()> {
    assert_awaited_async_closure_future_owner_context(
        "call_awaited_async_closure_future_alias_chain_with_body_call",
        "awaited_async_closure_future_alias_chain_call_context",
    )
    .await
}

async fn assert_awaited_async_closure_future_owner_context(
    owner_name: &str,
    ...
    let result = execute_fixture_request(&db, owner_name, 1, trace_name).await?;
    assert_result_ok(&result, owner_name, 1, "fixture_call_graph");
    ...
    // evidence. Tool context should expose that path to the async-closure
    // owner and then the body call to `local_target()`.
    let outer_part = result
        .context
        .iter()
        .find(|part| part.id == outer)
        .expect("request_code_context should preserve the awaited future outer function");
    let closure_call = outer_part
        .call_context
        .iter()
        .find(|call| {
        ...
        .expect("outer function should retain the awaited closure() future path call");
    assert_resolved_target(closure_call, async_closure, CallTargetKind::Closure);
    ...
    let async_part = result
        ...
        .iter()
        .find(|part| part.id == async_closure)
        .expect("request_code_context should materialize the awaited future closure owner");
    assert_expansion(
        async_part,
        outer,
        async_closure,
    ...
    let body_call = async_part
        ...
        .expect("awaited future async-closure owner should retain local_target() context");
    assert_resolved_target(body_call, target, CallTargetKind::Function);
