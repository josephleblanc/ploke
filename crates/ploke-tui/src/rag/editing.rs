...
use crate::{
    ...
    tools::{ToolError, ToolErrorCode, ToolName, ToolRetryContext, ToolUiPayload},
        ...
        .await
    {
        Ok(results) => {
            ...
            let applied_paths = results
                .iter()
                .zip(proposal.edits_ns.iter())
                .filter_map(|(res, edit)| res.as_ref().ok().map(|_| edit.file_path.clone()))
                .collect::<BTreeSet<_>>()
                .into_iter()
                .collect::<Vec<_>>();
            let applied_files = results
                .iter()
                .zip(proposal.edits_ns.iter())
                .filter(|(result, _)| result.is_ok())
                .map(|(_, edit)| edit.file_path.display().to_string())
                .collect::<BTreeSet<_>>()
                .into_iter()
                .collect::<Vec<_>>();
            ...
            let results_json: Vec<serde_json::Value> = results
                .into_iter()
                .zip(proposal.edits_ns.iter())
                .map(|(res, edit)| match res {
                    Ok(write_res) => serde_json::json!({
                        "file_path": edit.file_path.display().to_string(),
...
async fn finish_rescan_error(
    ...
    error: &ploke_error::Error,
) {
    let details = serde_json::json!({ "results": results }).to_string();
    let (request_id, parent_id, call_id, tool_name, message) = {
        let mut proposals = state.proposals.write().await;
        let proposal = proposals
            .get_mut(&proposal_id)
            .expect("applied proposal must remain registered during rescan");
        let message = format!(
            "Applied {applied} edit(s) across {file_count} file(s) to disk, but workspace refresh failed: {error}"
        );
        proposal.status = EditProposalStatus::PartiallyApplied(message.clone());
        let tool_name = if proposal.is_semantic {
            ToolName::ApplyCodeEdit
        } else {
            ToolName::NsPatch
    ...
    let tool_error = ToolError::new(tool_name, ToolErrorCode::Internal, message.clone())
        .retry_hint(
            "Use non_semantic_patch to repair the changed file, then retry semantic tools after parsing succeeds.",
        )
        .retry_context(
            ToolRetryContext::new()
                .field("mutation", true)
                .field("applied_files", applied_files)
                .field("failed_files", failed_files),
        );
    let ui_payload = ToolUiPayload::from_error(call_id.clone(), &tool_error)
        .with_proposal_id(proposal_id)
        .with_request_id(request_id)
        .with_field("status", "partially_applied")
        .with_field("ok", "false")
        .with_field("applied", applied.to_string())
        .with_field("partial", "true")
        .with_field("files", file_count.to_string())
        .with_field("mutation", "true")
        .with_details(details);
    let ui_payload_for_chat = ui_payload.clone();
    let error_wire = tool_error.to_wire_string();
    let _ = event_bus
        .realtime_tx
        .send(AppEvent::System(SystemEvent::ToolCallFailed {
            request_id,
...
async fn rescan_for_changes(
    ...
    if let Err(error) = crate::app_state::handlers::db::scan_paths_checked(
        state,
        event_bus,
    ...
    match scan_rx.await {
        ...
        Err(e) => {
            let error = ploke_error::Error::Domain(ploke_error::DomainError::Ui {
                message: format!("Targeted rescan barrier failed: {e}"),
            });
            tracing::warn!(
                target: "ploke_tui::edit_rescan",
                request_id = %request_id,
