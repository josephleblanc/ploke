...
use crate::{
    ...
    event_bus::{EventBus, EventBusCaps, EventPriority},
    llm::manager::events::{ChatEvt, LlmEvent},
    AppEvent,
};
...
async fn conversation_only_prompt_and_persistent_tip_without_workspace() {
    ...
    let rag = Arc::new(
        RagService::new(Arc::clone(&db), Arc::clone(&embedder)).expect("rag service"),
    );
    ...
    let (ctx_tx, _ctx_rx) = mpsc::channel(8);
    {
        ...
        tokio::spawn(crate::app_state::dispatcher::state_manager(s, cmd_rx, eb, ctx_tx));
    }
        ...
        loop {
            match rt_rx.recv().await {
                Ok(AppEvent::Llm(LlmEvent::ChatCompletion(ChatEvt::PromptConstructed {
                    ...
                    assert!(
                        !formatted_prompt.is_empty()
                            && formatted_prompt[0]
                                .content
                                .starts_with("No workspace context loaded;"),
                        "expected conversation-only system note at head of prompt"
                    );
                    break;
                }
                Ok(_) => continue,
    ...
    // Guidance message should be present and should NOT have stolen focus
    {
        ...
        let tip_count = chat
            .messages
            .values()
            .filter(|m| m.kind == crate::chat_history::MessageKind::SysInfo)
        ...
        loop {
            match rt_rx.recv().await {
                Ok(AppEvent::Llm(LlmEvent::ChatCompletion(ChatEvt::PromptConstructed {
                    parent_id,
                    ..
                }))) if parent_id == user_msg_id_2 => break,
                Ok(_) => continue,
                Err(_) => panic!("event channel closed"),
            }
        }
