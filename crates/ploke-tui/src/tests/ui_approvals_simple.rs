...
use crate::app_state::core::{EditProposal, EditProposalStatus, DiffPreview};
...
fn render_approvals_simple_test(
    ...
    let mut items: Vec<(Uuid, String)> = proposals_guard
        ...
        .map(|(id, p)| {
            ...
            (*id, format!("{}  {}", crate::app::utils::truncate_uuid(*id), status))
        })
        .collect();
...
mod simple_ui_tests {
    ...
    async fn test_deadlock_fix_demonstration() {
        ...
        let result = terminal.draw(|frame| {
            let selected = render_approvals_simple_test(
                frame, 
                frame.area(), 
                &proposals, 
                &ui_state
            ...
            assert_eq!(selected, Some(proposal_id), "Should select the test proposal");
        });
        
        assert!(result.is_ok(), "Fixed version should render without deadlock");
    ...
    async fn test_performance_with_fix() {
        ...
        // Add multiple proposals to test performance
        {
            ...
            for i in 0..100 {
                let proposal = EditProposal {
                    ...
                    status: if i % 3 == 0 { EditProposalStatus::Pending } else { EditProposalStatus::Applied },
                };
        ...
        println!("✅ Performance test passed - rendering 100 proposals took: {:?}", duration);
    }
        ...
        for i in 0..5 {
            ...
            let handle = tokio::spawn(async move {
                for _ in 0..10 {
                    ...
                    assert!(result.is_ok(), "Concurrent rendering task {} should succeed", i);
...
mod integration_verification {
    ...
    async fn test_realistic_usage_pattern() {
        ...
        let test_data = vec![
            ...
            ("failed_operation", EditProposalStatus::Failed("Permission denied".to_string())),
        ...
        let result = terminal.draw(|frame| {
            ...
            assert!(expected_ids.contains(&selected.unwrap()), 
        ...
        assert!(content.contains("Test Proposals"), "Should show proposals section");
