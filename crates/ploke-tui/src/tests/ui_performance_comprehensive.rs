...
async fn create_mock_app_state_with_proposals(proposal_count: usize) -> Arc<crate::app_state::AppState> {
    ...
    Arc::new(crate::app_state::AppState {
        ...
        config: crate::app_state::core::ConfigState::new(crate::app_state::core::RuntimeConfig::from(cfg.clone())),
...
mod performance_tests {
    ...
    async fn test_ui_performance_large_proposal_count() {
        ...
        for &count in PROPOSAL_COUNTS {
            ...
            assert!(result.is_ok(), "UI rendering should succeed for {} proposals", count);
    ...
    async fn test_concurrent_ui_access_under_load() {
        ...
        for i in 0..10 {
            ...
            let handle = tokio::spawn(async move {
                for j in 0..20 {
                    let ui_state = ApprovalsState { selected: j % 5, help_visible: false, view_lines: 0 };
                    let mut terminal = Terminal::new(TestBackend::new(80, 24)).unwrap();
                    
                    let start = Instant::now();
                    let result = terminal.draw(|frame| {
                        let _ = render_approvals_overlay(frame, frame.area(), &state_clone, &ui_state);
                    });
                    let duration = start.elapsed();
                    
                    assert!(result.is_ok(), "Concurrent rendering task {} iteration {} should succeed", i, j);
                    assert!(
        ...
        for (i, handle) in handles.into_iter().enumerate() {
            handle.await.expect(&format!("Concurrent task {} should not panic", i));
        }
    ...
    async fn test_ui_responsiveness_during_mutations() {
        ...
        let rendering_handle = tokio::spawn(async move {
            for i in 0..50 {
                let ui_state = ApprovalsState { selected: i % 10, help_visible: i % 5 == 0, view_lines: 0 };
                ...
                assert!(result.is_ok(), "UI should remain responsive during mutations");
                assert!(
    ...
    async fn test_edge_cases_extreme_data() {
        ...
        println!("✅ Edge case test passed - large diff content rendered in {:?}", duration);
    }
}
