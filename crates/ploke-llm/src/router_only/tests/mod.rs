    ...
    fn write_response(env_str: &str, parsed: &models::Response) -> Result<()> {
        ...
        match env_str {
            ...
            "pricing" => {
                ...
                dir.push(cli::MODELS_JSON_PRICING);
                println!(
...
// TODO:ploke-llm ensure this remains in `ploke-tui` and runs there correctly with the tools
// #[test]
// fn test_chat_comp_request_serialization_minimal() {
//     use crate::manager::RequestMessage;
//     use crate::request::ChatCompReqCore;
//     use crate::request::endpoint::ToolChoice;
//     use crate::router_only::default_model;
//     use ploke_core::tool_types::create_file::CreateFile;
//     use ploke_core::tool_types::{GatCodeEdit, RequestCodeContextGat, Tool};
//
//     let messages = vec![
...
//     ];
//
//     // Mirror the runtime default tool set (tool GAT pilots)
//     let tools = vec![
...
//     // Tools
//     let serialized_tools = v.get("tools").and_then(|t| t.as_array()).expect("tools");
//     assert_eq!(serialized_tools.len(), tools.len());
//     assert_eq!(
//         serialized_tools[0].get("type").and_then(|s| s.as_str()),
//         Some("function")
//     );
// }
