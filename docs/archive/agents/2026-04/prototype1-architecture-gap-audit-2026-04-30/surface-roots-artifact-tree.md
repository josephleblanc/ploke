...
Artifact identity is also still partial. The History docs say admitted Artifacts should eventually commit to backend tree key plus artifact-local manifest digest (`crates/ploke-eval/src/cli/prototype1_state/history.rs:226`). The older handoff note says the same and explicitly records that `FsBlockStore` lacks an authenticated lineage-head map (`docs/workflow/evalnomicon/drafts/history/handoff-2026-04-29.md:123`, `docs/workflow/evalnomicon/drafts/history/handoff-2026-04-29.md:159`).
...
Distributed consensus, cryptographic signatures, and OS-process uniqueness are explicitly outside the current claim (`crates/ploke-eval/src/cli/prototype1_state/history.rs:242`, `docs/workflow/evalnomicon/drafts/history/crown-authority-background.md:111`).
