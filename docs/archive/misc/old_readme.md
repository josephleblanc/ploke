...
Currently, you can demo the project after using `cargo run` using following command within the terminal user interface. Note that we use vim-like bindings by default right now, so you can enter "Normal Mode" with `<esc>` or  "Insert Mode" by pressing `i`, then type
...
❗**It is possible the program may crash**❗
...
| llm | 👷  Partial | LLM integration for code generation<br>(Partial, graph working but no code editing) |
| prompt | 👷 Partial | Prompt engineering of user prompt<br>(Basic setup for now, needs config) |
