...
| Child ready/completion channel and result projections | Child writes ready/projection records and sends terminal channel result through the child runtime path (`prototype1_process.rs`) | Parent `ObserveChild` waits for terminal `ToParent::Result` on the per-runtime child channel; result files are reconstruction evidence | Child runtime acknowledged, evaluated, wrote result artifacts, and sent terminal channel evidence | Ready/projection records carry runtime/generation/refs/paths/pid; result artifacts carry campaign/node/generation/branch/status/disposition/eval path and are written both attempt-scoped and as latest node result | Result artifacts are not lifecycle authority and do not replace the terminal channel payload. |
