...
* [ ] Prompt needed when restored DB root path does not match the current workspace (tool calls like `cargo` should not silently run against a different clone); add user-facing prompt/re-root flow once prompt framework exists.
