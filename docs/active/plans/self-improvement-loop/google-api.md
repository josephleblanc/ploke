...
Update 2026-05-21: The persisted model registry was refreshed to 411 rows, including 53 direct Google rows. Active and parent-patcher selections are now set to the direct registry row `google/models/gemini-2.5-flash`. The similarly named `google/gemini-2.5-flash` row is the OpenRouter catalog row, so default Google-direct routing should use the `google/models/...` id unless a command explicitly passes `--provider google`.
...
- Active and parent-patcher model selections now point at the direct Google registry row `google/models/gemini-2.5-flash`.
