...
- patch, candidate, evaluation, successor, and merge/composability edges
...
|---|---|---|---|
| 1. Typed blocker cleanup | 1-6 | Remove direct typed-persistence violations in protocol artifacts, tool calls/results, LLM attempts, and projections. | Owned persisted/transmitted payloads have named typed readers instead of `serde_json::Value` staging or stringly JSON. |
| 2. Replay and ownership joins | 7-10 | Clean up edit-surface source records, database prompt evidence, evaluation target identity, and successor selection replay. | The system can walk from parent to child, patch, evidence, target, candidate set, selected successor, and outcome. |
| 3. Archive graph primitives | 11-14 | Build lineage, candidate frontier, timeline, and patch/code-graph impact projections. | The UI can draw graph nodes/edges, timeline spans, patch diffs, and directly modified code graph items. |
| 4. Derived evidence and compatibility | 15-16 | Add score/locus evidence-strength views and patch/child composability checks. | The UI can compare parents, patches, loci, candidates, children, and merge/composability evidence without overclaiming causality. |
| 5. UI integration | After slice 16 or after stable subset | Consume typed graph primitives in frontend/native/WASM surfaces. | The archive graph canvas, inspectors, badges, comparisons, and timeline operate over typed records/projections. |
