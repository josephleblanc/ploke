...
| `tool.response.full_response_trace` | `tool-calls-results` | typed | `ploke-records` | [`survey-b`](reports/2026-05-10-tool-calls-results.survey-b.jsonl) |
...
| `llm.attempt.full_response_logs` | `llm-attempts` | typed | `ploke-records` | [`survey-e`](reports/2026-05-10-llm-attempts.survey-e.jsonl) |
...
| `tool-calls-results` | 5 | 3 | Tool arguments and trace projections are typed; remaining gaps are outside the full-response sidecar line shape. |
...
| `llm-attempts` | 8 | 5 | Request/response and full-response sidecar lines are typed; the sidecar envelope now uses UUID assistant ids and native `ploke-llm` recorded response payloads. Provider observation, DTO metadata/logprobs, and tool bridge records still need typed surfaces. |
