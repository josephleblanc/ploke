...
- Parser call-graph feature thread: [`agents/call-graph/README.md`](agents/call-graph/README.md) — current restart spine for typed call-site IDs, completed structural method/path/macro extraction, completed exact inherent `self.method()` resolver slice, and the active call-site coverage matrix for future dynamic/path-resolution slices. Read this before acting on older call-graph plans.
