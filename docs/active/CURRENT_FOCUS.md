...
- Parser call-graph feature thread: [`agents/call-graph/README.md`](agents/call-graph/README.md) — current restart spine for typed call-site IDs, completed structural method/path extraction, completed exact inherent `self.method()` resolver slice, and the next macro/dynamic/path-resolution slices. Read this before acting on older call-graph plans.
