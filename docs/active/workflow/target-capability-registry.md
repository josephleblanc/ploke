...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| `BurntSushi__ripgrep` | target-wide mixed-edition parse path | `resolved` | `parser_blocker` | `graph_valid` | `default_run` | Rust 2015 parse path under mixed-edition code | Mixed Rust 2015/2018 code exposed `syn` edition-keyword failures such as `fn async()`, which previously aborted parsing. The dual-`syn` path is now in place and the target has already been exercised successfully afterward, so this blocker is no longer considered active. Keep ripgrep as a useful regression/sentinel target for future parser spot-checks rather than as an out-of-scope target. | [A2 entry](./hypothesis-registry.md), [bug: syn 2 fails on Rust 2015 bare trait objects](../../active/bugs/2026-04-10-syn-2-fails-on-rust-2015-bare-trait-objects.md), [recent activity](./handoffs/recent-activity.md) | use the dual-`syn` path; treat future reruns as regression spot-checks rather than blocker validation | revisit only if mixed-edition parse regressions reappear or a parser refactor warrants a sentinel recheck | `A2` | 2026-04-12 |
