...
| `error.rs`          | Local `XtaskError`, `RecoveryHint`, `ErrorCode` — **not** `ploke_error::Error` (PRIMARY_TASK_SPEC §D gap).                           |
...
| `usage.rs`          | `UsageTracker`, JSONL persistence, rolling-suggestion threshold — **not** invoked from `Cli::execute` (PRIMARY_TASK_SPEC §B gap).    |
...
## PRIMARY_TASK_SPEC adherence snapshot (sections A–G)
...
## PRIMARY_TASK_SPEC spec errata (do not edit per file banner)

Track these when interpreting the spec; coordinate with task owner if PRIMARY_TASK_SPEC should be corrected later.


| Issue            | Location                   | Note                                                                                                                                                                                                |
| ---------------- | -------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| M.1.9 / M.1.10   | PRIMARY_TASK_SPEC M.4                 | Referenced milestone substeps are not defined in the M.1.1–M.1.5 list; treat as typo for “latest command list / test docs”.                                                                         |
| Heading typo     | PRIMARY_TASK_SPEC M.5                 | “A.5-A-6” should be “A.5–A.6”.                                                                                                                                                                      |
| Wrong path       | PRIMARY_TASK_SPEC sub-agent guideline | `docs/agents/2026-03-24-coordingation/` — typo and wrong folder; real path is `docs/active/agents/2026-03-24-coordination/`.                                                                        |
| M.5 vs M.6       | PRIMARY_TASK_SPEC M.3.1 vs M.5        | M.3.1 asks planning for “M.6” while milestone list uses **M.5** for TUI/tools. **Canonical milestone name for A.5–A.6: M.5.** Planning body file: [design/m6-planning.md](./design/m6-planning.md). |
| Test matrix path | PRIMARY_TASK_SPEC E.1 vs M.3.2        | E.1: coordination `2026-03-25-test_matrix.md`; M.3.2: `xtask/tests/test_matrix.md`. **Canonical:** coordination file; `xtask/tests/test_matrix.md` is a pointer.                                    |
...
## Future milestones (PRIMARY_TASK_SPEC-aligned)


| Milestone | Description                                                               | Status                              |
| --------- | ------------------------------------------------------------------------- | ----------------------------------- |
| M.4       | Full implementation of A.1–A.4 commands + PRIMARY_TASK_SPEC B/D gaps + binary wiring | Not started                         |
...
1. `**TEST_OPENROUTER_API_KEY`:** Required for agent ingest command per PRIMARY_TASK_SPEC A.3; no dedicated xtask ingest command yet.
...
| Document                                                       | Status                            |
| -------------------------------------------------------------- | --------------------------------- |
| [PRIMARY_TASK_SPEC.md](./PRIMARY_TASK_SPEC.md)                                       | Spec (agent-edited per banner)    |
...
- [PRIMARY_TASK_SPEC.md](./PRIMARY_TASK_SPEC.md) — sections A–G
