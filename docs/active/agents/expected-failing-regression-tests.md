...
| Marker | Status | File path | Exact test name or command | Expected result | Removal or update condition |
| --- | --- | --- | --- | --- | --- |
| `regr:googlevertex:23-05-26_19-10` | resolved | `crates/ploke-eval/src/cli/prototype1_state/tests/cli_tests.rs` | `PLOKE_EVAL_HEADLESS_TUI_GOOGLE_MODEL_ID=google/gemini-3.5-flash cargo test -p ploke-eval --features live_api_tests live_google_direct_broad_headless_tui_preflight_applies_edit_from_published_request -- --ignored --nocapture --test-threads=1` | Positive live preflight for direct Google/Vertex broad headless-TUI: creates a published request, routes through `cli_facing -> tui_adapter -> ploke-tui llm_manager`, receives a live Google response, applies `apply_code_edit`, writes request-bound diagnostics/result evidence, and validates the candidate workspace. | Keep ignored because it spends live Google provider calls; run before direct-Google Prototype 1 live campaigns when route/quota health is uncertain. |
