...
| Marker | Status | File path | Exact test name or command | Expected result | Removal or update condition |
| --- | --- | --- | --- | --- | --- |
| `regr:googlevertex:23-05-26_19-10` | retired stale live canary | `crates/ploke-eval/src/cli/prototype1_state/tests/cli_tests.rs` | Removed `live_google_direct_broad_headless_tui_rejects_applied_edit_missing_declared_validation` after the broad headless-TUI validation model changed. | The removed live canary asserted `AppliedValidationMissing` when a model applied an edit without itself running request-declared validation. Current broad headless-TUI admission runs declared validation in the harness after applied batches; the corresponding live failure mode is now `AppliedValidationFailed` when validation runs and fails. Direct-Google routing should be checked by narrower live protocol/TUI canaries, while validation classification is covered by deterministic `tui_adapter` tests such as `requested_validation_missing_blocks_applied_terminal`, `requested_validation_failure_blocks_applied_terminal`, and replay coverage for post-apply declared validation. | Do not re-add this exact live canary unless the validation model again depends on model-issued validation commands. If a new live canary is needed, make it assert the current harness-owned validation behavior instead of missing-validation. |
...
| `regr:candidatereview:18-07-26_06-21` | resolved | `crates/ploke-eval/src/cli/prototype1_state/candidate_review.rs` | `cargo test -p ploke-eval truncated_review_is_retried_with_fresh_adjudication` | Reproduces V26 candidate-review truncation and proves an EOF-class JSON parse failure requests a fresh adjudication while malformed evidence remains unpersisted. | Keep as fixed-contract coverage for `2026-07-17-prototype1-selection-without-candidate-safety-proof.md` and the preserved V26 R10 blocker. |
