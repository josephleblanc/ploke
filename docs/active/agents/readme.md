...
- [`2026-06-16_walk-server.md`](2026-06-16_walk-server.md) — historical June 2026 walk-server baseline; use the current Loop Walk guide and Operator Map for operator authority.
- [`2026-06-17_walk-command-guide.md`](2026-06-17_walk-command-guide.md) — historical June 2026 command guide; use the current Loop Walk guide and Operator Map for live operation.
...
- [`2026-06-19_ploke-loop-publication-docs-synthesis/`](2026-06-19_ploke-loop-publication-docs-synthesis/README.md) — initial source inventory and authority map for Ploke-loop paper, collaborator-doc, and symbolic-proof synthesis; preserves the home-machine typestate-change caveat.
- [`call-graph/`](call-graph/README.md) — current call-graph restart spine; read first after compaction before extending parser call-site extraction/resolution.
...
- [`2026-06-21_call-graph-fixture-nodes-orchestration-plan.md`](2026-06-21_call-graph-fixture-nodes-orchestration-plan.md) — active orchestrator-ready plan for the first `fixture_nodes` typed parser call-graph slice: structural call sites, narrow `self.method()` resolution, and proof projection gates.
- [`2026-06-22_call-graph-id-domain-correction.md`](2026-06-22_call-graph-id-domain-correction.md) — corrective note for keeping call-site IDs in a separate `CallId` universe rather than backing expression occurrences with `NodeId`; read before extending parser call-graph IDs.
...
- [`2026-07-13_prototype1-loop-operator-control-plan.md`](2026-07-13_prototype1-loop-operator-control-plan.md) — active implementation spine for configuration validation, single-controller UI/CLI operation, R-state observability, replay/branching, successor handoff, and domain-preserving database migration; the current slice adds canonical UI setup/config read-back, bounded guarded controls, UI-local Step-mode automation, and named immutable DB evidence views, with fresh three-parent live validation still pending.
