...
| R14a | Final report after stopped/no-selection. | completion resource sample and emitted `Prototype1StateReport`. | Appends completion sample and prints/writes report. | Replay displays final report. | Current walk admits stopped/no-selection final report and stops at R14a. No handoff continuation. |
...
- Live smoke has proven R11a, strict fanout blockers, and a successful historical fanout branch after terminal channel `Result` -> parent observe/comparison recovery.
...
- terminal child evidence used by the parent comes from per-child channel messages, either inline or as channel-carried verifiable references to child-owned durable stores; result files are reconstruction/projection unless named by the channel;
...
Channel/History invariant for this slice and later handoff work:

- Parent/child communication must be modeled as a role-indexed `Channel` with a transport backend, not as direct reads of whatever local files happen to exist. The current backend can be JSONL file buffers, but the driver should be able to swap to sockets, websocket streams, queues, or VM-local channels without changing transition semantics.
- Each runnable child/runtime gets a dedicated channel. Child-owned records, run roots, logs, and git commits may remain local to that child and can be cleaned up or checked out later, but any fact the parent uses for comparison, successor selection, promotion, or History sealing must arrive through the child's channel as a payload or a verifiable reference.
- Parent-owned projections such as `runner-result.json`, `node.json`, `transition-journal.jsonl`, `branches.json`, and `evaluations/<branch>.json` are useful caches/reconstruction surfaces. They must not be promoted to selection or History authority unless they are derived from an admitted channel message or from already sealed History.
- History entries for successor promotion should retain enough references to revisit the child evidence store later: at minimum candidate node/branch/generation identity, selected membership/candidate-set evidence, selection input payload hashes, branch evaluation references, and a checkout/cherry-pickable artifact/tree/commit reference for the child records used by the decision. Existing `SelectionDecisionEntry`, `EvaluationPayload`, `CandidateArtifact`, candidate-set membership, and `Verifiable<Artifact, ArtifactLocator>` shapes are partial carriers for this; any missing child-store reference should be treated as migration debt before R13b/R14b live admission.
...
- Stopped/no-selection R14a final report emission is now admitted in the next slice; R13b handoff remains blocked.
...
- `walk` admits only `R13a -> R14a` stopped/no-selection final report.
- The edge uses canonical `r13_to_r14` wrapped with `R12ContinuationBranch::Stopped`, so R14b handoff-final remains unreachable from walk.
- Smoke reaches R14a and records completion/report axes; selected-successor handoff remains blocked earlier at R12.
