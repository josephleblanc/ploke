...
- Current gate: readiness and Phases 1-6 are complete; Phase 7 `C6` now has DB-side namespace inventory plus real subset remove/export/import primitives with duplicate conflict validation, but TUI/runtime subset command wiring is still missing
...
| Phase 7 `C6` namespace-scoped subset DB operations | `in progress` | `ploke-db` now has fixture-backed `remove_namespace(...)`, `export_namespace(...)`, and `import_namespace(...)` with duplicate namespace/name/root conflict validation; next step is wiring those primitives through subset commands so runtime membership, focus, IO roots, and search availability update end to end |
...
| Phase 7 `C6` namespace-scoped subset DB operations | `in progress` | `workspace_fixture_namespace_inventory_matches_crate_context_membership`; `workspace_fixture_namespaces_remain_distinct_in_subset_inventory`; `remove_namespace_removes_only_target_namespace_and_invalidates_search_state`; `export_namespace_artifact_contains_only_target_namespace_rows`; `import_namespace_restores_exported_namespace_into_populated_db_and_invalidates_search_state`; `import_namespace_reports_duplicate_namespace_name_and_root_conflicts` | Partial evidence only: DB-side namespace inventory plus real subset removal/export/import primitives now exist in `ploke-db`; targeted import/remove/export/conflict runs and broader `cargo test -p ploke-db --lib -- --nocapture` passed, but the end-to-end TUI/runtime subset path is still missing |
...
- Next target after compaction: wire the now-passing DB subset primitives into
  the first TUI/runtime subset command path so loaded membership, focus, IO
  roots, registry/snapshot metadata, and search availability update together.
