...
- Current gate: readiness and Phases 1-5 are complete; Phase 6 `C5` has started and is blocked on completing retrieval-scope proof plus broader regression cleanup
...
| Phase 6 `C5` shared retrieval scope model | `in progress` | Added shared `RetrievalScope` threading across `ploke-core`, `ploke-db`, `ploke-rag`, and `ploke-tui`, plus a BM25 witness proving `SpecificCrate` scope is applied before `top_k`; next step is dense/hybrid/context proof and resolving the `get_code_edges_regression` backup-freshness failure before broader validation can pass |
...
| Phase 6 `C5` shared retrieval scope model | `in progress` | `bm25_specific_crate_scope_filters_before_top_k_truncation` | First witness proves BM25 applies crate scope before `top_k`; targeted `ploke-db`, `ploke-rag`, `index_workspace_targets`, and `test_update_embed` runs passed, but broader `cargo test -p ploke-tui --tests -- --nocapture` is currently blocked by `get_code_edges_regression` tracking-hash freshness checks against `ploke_db_primary` after edits to `crates/ploke-db/src/database.rs` |
...
- Current implementation state: readiness and Phases 1-5 are complete; Phase 6
  `C5` is in progress.
- `C5` now has a first shared-scope slice landed:
  `RetrievalScope` exists in `ploke-core`, is threaded through dense/BM25/RAG
  entrypoints, and BM25 has a witness proving `SpecificCrate` scope is applied
  before `top_k`.
- Broader validation is not green yet. `cargo test -p ploke-tui --tests -- --nocapture`
  currently fails in
  `crates/ploke-tui/tests/get_code_edges_regression.rs` because the
  `ploke_db_primary` backup freshness guard now disagrees with the edited
  `crates/ploke-db/src/database.rs` tracking hash.
...
- Next target after compaction: finish `C5` by adding dense/hybrid/context
  scope witnesses and deciding whether to refresh `ploke_db_primary` or adjust
  the broader regression slice around the freshness-guarded
  `get_code_edges_regression` tests.
