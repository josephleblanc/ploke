...
- Current gate: readiness and Phases 1-7 are complete; the next implementation gate is Phase 8 `C7` workspace-aware tools with strict edit safety
...
| Phase 7 `C6` namespace-scoped subset DB operations | `done` | `ploke-db` has fixture-backed `remove_namespace(...)`, `export_namespace(...)`, and `import_namespace(...)`; `ploke-tui` now wires `/workspace rm <crate>` plus `load crates <workspace> <crate>` through those primitives and republishes loaded membership, focus, IO roots, search invalidation messaging, and registry/snapshot metadata coherently |
| Phase 8 `C7` workspace-aware tools with strict edit safety | `not started` | Expand read/context behavior without widening edit permissions; next step is workspace-aware prompt/context/tool gating on top of the completed workspace/session model |
...
| Phase 7 `C6` namespace-scoped subset DB operations | `done` | `workspace_fixture_namespace_inventory_matches_crate_context_membership`; `workspace_fixture_namespaces_remain_distinct_in_subset_inventory`; `remove_namespace_removes_only_target_namespace_and_invalidates_search_state`; `export_namespace_artifact_contains_only_target_namespace_rows`; `import_namespace_restores_exported_namespace_into_populated_db_and_invalidates_search_state`; `import_namespace_reports_duplicate_namespace_name_and_root_conflicts`; `workspace_remove_updates_runtime_membership_focus_and_snapshot_metadata`; `workspace_load_crates_restores_removed_member_and_snapshot_metadata`; `workspace_load_crates_conflict_preserves_runtime_state` | `ploke-db` has direct namespace-scoped remove/export/import/conflict witnesses, `ploke-tui` now has end-to-end subset remove and subset import/conflict witnesses, and both targeted `workspace_subset_remove` plus broader `cargo test -p ploke-tui --tests -- --nocapture` passed via sub-agent |
...
- `C6` now has DB-side groundwork plus real subset remove/export/import
  ...
  `import_namespace(...)`.
- `ploke-tui` now has end-to-end subset command paths for both
  `workspace rm <crate-name-or-exact-root>` and
  `load crates <workspace-name-or-id> <crate-name-or-exact-root>`, both wired
  through namespace-scoped `ploke-db` primitives and both republishing loaded
  membership, focus, IO roots, and registry/snapshot metadata coherently.
...
- New `C6` witnesses are recorded in
  ...
  plus `workspace_load_crates_restores_removed_member_and_snapshot_metadata`
  and `workspace_load_crates_conflict_preserves_runtime_state`.
