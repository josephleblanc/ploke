...
| D14 | `(async || f())()` | `fixture_call_graph::call_async_closure_literal_with_body_call` | outer `DynamicCall`; nested async-closure body calls are owned by a closure executable owner labeled `async_closure`, not by the enclosing owner | outer call is `Unsupported`, no edge; the body owner can traverse its own calls, but poll/resume edges for the returned future remain future |
