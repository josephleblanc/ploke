...
| X10 | item macro inside body | `fixture_call_graph::call_item_macro_inside_body`, `fixture_call_graph::call_item_macro_generated_function`, `fixture_call_graph::call_const_item_macro_generated_const_initializer`, `fixture_call_graph::call_static_item_macro_generated_static_initializer` | statement-position `MacroCall`; for bounded no-arg one-item macros, an additional generated `LocalItem` owner is recorded for exactly one local `fn`, one local `const`, or one local `static` | default remains `Unsupported`; the bounded generated local `fn` case resolves only the later source-visible `generated_by_item_macro()` call through `LocalFunction -> LocalItem`, and the bounded generated local `const` / `static` cases own initializer calls under `local_const` / `local_static` |
| X11 | macro expands to call/item | RA call hierarchy has FIXME outgoing macro gaps; fixture fallback covers no-arg one-function, one-const, and one-static local item macros | invocation only by default; bounded local item owner only when a unique already-seen no-arg `macro_rules!` definition expands to exactly one supported local item | no broad expanded edge until backend; current bounded cases use existing local-item traversal and keep the macro invocation targetless |
