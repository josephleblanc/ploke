...
- `make_closure()()` where the maker returns a closure literal -> inner `PathCall` for `make_closure()`, `Resolved(LocalExact)`, plus outer `DynamicCall` with `ReturnedPathCall`, `Unsupported`, no edge to the returned closure body without return-value flow proof.
...
| `fixture_call_graph` | `src/lib.rs:1431` | `make_closure()()` where `make_closure` returns a closure literal | D04/P30 | **green** | Covered by `fixture_call_graph_call_returned_closure_resolves_inner_make_closure_path_call_site` and `fixture_call_graph_call_returned_closure_records_unsupported_outer_dynamic_call_site`; the inner maker call resolves, while the outer returned-path dynamic call fails closed as `Unsupported` with fixture-backed DB context/proof blocker rows and no dynamic edge to the closure body. |
| `fixture_call_graph` | `src/lib.rs:1371,1435` | `assoc_const_value()` in function-local const/static initializers | owner matrix local item | **green** | Covered by `fixture_call_graph_local_const_initializer_call_is_not_recorded_as_outer_call_site`, `fixture_call_graph_local_static_initializer_call_is_not_recorded_as_outer_call_site`, and table-driven DB owner/snippet tests; initializer calls are owned by `LocalItem` executable owners labeled `local_const` and `local_static`, not by the enclosing function owner. |
...
| Function-local const/static initializer | `fn f() { const X: i32 = g(); static Y: i32 = h(); }` | yes for local const/static initializers | Inner calls are owned by `CallBodyOwnerId::Executable(LocalItemBodyId)` with `owner_kind = "LocalItem"` and labels such as `local_const` / `local_static`; they are not attributed to the enclosing function owner | Broader function-local item bodies remain future. |
...
| P30 | chained call callee `f()(1)` / returned closure call `make_closure()()` | `fixture_call_graph::{call_chained_returned_function, call_returned_closure}` | inner `PathCall`, outer `DynamicCall` with `ReturnedPathCall`, preserving outer argument count when present | inner resolved to exact local function when proven; outer resolves only when the maker body directly returns a function path, and returned closure literals remain `Unsupported` with no fabricated edge | function + dynamic |
...
| D04 | `make_fn()()` / `make_closure()()` | `fixture_call_graph::{call_returned_function, call_returned_closure}` | inner `PathCall`, outer `DynamicCall` with `ReturnedPathCall` proof | inner maker call resolves if local function; outer call resolves to the direct function path returned by the maker, while returned closure literals fail closed without an edge |
