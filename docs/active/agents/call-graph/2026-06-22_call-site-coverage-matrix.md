...
| D14 | `(async || f())()` / `(async || f())().await` | `fixture_call_graph::{call_async_closure_literal_with_body_call, call_awaited_async_closure_literal_with_body_call}` | outer `DynamicCall`; nested async-closure body calls are owned by a closure executable owner labeled `async_closure`, not by the enclosing owner; the awaited form records `AwaitedAsyncClosureLiteral` only when the dynamic call is the immediate base of `.await` | non-awaited outer call is `Unsupported`, no edge; immediate awaited outer call resolves to `CallRelation::DynamicClosure` and can traverse through the async-closure owner; broader poll/resume/value-flow remains future |
