...
| Calls inside closure/async/block bodies | Executable-owner model | Closure, async, and executable-local item bodies are explicit extraction boundaries today; inner calls are attributed to typed executable owners instead of the enclosing function owner when owner metadata exists. |
...
- `let source = LocalAssoc; let value = source; value.instance_value()` -> `MethodCall` with `InitializedLocalBinding { name: "value", init_path: ["LocalAssoc"] }`, `Resolved(LocalExact)`, `CallRelation::Method` through one local initializer alias.
- `let (value, _) = (LocalAssoc, 0); value.instance_value()` -> `MethodCall` with `InitializedLocalBinding { name: "value", init_path: ["LocalAssoc"] }`, `Resolved(LocalExact)`, `CallRelation::Method` through direct same-arity tuple-pattern binding proof.
...
- `local_target()` inside closure and async block bodies in `fixture_call_graph` -> nested executable-owner call sites through `ClosureBodyId` and `AsyncBlockBodyId`; no fabricated outer-owner call site.
...
- `alias_checker(&_trait_user)` where `alias_checker` is a local closure binding -> `PathCall` with `ClosureBinding { path: ["alias_checker"], closure_id }`, `Resolved(LocalExact)`, `CallRelation::Closure`, so syntactic path calls to closure bindings reach the closure executable owner without fake local function edges.
- `let local_target = || 377; local_target()` -> `PathCall` with `ClosureBinding { path: ["local_target"], closure_id }`, `Resolved(LocalExact)`, `CallRelation::Closure`, so local closure bindings do not fake-resolve to same-named module functions.
...
- `(closure)()` and non-async closure literals such as `(|| 11)()` -> `DynamicCall`, `Resolved(LocalExact)`, `CallRelation::DynamicClosure` targeting a closure executable owner.
- `(async || local_target())().await` -> `DynamicCall` with immediate await proof, `Resolved(LocalExact)`, `CallRelation::DynamicClosure` targeting the async-closure executable owner; non-awaited async-closure calls remain targetless.
- `make_fn()()` -> inner `PathCall` for `make_fn()`, `Resolved(LocalExact)`, plus outer `DynamicCall` with `ReturnedPathCall`, `Resolved(LocalExact)`, `CallRelation::DynamicFunction` to the direct returned function path.
- `make_unary_fn()(5)` -> inner `PathCall` for `make_unary_fn()`, `Resolved(LocalExact)`, plus outer one-argument `DynamicCall` with `ReturnedPathCall`, `Resolved(LocalExact)`, `CallRelation::DynamicFunction` to the direct returned function path.
- `make_closure()()` where the maker directly returns a closure literal -> inner `PathCall` for `make_closure()`, `Resolved(LocalExact)`, plus outer `DynamicCall` with `ReturnedPathCall`, `Resolved(LocalExact)`, `CallRelation::DynamicClosure` to the closure executable owner.
- `make_bound_closure()()` where the maker returns a local closure binding initialized by a closure literal -> inner `PathCall` for `make_bound_closure()`, `Resolved(LocalExact)`, plus outer `DynamicCall` with `ReturnedPathCall`, `Resolved(LocalExact)`, `CallRelation::DynamicClosure` to the single recorded closure executable owner.
...
- `(match flag { true => match flag { true => local_target, false => local_target }, false => local_target })()` -> `DynamicCall` with `MatchArmPaths`, `Resolved(LocalExact)`, `CallRelation::DynamicFunction` when every nested arm leaf is an unshadowed item path resolving to the same local function.
- `(match flag { true if flag => local_target, _ => local_target })()` -> `DynamicCall` with `MatchArmPaths`, `Resolved(LocalExact)`, `CallRelation::DynamicFunction` when every guarded or unguarded arm body path resolves to the same local function.
...
failures, bare callable-value path failures, fail-closed opaque/non-path branch
...
including guarded same-target match arms,
unsupported
...
`type_resolution_missing` blockers, opaque/non-path branch/match dynamic calls
as `dynamic_dispatch_unbounded` blockers, and target-centered projection
...
| `fixture_nodes` | `src/imports.rs:134` | `alias_checker(&_trait_user)` | P25, D12/D13 policy | **green** | Covered by `fixture_nodes_use_imported_items_records_alias_checker_closure_binding_path_call_site`; syntactically path call to closure binding records `ClosureBinding` and resolves to the closure executable owner. |
...
| `fixture_edge_cases` | `src/lib.rs:51` | `T::default()` in `GenericItem::new` | P14/P20-like | **green** | Covered by `fixture_edge_cases_generic_item_new_records_t_default_external_path_call_site`; generic type-parameter associated-function-looking path calls with source-visible external/prelude `Default` bounds are recorded structurally and classified as targetless `External` frontiers without claiming concrete trait dispatch. |
...
| `fixture_generics` | `src/lib.rs:32,62` | `T::default()` in `generic_function`, `format!(...)` in generic trait impl method | P14/P20-like, X02/X09 | **green** | Covered by `fixture_generics_generic_function_records_t_default_external_path_call_site` and `fixture_generics_trait_impl_process_records_format_macro_call_site`; generic type-parameter associated-function-looking path calls with source-visible external/prelude `Default` bounds classify as targetless `External` frontiers, and macro invocation is recorded structurally without expansion. |
...
| `fixture_call_graph` | `src/lib.rs:6` | `(|| 11)()` | D03 | **green** | Covered by `fixture_call_graph_dynamic_calls_records_closure_literal_dynamic_call_site`; pathless non-async closure literal calls resolve to `CallRelation::DynamicClosure` targeting the closure executable owner. |
| `fixture_call_graph` | `src/lib.rs:710` | `(move || local_target())()` | D03 / owner matrix closure | **green** | Covered by `fixture_call_graph_call_move_closure_literal_with_body_call_records_outer_dynamic_call_site` and `fixture_call_graph_move_closure_body_call_is_not_recorded_as_outer_call_site`; outer dynamic call targets the closure executable owner, and the inner `local_target()` call remains owned by that closure rather than the enclosing function. |
...
| `fixture_call_graph` | `src/lib.rs:37` | `make_fn()()` | D04/P30 | **green** | Covered by `fixture_call_graph_call_returned_function_resolves_inner_make_fn_path_call_site` and `fixture_call_graph_call_returned_function_resolves_outer_dynamic_call_site`; nested visitor records the inner path call and the outer returned-path dynamic call, resolving the outer edge to `local_target`. |
| `fixture_call_graph` | `src/lib.rs:751` | `make_unary_fn()(5)` | P30 | **green** | Covered by `fixture_call_graph_call_chained_returned_function_resolves_inner_make_unary_fn_path_call_site` and `fixture_call_graph_call_chained_returned_function_resolves_outer_dynamic_call_site`; the outer dynamic call records `arg_count = 1` and resolves to `unary_target` when the returned function body is a direct function path. |
| `fixture_call_graph` | `src/lib.rs:1431` | `make_closure()()` where `make_closure` directly returns a closure literal | D04/P30 | **green** | Covered by `fixture_call_graph_call_returned_closure_resolves_inner_make_closure_path_call_site` and `fixture_call_graph_call_returned_closure_resolves_outer_dynamic_closure_call_site`; the inner maker call resolves to `make_closure`, while the outer returned-path dynamic call resolves to `CallRelation::DynamicClosure` targeting the returned closure executable owner. |
| `fixture_call_graph` | `src/lib.rs:1453` | `make_bound_closure()()` where `make_bound_closure` returns a local closure binding | D04/P30 | **green** | Covered by `fixture_call_graph_call_returned_bound_closure_resolves_inner_make_bound_closure_path_call_site` and `fixture_call_graph_call_returned_bound_closure_resolves_outer_dynamic_closure_call_site`; the inner maker call resolves to `make_bound_closure`, while the outer returned-path dynamic call resolves to `CallRelation::DynamicClosure` when the maker has exactly one recorded closure executable owner and the final expression returns that local binding. |
| `fixture_call_graph` | `src/lib.rs:1371,1435,1441` | `assoc_const_value()` in function-local const/static initializers and local `fn` body | owner matrix local item | **green** | Covered by `fixture_call_graph_local_const_initializer_call_is_not_recorded_as_outer_call_site`, `fixture_call_graph_local_static_initializer_call_is_not_recorded_as_outer_call_site`, `fixture_call_graph_local_fn_body_call_is_not_recorded_as_outer_call_site`, and table-driven DB/RAG/TUI owner tests; body calls are owned by `LocalItem` executable owners labeled `local_const`, `local_static`, and `local_fn`, not by the enclosing function owner. |
...
| `fixture_call_graph` | `src/lib.rs:1423` | `value.instance_value()` after `let source = LocalAssoc; let value = source` | M05 | **green** | Covered by `fixture_call_graph_call_initialized_local_alias_instance_method_resolves_initialized_local_binding_method_call_site`; one local initializer alias preserves the exact local receiver type proof for parser and fixture-backed DB query rows. |
...
| `fixture_call_graph` | `src/lib.rs:658` | `(match flag { true if flag => local_target, _ => local_target })()` | D06 | **green** | Covered by `fixture_call_graph_call_match_guarded_function_item_resolves_dynamic_function_call_site`; guarded arm bodies enter the exact arm-path resolver and emit one `CallRelation::DynamicFunction` when they prove the same local function. |
...
| `fixture_call_graph` | `src/lib.rs:721` | `(if flag { f } else { f })()` where public `f: fn() -> i32` is a parameter | D05/P26 | **green** | Covered by `fixture_call_graph_call_if_function_pointer_param_branch_fails_closed_dynamic_call_site`; public/unproven parameter branches record a parameter-specific dynamic callee path and remain unsupported with no fabricated target. |
| `fixture_call_graph` | `src/lib.rs:725` | `(match flag { true => f, false => f })()` where public `f: fn() -> i32` is a parameter | D06/P26 | **green** | Covered by `fixture_call_graph_call_match_function_pointer_param_arm_fails_closed_dynamic_call_site`; public/unproven parameter arms record a parameter-specific dynamic callee path and remain unsupported with no fabricated target. |
| `fixture_call_graph` | `src/lib.rs:1655` | `(if flag { f } else { f })()` in private `call_single_if_function_pointer_param_branch` | D05/P26 | **green** | Covered by `fixture_call_graph_call_single_if_function_pointer_param_branch_resolves_single_caller_argument`; the parameter-specific branch carrier resolves through the existing complete private single-caller argument proof when the only local caller passes `local_target`. |
| `fixture_call_graph` | `src/lib.rs:1663` | `(match flag { true => f, false => f })()` in private `call_single_match_function_pointer_param_arm` | D06/P26 | **green** | Covered by `fixture_call_graph_call_single_match_function_pointer_param_arm_resolves_single_caller_argument`; the parameter-specific match carrier resolves through the existing complete private single-caller argument proof when the only local caller passes `local_target`. |
...
| `fixture_call_graph` | `src/lib.rs:158` | `local_target()` inside async block | owner matrix async | **green** | Covered by `fixture_call_graph_async_block_call_is_not_recorded_as_outer_call_site`; parser records the nested call under `CallBodyOwnerId::Executable(AsyncBlockBodyId)` and does not attribute it to the outer function owner. |
| `fixture_call_graph` | `src/lib.rs:849` | `(async || local_target())()` | D14 / owner matrix closure | **green** | Covered by `fixture_call_graph_call_async_closure_literal_with_body_call_records_outer_dynamic_call_site` and `fixture_call_graph_async_closure_body_call_is_not_recorded_as_outer_call_site`; the outer async closure call is an unsupported targetless `DynamicCall`, and the inner closure-body call is owned by an `async_closure` executable body instead of the outer function. |
| `fixture_call_graph` | `src/lib.rs:1531` | `(async || local_target())().await` | D14 / owner matrix closure | **green** | Covered by `fixture_call_graph_call_awaited_async_closure_literal_with_body_call_resolves_outer_dynamic_call_site`, `fixture_call_graph_awaited_async_closure_body_call_is_not_recorded_as_outer_call_site`, and `fixture_context_resolves_awaited_async_closure_literal_call_to_executable_owner`; the immediate `.await` is the proof that the outer dynamic call can target the async-closure owner, while the body call remains owned by that executable owner. |
| `fixture_call_graph` | EOF appended case | `let futures = (closure(),); futures.0.await` where `closure` is an async closure binding | D14 / owner matrix closure | **green** | Covered by `fixture_call_graph_call_awaited_async_closure_future_tuple_field_resolves_path_call_site`, DB `fixture_context_resolves_awaited_async_closure_future_tuple_field_to_executable_owner`, RAG async future tuple-field collection, and exact lookup/edges tests; bounded same-block tuple-field poll proof marks only the original `closure()` call as awaited and traverses through the async-closure owner without modeling returned futures or arbitrary aggregate value flow. |
| `fixture_call_graph` | EOF appended case | `let holder = AsyncFutureHolder { future: closure() }; holder.future.await` and `let alias = holder.future; alias.await` where `closure` is an async closure binding | D14 / owner matrix closure | **green** | Covered by `fixture_call_graph_call_awaited_async_closure_future_named_field_resolves_path_call_site`, `fixture_call_graph_call_awaited_async_closure_future_named_field_alias_resolves_path_call_site`, DB named-field / named-field-alias context tests, RAG async future named-field collection, and exact lookup/edges tests; bounded same-block named-field poll proof marks only the original `closure()` call as awaited and traverses through the async-closure owner without modeling returned futures or arbitrary aggregate value flow. |
...
| `fixture_call_graph` | `src/lib.rs:182` | `local_target()` after `let local_target = || 377` | P25 | **green** | Covered by `fixture_call_graph_call_shadowed_local_target_binding_records_closure_binding_path_call_site`; records closure-binding path callee, resolves to the closure executable owner, and does not fake an edge to the same-named module function. |
...
| `fixture_call_graph` | `src/lib.rs:1522-1527` | private helper `(f)()` where `f: fn() -> i32` has one local caller passing `local_target` | D01/P26 | **green** | Covered by `fixture_call_graph_call_single_parenthesized_function_pointer_param_resolves_single_caller_argument`; the same private bare-function-pointer single-caller proof used for `f()` resolves the parenthesized dynamic call to `CallRelation::DynamicFunction`, while public `(f)()` remains targetless. |
...
| `fixture_call_graph` | `src/lib.rs:689` | `(closure as fn() -> i32)()` where `closure` is a local closure binding | D11/P25 | **green** | Covered by `fixture_call_graph_call_closure_binding_cast_resolves_dynamic_closure_call_site`; local closure-binding casts preserve closure-owner proof and resolve to `CallRelation::DynamicClosure`. |
| `fixture_call_graph` | `src/lib.rs:694` | `(*closure)()` where `closure` is a local closure binding | D10/P25 | **green** | Covered by `fixture_call_graph_call_dereferenced_closure_binding_resolves_dynamic_closure_call_site`; exact local closure-binding deref proof records `DereferencedClosureBinding` and resolves to `CallRelation::DynamicClosure`. |
...
| `fixture_call_graph` | `src/lib.rs:878` | `holder.callbacks[0]()` where `holder: CallbackArrayHolder` is a parameter | D09 | **green** | Covered by `fixture_call_graph_call_indexed_field_function_param_fails_closed_dynamic_call_site`; indexed field callees rooted at opaque parameters preserve the `holder.callbacks.0` path but remain unsupported and edge-free. |
| `fixture_call_graph` | `src/lib.rs:1534-1539` | private helper `holder.callbacks[0]()` where the only local caller constructs `CallbackArrayHolder { callbacks: [local_target] }` | D09/P26 | **green** | Covered by `fixture_call_graph_call_single_indexed_field_function_param_resolves_single_caller_field_argument`, `fixture_context_resolves_single_caller_indexed_field_function_parameters`, RAG dynamic-row collection, and lookup/edges tool tests; exact complete-local-caller field proof resolves to `CallRelation::DynamicFunction`, while public holder parameters remain targetless. |
...
| `fixture_call_graph` | `src/lib.rs:903` | `holder.0[0]()` where `holder: TupleCallbackArrayHolder` is a parameter | D09 | **green** | Covered by `fixture_call_graph_call_indexed_tuple_field_function_param_fails_closed_dynamic_call_site`; indexed tuple-field callees rooted at opaque parameters preserve the `holder.0.0` path but remain unsupported and edge-free. |
| `fixture_call_graph` | `src/lib.rs:1542-1547` | private helper `holder.0[0]()` where the only local caller constructs `TupleCallbackArrayHolder([local_target])` | D09/P26 | **green** | Covered by `fixture_call_graph_call_single_indexed_tuple_field_function_param_resolves_single_caller_field_argument`, `fixture_context_resolves_single_caller_indexed_field_function_parameters`, RAG dynamic-row collection, and lookup/edges tool tests; exact complete-local-caller tuple-field proof resolves to `CallRelation::DynamicFunction`, while public holder parameters remain targetless. |
...
| `fixture_call_graph` | `src/lib.rs:827-828`, `src/lib.rs:1696-1699` | `Box::new(local_target); (boxed_fn)()` / `(*boxed_fn)()` | D12/P14 | **green** | Covered by `fixture_call_graph_call_parenthesized_boxed_dyn_fn_value_binding_records_box_new_external_path_call_site`, `fixture_call_graph_call_parenthesized_boxed_dyn_fn_value_binding_resolves_dynamic_call_site`, and `fixture_call_graph_call_dereferenced_boxed_dyn_fn_value_binding_resolves_dynamic_call_site`; setup call is external, while exact `Box<dyn Fn()> = Box::new(local_target)` initializer proof resolves parenthesized and dereferenced dynamic calls to `CallRelation::DynamicFunction`. |
| `fixture_call_graph` | `src/lib.rs:2121-2124`, `src/lib.rs:2131-2134` | `(referenced_fn)()` where `referenced_fn` is `&dyn Fn()` or `&mut dyn FnMut()` initialized from `local_target` | D12 | **green** | Covered by `fixture_call_graph_call_parenthesized_referenced_dyn_fn_value_binding_resolves_dynamic_call_site` and `fixture_call_graph_call_parenthesized_mut_referenced_dyn_fnmut_value_binding_resolves_dynamic_call_site`; exact referenced callable trait-object initializer proof resolves to `CallRelation::DynamicFunction` without generalizing to unproven trait-object dispatch. |
| `fixture_call_graph` | `src/lib.rs:2148-2161` | `f()` / `(f)()` where private `f: &dyn Fn() -> i32` helper has exactly one local caller passing `&local_target` | P25/D12 | **green** | Covered by `fixture_call_graph_call_single_referenced_dyn_fn_param_resolves_single_caller_argument` and `fixture_call_graph_call_single_parenthesized_referenced_dyn_fn_param_resolves_single_caller_argument`; exact private-caller referenced-argument proof resolves the path call to `CallRelation::Function` and the parenthesized dynamic call to `CallRelation::DynamicFunction`, without treating public or unproven callable trait-object parameters as resolved. |
| `fixture_call_graph` | EOF appended case | `f()` / `(f)()` where private `f: Box<dyn Fn() -> i32>` helper has exactly one local caller passing `Box::new(local_target)` | P25/D12 | **green** | Covered by `fixture_call_graph_call_single_boxed_dyn_fn_param_resolves_single_caller_argument`, `fixture_call_graph_call_single_parenthesized_boxed_dyn_fn_param_resolves_single_caller_argument`, DB `fixture_context_resolves_single_caller_boxed_callable_trait_object_parameter`, RAG callable path collection, and lookup/edges callable-parameter tests; exact private-caller boxed-argument proof resolves the path call to `CallRelation::Function` and the parenthesized dynamic call to `CallRelation::DynamicFunction`, while public, unproven, field-held, and runtime-dispatched callable trait objects remain targetless. |
| `fixture_call_graph` | EOF appended case | `f()` where private `f: &dyn Fn() -> i32` is forwarded through one or two private helpers and the complete caller chain passes `&local_target` | P25/D12 | **green** | Covered by DB `fixture_context_resolves_forwarded_callable_trait_object_parameters`, RAG callable path collection, and lookup/edges callable-parameter matrices; bounded forwarding proof reuses the existing private complete-caller contract for referenced callable parameters and resolves only the proven `local_target` edge. |
| `fixture_call_graph` | EOF appended case | `f()` where private `f: Box<dyn Fn() -> i32>` is forwarded through one or two private helpers and the complete caller chain passes `Box::new(local_target)`; paired conflicting callers pass `Box::new(local_target)` and `Box::new(other_target)` | P25/D12 | **green** | Covered by parser `fixture_call_graph_call_forwarded_boxed_dyn_fn_leaf_resolves_forwarded_private_parameter`, `fixture_call_graph_call_two_hop_forwarded_boxed_dyn_fn_leaf_resolves_forwarded_private_parameter`, and `fixture_call_graph_call_forwarded_conflicting_boxed_dyn_fn_leaf_preserves_forwarded_candidates`, DB `fixture_context_resolves_forwarded_callable_trait_object_parameters` and `fixture_context_preserves_forwarded_conflicting_boxed_dyn_fn_candidates`, RAG callable path collection, and exact lookup/edges boxed-forwarding tests; bounded forwarding proof resolves only complete private boxed caller chains and preserves conflicting boxed function items as ambiguous candidates. |
...
| `fixture_call_graph` | EOF appended case | `call_graph_item_macro!(); generated_by_item_macro()` where a unique no-arg local `macro_rules!` item expands to one block-local `fn generated_by_item_macro()` | X10/X11 / local item owner | **green** | Covered by `fixture_call_graph_item_macro_generated_function_records_macro_call_site`, `fixture_call_graph_item_macro_generated_function_records_local_fn_owner`, `fixture_call_graph_item_macro_generated_function_resolves_local_function_path_call_site`, and DB `fixture_context_projects_macro_generated_local_fn_call_to_local_item_target`; the macro invocation remains targetless `Unsupported`, while the generated local `fn` is modeled through the existing `LocalItem` owner and `LocalFunction` edge. |
| `fixture_call_graph` | EOF appended case | `call_graph_const_item_macro!()` where a unique no-arg local `macro_rules!` item expands to one block-local `const` initializer calling `assoc_const_value()` | X10/X11 / local item owner | **green** | Covered by `fixture_call_graph_const_item_macro_generated_const_initializer_records_macro_call_site`, `fixture_call_graph_const_item_macro_generated_const_initializer_records_local_const_owner`, `fixture_call_graph_const_item_macro_generated_const_initializer_owns_initializer_call`, and DB/RAG/TUI executable-owner tests; the macro invocation remains targetless `Unsupported`, while the generated local `const` initializer is modeled as a `local_const` `LocalItem` owner and the initializer call belongs to that owner, not the enclosing function. |
| `fixture_call_graph` | EOF appended case | `call_graph_static_item_macro!()` where a unique no-arg local `macro_rules!` item expands to one block-local `static` initializer calling `assoc_const_value()` | X10/X11 / local item owner | **green** | Covered by `fixture_call_graph_static_item_macro_generated_static_initializer_records_macro_call_site`, `fixture_call_graph_static_item_macro_generated_static_initializer_records_local_static_owner`, `fixture_call_graph_static_item_macro_generated_static_initializer_owns_initializer_call`, and DB/RAG/TUI executable-owner tests; the macro invocation remains targetless `Unsupported`, while the generated local `static` initializer is modeled as a `local_static` `LocalItem` owner and the initializer call belongs to that owner, not the enclosing function. |
| `fixture_call_graph` | EOF appended case | `call_graph_expr_path_macro!()` where a unique no-arg local `macro_rules!` item expands to one `local_target();` path-call statement | X10/X11 / generated source-body path call | **green** | Covered by `fixture_call_graph_expr_macro_generated_path_call_records_macro_call_site`, `fixture_call_graph_expr_macro_generated_path_call_resolves_generated_path_call_site`, DB `fixture_context_projects_macro_generated_expr_path_call_to_outer_owner`, RAG `call_context_collection_reads_macro_generated_expr_path_call`, and exact TUI lookup/edges tests; the macro invocation remains targetless `Unsupported`, while the generated path call is owned by the enclosing function and resolves as a one-hop `Function` edge to `local_target`. |
...
| `fixture_call_graph` | `src/lib.rs:2294` | `Ok::<i32, ()>(1).and_then(f)` in private `call_single_result_callback(f)` where the only local caller passes `local_result_target` | M27/P26 | **green** | Covered by `fixture_call_graph_call_single_result_callback_resolves_method_callback_function_call_site`, `fixture_context_resolves_result_method_callback_function`, RAG collection, and exact lookup/edges tool tests; records a `PathCallResult` method receiver and resolves the callback argument to `CallRelation::MethodCallbackFunction` only under complete private-caller proof. |
...
| Function-local const/static/local-fn body | `fn f() { const X: i32 = g(); static Y: i32 = h(); fn inner() { z(); } }` | yes for local const/static initializers and local `fn` bodies | Inner calls are owned by `CallBodyOwnerId::Executable(LocalItemBodyId)` with `owner_kind = "LocalItem"` and labels such as `local_const` / `local_static` / `local_fn`; they are not attributed to the enclosing function owner | Calls to local `fn` items themselves remain targetless until local item definitions become callable targets. |
...
| P25 | `closure_binding()` / same-name shadowing `local_target()` / function-item binding `f()` or alias `g()` | `fixture_call_graph::{call_shadowed_local_target_binding, call_local_function_item_binding, call_imported_function_item_binding, call_parenthesized_function_item_binding, call_aliased_function_item_binding, call_parenthesized_aliased_function_item_binding, call_closure_binding_cast, call_dereferenced_closure_binding}` | syntactically `PathCall` with closure-binding, value-binding, or initialized-value-binding callee classifier when local binding is visible; parenthesized form is `DynamicCall` with matching initialized-binding or closure-binding proof; local closure-binding `as fn` casts and derefs keep closure-owner proof | `Resolved(LocalExact)` for exact local function-item bindings, imported initializer paths, exact local aliases, closure bindings with executable owners, local closure-binding `as fn` casts, and exact local closure-binding derefs | broader Fn trait dispatch |
| P26 | `fn_ptr()` / typed function pointer binding `f()` or alias `g()` | `fixture_call_graph::{call_typed_function_pointer_binding, call_typed_function_pointer_alias_binding, call_parenthesized_typed_function_pointer_alias_binding, call_function_pointer_param, call_parenthesized_function_pointer_param, call_function_pointer_param_cast, call_single_function_pointer_param, call_single_parenthesized_function_pointer_param}` | syntactically `PathCall` for bare calls and `DynamicCall` for parenthesized/cast calls; local binding keeps initializer path proof when available, function-pointer casts over opaque parameters preserve the local binding path, and private bare-function-pointer parameters can reuse complete single-caller argument proof | `Resolved(LocalExact)` for typed function pointer bindings initialized directly from one local function or exact alias of one; private bare-function-pointer parameters resolve only when every local caller supplies the same exact callable target; public/unproven function-pointer parameters fail closed with `Unsupported` and no edge | fn pointer |
| P27 | generic `F: Fn`, `f()` | `fixture_call_graph::{call_generic_fn_once_value_binding, call_single_generic_fn_once_param, call_multi_generic_fn_once_param, call_multi_conflicting_generic_fn_once_param}` | syntactically `PathCall`; semantic Fn impl | Public or unproven generic value-binding calls remain `Unsupported` with no edge; private generic callable parameters resolve only when every local caller supplies the same exact callable target; conflicting caller targets fail closed with `Unsupported` and no edge | Fn trait impl |
...
| P30 | chained call callee `f()(1)` / returned closure calls `make_closure()()` and `make_bound_closure()()` | `fixture_call_graph::{call_chained_returned_function, call_returned_closure, call_returned_bound_closure}` | inner `PathCall`, outer `DynamicCall` with `ReturnedPathCall`, preserving outer argument count when present | inner resolved to exact local function when proven; outer resolves when the maker body directly returns either a function path, a single closure literal executable owner, or a local binding initialized by that single closure executable owner | function + dynamic |
...
| M05 | `local.clone()` / parameter `value.instance_value()` / parameter alias `alias.instance_value()` / borrowed parameter alias after `let alias = &value` / typed local `value.instance_value()` / path-initialized local `value.instance_value()` / path-initialized local alias `value.instance_value()` / tuple-pattern local `value.instance_value()` / struct-literal initialized local `x.method()` / parenthesized local `(value).method()` | `fixture_call_graph::{call_param_instance_method, call_param_alias_instance_method, call_borrowed_param_alias_instance_method, call_typed_local_instance_method, call_initialized_local_instance_method, call_initialized_local_alias_instance_method, call_tuple_pattern_local_instance_method, call_parenthesized_typed_local_instance_method, call_parenthesized_initialized_local_trait_method, call_type_alias_chain_instance_method, call_imported_type_alias_instance_method}` plus `fixture_impls::main` | local binding receiver | `Resolved(LocalExact)` for named owner parameters, direct local aliases of owner parameters, one-hop borrowed aliases of owner parameters, explicit local type annotations including bounded Rust type-alias chains and imported Rust type aliases, path-initialized locals, one local alias of path-initialized locals, direct same-arity tuple-pattern locals, struct-literal initialized locals, and parenthesized local receivers with direct local proof; broader untyped locals still future | `MethodNodeId` or external |
...
| M27 | `result.and_then(f)` where `f` is a callback parameter | `fixture_call_graph::call_single_result_callback` and axum `expand_with` | path-call result receiver plus callback argument proof | `Resolved(LocalExact)` to `CallRelation::MethodCallbackFunction` only when complete private-caller proof shows every local caller passes the same function item; finite ambiguous candidate rows remain candidate-only and edge-free | callback function target |
...
| X10 | item macro inside body | `fixture_call_graph::call_item_macro_inside_body`, `fixture_call_graph::call_item_macro_generated_function`, `fixture_call_graph::call_const_item_macro_generated_const_initializer`, `fixture_call_graph::call_static_item_macro_generated_static_initializer`, `fixture_call_graph::call_expr_macro_generated_path_call` | statement-position `MacroCall`; for bounded no-arg one-item macros, an additional generated `LocalItem` owner is recorded for exactly one local `fn`, one local `const`, or one local `static`; for a bounded no-arg single path-call statement macro, the generated path call is recorded under the enclosing owner | default remains `Unsupported`; the bounded generated local `fn` case resolves only the later source-visible `generated_by_item_macro()` call through `LocalFunction -> LocalItem`, the bounded generated local `const` / `static` cases own initializer calls under `local_const` / `local_static`, and the bounded generated path-call statement resolves to its local function target |
| X11 | macro expands to call/item | RA call hierarchy has FIXME outgoing macro gaps; fixture fallback covers no-arg one-function, one-const, one-static local item macros, and one path-call statement macro | invocation only by default; bounded generated rows only when a unique already-seen no-arg `macro_rules!` definition expands to exactly one supported local item or exactly one supported path-call statement | no broad expanded edge until backend; current bounded cases use existing local-item or path-call traversal and keep the macro invocation targetless |
...
| D02 | `(|| 1)()` | `fixture_call_graph::dynamic_calls` | `DynamicCall` with `ClosureLiteral` proof for non-async closure literals | `Resolved(LocalExact)`, `CallRelation::DynamicClosure` targeting the closure executable owner |
| D03 | `(move || f())()` | `fixture_call_graph::call_move_closure_literal_with_body_call` | Outer expression remains `DynamicCall`; nested closure body path calls are owned by `CallBodyOwnerId::Executable(ClosureBodyId)` and are not attributed to the enclosing owner | outer non-async closure literal call resolves to `CallRelation::DynamicClosure`; closure-body calls remain owned by the closure owner |
| D04 | `make_fn()()` / `make_closure()()` / `make_bound_closure()()` | `fixture_call_graph::{call_returned_function, call_returned_closure, call_returned_bound_closure}` | inner `PathCall`, outer `DynamicCall` with `ReturnedPathCall` proof | inner maker call resolves if local function; outer call resolves to the direct function path, direct closure literal, or local closure binding returned by the maker when exactly one closure executable owner is recorded |
| D05 | `(if cond { f } else { g })()` | `fixture_call_graph::{call_if_same_function_item, call_if_ambiguous_function_item, call_if_closure_branch, call_if_function_pointer_param_branch, call_single_if_function_pointer_param_branch, call_if_nested_branch_expression}` | `DynamicCall` with `IfBranchPaths` for unshadowed item-path branch leaves, including nested branch expressions; same-parameter callable branches use a parameter-specific carrier; non-path branches remain plain dynamic calls | `Resolved(LocalExact)` when all item branch paths prove the same local function or when the same parameter branch resolves through complete private single-caller argument proof; `Ambiguous` with no edge when item branch paths prove different local functions; public/unproven parameter and non-path branches remain unsupported |
| D06 | `(match x { A => f, B => g })()` | `fixture_call_graph::{call_match_same_function_item, call_match_ambiguous_function_item, call_match_guarded_function_item, call_match_closure_arm, call_match_function_pointer_param_arm, call_single_match_function_pointer_param_arm, call_match_nested_arm_expression}` | `DynamicCall` with `MatchArmPaths` for unshadowed item-path arm leaves, including nested and guarded arm expressions; same-parameter callable arms use a parameter-specific carrier; non-path arms remain plain dynamic calls | `Resolved(LocalExact)` when all item arm paths prove the same local function or when the same parameter arms resolve through complete private single-caller argument proof; `Ambiguous` with no edge when item arm paths prove different local functions; public/unproven parameter and non-path arms remain unsupported |
...
| D09 | `funcs[0]()` / `alias[0]()` / `holder.callbacks[0]()` / `holder.0[0]()` | `fixture_call_graph::{call_indexed_function_pointer, call_indexed_field_function_param, call_indexed_tuple_field_function_param, call_indexed_initialized_function_array, call_typed_indexed_initialized_function_array, call_aliased_indexed_initialized_function_array, call_indexed_named_field_function_binding, call_indexed_named_field_array_alias_binding, call_aliased_indexed_named_field_function_binding, call_indexed_tuple_field_function_binding, call_indexed_tuple_field_array_alias_binding, call_aliased_indexed_tuple_field_function_binding}` | `DynamicCall`; opaque indexed parameters and indexed opaque fields record `FieldLocalBinding` with the literal index segment preserved in the path; exact local array-literal index proof records `IndexedInitializedLocalBinding` for untyped, typed, one-step aliased, constructed named-field, constructed tuple-field, array-alias field, and constructed-holder alias bindings | opaque indexed callees remain `Unsupported` and targetless; exact path-valued array elements resolve when the literal index selects one proven local function |
| D10 | `(*fp)()` / `(*closure)()` | `fixture_call_graph::{call_dereferenced_function_pointer_binding, call_dereferenced_closure_binding}` | `DynamicCall` with `DereferencedInitializedLocalBinding` when the dereferenced operand is an initialized local binding; `DereferencedClosureBinding` when the dereferenced operand is an exact local closure binding; opaque deref operands remain plain dynamic calls | `Resolved(LocalExact)` when the initializer path resolves to one local function; exact local closure-binding derefs resolve to `DynamicClosure`; opaque deref calls remain unsupported |
| D11 | `(foo as fn())()` | `fixture_call_graph::{call_function_pointer_cast_path, call_function_pointer_cast_binding, call_closure_binding_cast}` | `DynamicCall` with `FnPointerCastPath` for unshadowed paths, `FnPointerCastInitializedLocalBinding` for initialized local bindings, or `FnPointerCastClosureBinding` for local closure bindings with executable-owner proof | `Resolved(LocalExact)` when the cast operand or initializer path resolves to one local function; closure-binding casts resolve to `DynamicClosure`; opaque value casts remain unsupported |
| D12 | `boxed_fn()` / `(boxed_fn)()` / `(*boxed_fn)()` where `boxed_fn: Box<dyn Fn()>`; `(referenced_fn)()` where `referenced_fn: &dyn Fn()` or `&mut dyn FnMut()`; private helper `f()` / `(f)()` where `f: &dyn Fn()` and the complete local caller set passes `&local_target`; private helper `f()` / `(f)()` where `f: Box<dyn Fn()>` and the complete local caller set passes `Box::new(local_target)` | `fixture_call_graph::{call_boxed_dyn_fn_value_binding, call_parenthesized_boxed_dyn_fn_value_binding, call_dereferenced_boxed_dyn_fn_value_binding, call_parenthesized_referenced_dyn_fn_value_binding, call_parenthesized_mut_referenced_dyn_fnmut_value_binding, call_single_referenced_dyn_fn_param, call_single_parenthesized_referenced_dyn_fn_param, call_single_boxed_dyn_fn_param, call_single_parenthesized_boxed_dyn_fn_param}` | syntactically `PathCall` unless parenthesized or dereferenced dynamic; exact callable trait-object initializer proof is preserved for `Box<dyn Fn()> = Box::new(local_target)`, `&dyn Fn() = &local_target`, and `&mut dyn FnMut() = &mut target` where `target` is initialized from `local_target`; private helper parameters can also use exact referenced-argument or boxed-argument proof when every local caller supplies the same callable target through the matching parameter type | direct public/unproven path calls remain value-binding rows; parenthesized and dereferenced dynamic calls resolve to `CallRelation::DynamicFunction` only with exact initializer or complete-private-caller proof, and the private `f()` path call resolves to `CallRelation::Function`. Broader callable trait objects without proof remain unsupported with no edge |
...
| D14 | `(async || f())()` / `(async || f())().await`; `let future = closure(); future.await`; `let futures = (closure(),); futures.0.await`; `let holder = AsyncFutureHolder { future: closure() }; holder.future.await`; `let alias = holder.future; alias.await` | `fixture_call_graph::{call_async_closure_literal_with_body_call, call_awaited_async_closure_literal_with_body_call, call_awaited_async_closure_future_binding_with_body_call, call_awaited_async_closure_future_tuple_field_with_body_call, call_awaited_async_closure_future_named_field_with_body_call, call_awaited_async_closure_future_named_field_alias_with_body_call}` | outer `DynamicCall` or async-closure binding `PathCall`; nested async-closure body calls are owned by a closure executable owner labeled `async_closure`, not by the enclosing owner; awaited forms record `AwaitedAsyncClosureLiteral` or `AwaitedAsyncClosureBinding` only when immediate await or bounded same-block future binding evidence proves the returned future is polled | non-awaited outer calls are `Unsupported`, no edge; immediate awaited and bounded same-block direct/alias/tuple-field/named-field/named-field-alias awaited calls resolve to the async-closure owner and can traverse through that owner; returned futures, arbitrary aggregate flow, and broader poll/resume/value-flow remain future |
...
| Method callback function | method callback argument proof | current `CallRelation::MethodCallbackFunction { MethodCallSiteId, FunctionNodeId }` for exact private complete-caller callback arguments such as `and_then(f)`; ambiguous candidate sets remain unresolved |
...
3. For dynamic/value-binding calls, keep the current structural split:
   ...
   - visible local closure bindings are classified on the path-call node and
     resolve to their closure executable owner when the binding proof is exact;
   - other visible local value bindings are classified on the path-call node and
     get `Unsupported` with no item edge until target proof exists;
