...
| D04 | `make_fn()()` | `fixture_call_graph::call_returned_function` | inner `PathCall`, outer `DynamicCall` with `ReturnedPathCall` proof | inner maker call resolves if local function; outer call resolves to the direct function path returned by the maker, otherwise fails closed |
