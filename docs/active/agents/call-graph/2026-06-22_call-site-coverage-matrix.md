...
| X10 | item macro inside body | `fixture_call_graph::call_item_macro_inside_body`, `fixture_call_graph::call_item_macro_generated_function` | statement-position `MacroCall`; for the bounded no-arg one-function item macro, an additional generated `LocalItem` owner is recorded | default remains `Unsupported`; the bounded generated local `fn` case resolves only the later source-visible `generated_by_item_macro()` call through `LocalFunction -> LocalItem` |
| X11 | macro expands to call/item | RA call hierarchy has FIXME outgoing macro gaps; fixture fallback covers no-arg one-function local item macro | invocation only by default; bounded local item owner only when a unique already-seen no-arg `macro_rules!` definition expands to exactly one local `fn` | no broad expanded edge until backend; current bounded case uses existing local-item traversal and keeps the macro invocation targetless |
