...
| D12 | `boxed_fn()` / `(boxed_fn)()` / `(*boxed_fn)()` where `boxed_fn: Box<dyn Fn()>`; `(referenced_fn)()` where `referenced_fn: &dyn Fn()` or `&mut dyn FnMut()` | `fixture_call_graph::{call_boxed_dyn_fn_value_binding, call_parenthesized_boxed_dyn_fn_value_binding, call_dereferenced_boxed_dyn_fn_value_binding, call_parenthesized_referenced_dyn_fn_value_binding, call_parenthesized_mut_referenced_dyn_fnmut_value_binding}` | syntactically `PathCall` unless parenthesized or dereferenced dynamic; exact callable trait-object initializer proof is preserved for `Box<dyn Fn()> = Box::new(local_target)`, `&dyn Fn() = &local_target`, and `&mut dyn FnMut() = &mut target` where `target` is initialized from `local_target` | direct path calls remain value-binding rows; parenthesized and dereferenced dynamic calls resolve to `CallRelation::DynamicFunction` only with exact initializer proof. Broader callable trait objects without initializer proof remain unsupported with no edge |
