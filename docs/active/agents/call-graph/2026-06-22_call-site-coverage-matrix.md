...
| `fixture_call_graph` | EOF appended case | `f()` where private `f: &dyn Fn() -> i32` is forwarded through one or two private helpers and the complete caller chain passes `&local_target` | P25/D12 | **green** | Covered by DB `fixture_context_resolves_forwarded_referenced_dyn_fn_parameters`, RAG callable path collection, and lookup/edges callable-parameter matrices; bounded forwarding proof reuses the existing private complete-caller contract for referenced callable parameters and resolves only the proven `local_target` edge. |
