...
| P30 | chained call callee `f()(1)` / returned closure call `make_closure()()` | `fixture_call_graph::{call_chained_returned_function, call_returned_closure}` | inner `PathCall`, outer `DynamicCall` with `ReturnedPathCall`, preserving outer argument count when present | inner resolved to exact local function when proven; outer resolves when the maker body directly returns either a function path or a single closure literal executable owner | function + dynamic |
...
| D04 | `make_fn()()` / `make_closure()()` | `fixture_call_graph::{call_returned_function, call_returned_closure}` | inner `PathCall`, outer `DynamicCall` with `ReturnedPathCall` proof | inner maker call resolves if local function; outer call resolves to the direct function path or direct closure literal returned by the maker |
