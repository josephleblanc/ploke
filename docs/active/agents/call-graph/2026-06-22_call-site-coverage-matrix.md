...
| `fixture_call_graph` | `src/lib.rs:182` | `local_target()` after `let local_target = || 377` | P25 | **green** | Covered by `fixture_call_graph_call_shadowed_local_target_binding_records_value_binding_path_call_site`; records closure-binding path callee, resolves to the closure executable owner, and does not fake an edge to the same-named module function. |
