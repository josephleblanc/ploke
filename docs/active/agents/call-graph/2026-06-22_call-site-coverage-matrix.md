...
| `fixture_call_graph` | EOF appended case | `let holder = AsyncFutureHolder { future: closure() }; holder.future.await` where `closure` is an async closure binding | D14 / owner matrix closure | **green** | Covered by `fixture_call_graph_call_awaited_async_closure_future_named_field_resolves_path_call_site`, DB `fixture_context_resolves_awaited_async_closure_future_named_field_to_executable_owner`, RAG async future named-field collection, and exact lookup/edges tests; bounded same-block named-field poll proof marks only the original `closure()` call as awaited and traverses through the async-closure owner without modeling returned futures or arbitrary aggregate value flow. |
