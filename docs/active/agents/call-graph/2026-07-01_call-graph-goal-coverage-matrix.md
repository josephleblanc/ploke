...
Next bucket: start a new proof-carrier slice before selecting another
parser/resolver breadth row. The 2026-07-10 inventory found that missing trait
visibility proof, public callable parameter blockers, fixture test-body macro
blockers, the axum-core `request_parts.rs:164` tuple-return receiver row,
routing helper macro/generated rows, and generated `IntoServiceFuture::new`
rows are already covered or intentionally fail-closed. Do not revisit those
families unless the implementation adds new proof input such as
