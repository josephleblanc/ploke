...
Current bucket: generated macro-boundary expanded-item proof linkage.

Exit criteria:

- Reuse the existing admitted macro-boundary summaries for axum
  `opaque_future!` and `top_level_handler_fn!(post, POST)`.
- Add strict `expanded_item` proof rows linked by `boundary_id`.
- Prove DB GraphRAG proof context follows summary/boundary linkage to the
  generated item metadata.
- Propagate the same linked proof row through RAG and exact TUI/tool payloads.
- Keep generated constructor/function rows targetless; do not fabricate local
  call edges or source item nodes.

Status: implemented for `IntoServiceFuture::new` generated by
