...
`corpus_memchr_call_graph_2026-07-12.sqlite` fixture now preserves finite
ambiguous `DynamicFunction` candidates for real memchr `Searcher.call` and
`Prefilter.call` through shorthand local aliases while omitting cfg-gated
architecture helpers that are not visible in the fixture.
...
completes the downstream surface for the existing parser/DB/RAG returned
callable proof without changing resolver semantics. Dedicated exact
lookup/edges tests reuse the callable blocker fixture for the two conflicting
returned-callable owners, preserving their ambiguous `local_target` /
`other_target` candidate sets without broadening the heavyweight blocker
batches.

Next bucket selection checkpoint: do not continue with tool-only breadth such
as `call_chained_returned_function` or additional boxed-callable rows unless a
downstream regression appears. The remaining autonomous implementation
candidates need a true proof input: macro-expanded/generated source bodies,
async poll/resume execution proof, broader interprocedural callable value-flow,
or a newly reviewed dependency-root/external-summary source oracle. Start the
next slice only when one of those carriers has a bounded source oracle and can
be verified parser/DB-first.

Previous completed slice: generated macro-boundary expanded-item proof linkage
for `IntoServiceFuture::new` generated by
`opaque_future!` and generated `routing::post`. The proof context now carries
the admitted `external_summary`, callsite-linked `expansion_boundary`, and
...
Latest completed slice: private referenced callable trait-object parameter
proof. The fixture-backed source oracle
`call_single_referenced_dyn_fn_param(f: &dyn Fn() -> i32) { f() }` and
`call_single_parenthesized_referenced_dyn_fn_param(f: &dyn Fn() -> i32) {
(f)() }` each have one local caller passing `&local_target`. Parser argument
capture now preserves `&local_target` as a referenced path; the resolver admits
that proof only for complete private caller sets whose parameter type is a
callable trait-object reference. DB/RAG assertions prove the path `f()` edge to
`local_target` as `Function` and the parenthesized `(f)()` edge as
`DynamicFunction`; exact TUI lookup/edges assertions cover the dynamic surface.
...
and transform projects non-null `is_async` values into the `function` and
`method` relations. DB `call_node_info` and `call_impact_for_target` expose the
same async/sync distinction through call node metadata, RAG exact impact maps
the field into typed payloads, and exact `code_item_lookup` / `code_item_edges`
serialize the same target-centered async metadata. The fixture-backed source
oracle is `make_ready_local_assoc` and its caller
`call_await_result_instance_method`: both are `async fn`, while `local_target`
remains a sync negative check. This slice is metadata propagation only; it does
not add async poll/resume traversal edges, future value-flow, or broader async
callable trait-object dispatch.
...
preserves `let iter = iter.into_iter(); iter.size_hint()` as a
`MethodResultLocalBinding` receiver with the initializer method span, and the
resolver classifies the outer `size_hint` row as an external targetless
frontier only when the original parameter has a source-visible external
`IntoIterator` bound. Transform, DB receiver decoding, DB fixture context, RAG
call-context collection, and TUI formatting preserve the same payload without
fabricating a local traversal edge.
...
Follow-up completed slice: when exact `code_item_lookup` / `code_item_edges`
omit `allowed_effects`, the tools now reuse the existing RAG stored-owner
policy helper if an admitted `effect_policy` exists for the resolved owner.
Missing stored policy preserves the prior empty-result behavior, while
ambiguous or invalid stored policies still fail loudly through the existing
DB/RAG strict helper. The axum task-spawn tool fixture now admits the same
`ffi_boundary` owner policy and proves both exact tools report the stored-policy
`async_task_spawn` violation without adding a local edge for `tokio::spawn`.
...
- Registry-backed verification passed with
  `cargo run -p xtask --features call_graph -- verify-backup-dbs`.
- DB fallback tests now assert the memchr function-pointer field rows preserve
  finite ambiguous `DynamicFunction` candidates, while boxed `dyn FnMut`
  callable-field rows remain targetless blockers with projected
  `type_resolution_missing` proof rows and source provenance.
...
- The bounded generated-item model projects the
...
- Added a DB real-target proof test for the axum generated constructor row:
...
- The original proof-layer slice kept the generated constructor callsite
  unresolved and targetless while admitting an `expansion_boundary` summary.
  The bounded generated-item follow-up now projects enough generated
  struct/impl/method shape for that callsite to resolve to the generated
  inherent `new` method.
- The admitted `opaque_future!` summary remains linked as proof metadata, but
  the callsite-level `type_resolution_missing` blocker no longer applies to
  `IntoServiceFuture::new`.
- Strict `expanded_item` proof rows remain for both generated axum
  macro-boundary cases: `expanded:item:axum-opaque-future-new` linked to
  `def:axum::future::IntoServiceFuture::new`, and
  `expanded:item:axum-routing-post` linked to
  `def:axum::routing::method_routing::post`.
- DB proof GraphRAG context follows matched summary/boundary rows through
  `boundary_id`, so summary or boundary lookups include generated item metadata.
...
- Verification passed before this follow-up:
  `cargo run -p xtask --features call_graph -- fixtures regenerate --active`,
  `cargo run -p xtask --features call_graph -- verify-backup-dbs`,
  `cargo test -p ploke-db proof_graphrag_context_matches_json_only_expansion_boundary_fields -- --nocapture`,
  `cargo test -p ploke-db axum_generated_constructor_macro_boundary_accepts_summary_proof -- --nocapture`,
  `cargo test -p ploke-db axum_real_target_generated_post_function_resolves -- --nocapture`,
  `cargo test -p ploke-rag proof_context_collection_preserves_axum_generated_constructor_summary -- --nocapture`,
  `cargo test -p ploke-rag proof_context_collection_preserves_axum_generated_post_resolution -- --nocapture`,
  `cargo test -p ploke-tui --test integration code_item_lookup_returns_generated_macro_boundary_path_rows -- --nocapture`,
  `cargo test -p ploke-tui --test integration code_item_edges_returns_generated_macro_boundary_path_rows -- --nocapture`,
  `cargo fmt --all --check`, and `git diff --check`.
...
- Split shared-matrix RAG coverage so `RagApi` means bounded prompt-context
  collection through `collect_call_context`, while `RagExactApi` means exact
  owner/target lookup through `exact_call_context`. This prevents late
  callsites in large owners from being misclassified as prompt-collection
  coverage.
- Verification passed:
...
- Follow-up coverage keeps the chrono guarded `self.queue.is_empty()`
  ...
  and external-summary coverage, in dedicated exact RAG coverage, and in the
  exact TUI lookup/edges adapter. The exact TUI run still computes owner-wide
  usage summaries for the long `parse_next_item` body, but filtered promotion now
  completes after batched exact path-node materialization. The generic shared RAG
  adapter still intentionally exercises bounded owner collection, which truncates
  this late callsite.
...
- Added `ploke_test_utils::call_shape_matrix` as the call-graph counterpart to
  ...
  generated constructor row, and an Axum dynamic callable field.
...
Previously completed bucket: async callable item metadata for usage summaries.

Completed evidence:

- Parser stores source signature asyncness on `FunctionNode::is_async` and
  `MethodNode::is_async` for free functions, inherent impl methods, and trait
  method declarations.
- Transform projects non-null `is_async` values into the `function` and
  `method` relations, and active backup fixtures were regenerated and
  round-tripped successfully.
- DB fixture coverage proves
  `call_await_result_instance_method() -> make_ready_local_assoc()` marks the
  async target function, preserves the async direct caller metadata, and keeps
  the sync negative check `local_target` unmarked.
- RAG exact call-impact summaries and the `code_item_lookup` / `code_item_edges`
  TUI tools preserve the same async target and async direct caller metadata.
- This covers async function/method item qualifiers only. Async poll/resume
  edges, returned futures, async callable trait-object dispatch, and general
  future value-flow remain separate future buckets.
...
`tests/fixture_crates/fixture_call_graph/src/lib.rs:2351-2365`, where
...
does not poll the future, `call_awaited_returned_async_closure()` invokes and
immediately awaits it, and `call_stored_returned_async_closure()` stores the
returned future in a same-block local binding before awaiting it. Parser
extraction records returned-path dynamic callee awaitedness, the resolver emits
a `DynamicClosure` edge only for the polled owners, and the un-awaited owner
stays `Unsupported` with no traversal edge. DB context/traversal/proof tests,
RAG callable-path collection, exact `code_item_lookup`, and exact
`code_item_edges` all preserve that distinction.
...
invokes only the producer function result. The caller traverses a resolved
