...
- Latest follow-up adds strict `expanded_item` proof rows for both generated
  axum macro-boundary cases:
  `expanded:item:axum-opaque-future-new` linked to
  `def:axum::future::IntoServiceFuture::new`, and
  `expanded:item:axum-routing-post` linked to
  `def:axum::routing::method_routing::post`.
- DB proof GraphRAG context now follows matched summary/boundary rows through
  `boundary_id`, so summary or boundary lookups include the generated item
  metadata. RAG proof context and exact TUI lookup/edges assertions preserve the
  same expanded-item rows without creating generated source nodes or traversal
  edges.
- Verification passed:
  `cargo run -p xtask --features call_graph -- fixtures regenerate --active`,
  `cargo run -p xtask --features call_graph -- verify-backup-dbs`,
