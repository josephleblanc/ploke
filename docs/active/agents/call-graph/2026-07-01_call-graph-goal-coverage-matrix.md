...
Current bucket: none selected after the unsafe callable item metadata slice.

Latest completed bucket: unsafe callable item metadata for usage summaries.
...
- RAG exact call-impact summaries and the `code_item_lookup` TUI tool preserve
  the same unsafe target and safe direct caller metadata.
- This covers unsafe function/method item qualifiers only. Unsafe blocks,
  unsafe trait obligations, unsafe impl metadata, inherited trait-method
  unsafety, and FFI boundary semantics remain separate future buckets.
...
| Unsafe callable item metadata | Met for direct item qualifiers | Parser and transform store `is_unsafe` for function/method items; DB fixture coverage proves `call_unsafe_function -> unsafe_target` marks the unsafe target and not the safe direct caller in node metadata and target impact | RAG exact impact preserves the same unsafe target and safe direct caller metadata | `code_item_lookup` exposes the same `call_impact.target.is_unsafe` and safe direct-caller flag | local fixture fallback | Switch buckets; unsafe blocks, unsafe trait obligations, unsafe impl metadata, inherited trait-method unsafety, and FFI boundary semantics remain separate future modeling work. |
