...
Current bucket: import/re-export/glob completeness with explicit workspace type
proof.

Exit criteria:

- Reuse the existing typed import/module/tree proof patterns rather than adding
  a parallel import resolver.
- Resolve only workspace type paths whose import/re-export/glob chain can be
  proven inside the selected parsed workspace.
- Keep dependency-root and source-oracle rows targetless/unsupported when the
  proof carrier is absent.
...
- RAG and `request_code_context` now preserve the closure owner as both the
  ...
  fixture-backed `async { local_target(); }` rows, preserve local-item owners
  as callers for fixture-backed function-local const/static initializer and
  local `fn` body `assoc_const_value()` rows, and can expose projected proof
  facts for those executable-local owners.
- `b01c07ff8` resolves local re-exported workspace type paths for associated
  function calls, including inherited `super::*` import chains, and regenerates
  the axum backup fixture.
- DB/RAG/TUI real-corpus assertions now prove twenty-three incoming
  `Body::empty` caller edges through axum-core direct rows, direct parsed
  workspace imports, local re-export imports, inherited glob imports,
  closure-owned rows, and local-item rows.

Reason to stay in this bucket: only if the next slice adds a bounded explicit
workspace proof carrier for a documented import/re-export/glob source oracle.
Otherwise switch buckets; executable-local owner propagation and the current
workspace type import proof slice are met for now.
