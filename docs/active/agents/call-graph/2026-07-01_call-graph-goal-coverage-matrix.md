...
- Parser-side `ClosureBodyId` / `ExecutableBodyId` facts now cover ordinary
  and `move` closure body calls under `CallBodyOwnerId::Executable`.
- DB projection now persists `call_body_owner` metadata for ordinary closure
  owners, keeps closure-body calls off the outer function owner, and makes
  closure owners snippet-materializable for RAG/tool expansion.
...
- RAG and `request_code_context` now preserve the closure owner as both the
  ...
  `|| local_target()` rows, and can expose projected proof facts for that
  owner.

Reason to stay in this bucket: only if the next slice targets a remaining
executable-local shape such as async blocks or function-local executable items.
Otherwise switch buckets; ordinary closure-owner DB/RAG/TUI propagation is met
for now.
...
| Dynamic callable values | Stronger fixture-backed subset plus real-corpus targetless fallback | Fixture-backed direct, alias, same-target branch/match including guarded same-target match arms, single-expression block initialized callable values, exact `Box<dyn Fn()> = Box::new(local_target)` path/dynamic calls, and named local closure binding `(closure)()` calls resolve; mixed-target branches and closure literal dynamic calls remain targetless; memchr function-pointer field rows remain unsupported and targetless by owner/source-line fanout | RAG preserves resolved direct/branch/block/guarded-match initialized rows, exact boxed dyn Fn initializer rows, named dynamic closure-binding rows, blocker rows, and now preserves the two memchr function-pointer field blockers with argument counts 4 and 2 | `code_item_lookup` covers resolved dynamic-function callable rows, including block-initialized and guarded same-target match forms; DB owner/caller/path queries now cover resolved `DynamicClosure` rows; lookup and edges targetless matrix covers axum callable-field rows plus memchr function-pointer field rows and blocked proof rows | local fixture; memchr real-corpus fallback | Switch buckets; broader callable trait objects without exact initializer proof, literal dynamic closures, and returned closures still need binding/body ownership work. |
...
| Closures / executable-local body ownership | Named closure-binding traversal met for now; async/local executable shapes remain future | Ordinary and `move` closure body calls are parser-owned by `CallBodyOwnerId::Executable`; `call_body_owner` projects closure metadata, validates as a call owner, keeps `local_target()` off the enclosing function, exposes `callers_for_target(local_target)` and one-hop paths from the closure owner, resolves named closure binding calls to the closure owner, proves an outer function -> closure body -> `local_target` two-hop path, and snippet-materializes closure owners for RAG expansion; async body calls and function-local const initializer calls remain intentionally absent rather than flattened into enclosing owners | RAG collection, incoming expansion, and projected proof-context tests preserve fixture-backed closure owners as callers for `local_target()` and preserve the outer `closure()` binding call to the closure owner | `request_code_context` tests materialize the closure owner with call context and projected proof rows; outer function is not fabricated as the direct `local_target()` caller | axum `parse_attrs` closure-body rows remain future until registered axum fixtures are regenerated/reviewed with executable owners; fixture-backed ordinary closure case is positive | Switch buckets unless the next planned slice explicitly targets async blocks, closure literals without a named binding, or function-local executable owners. |
