...
| Unsafe callable item metadata | Met for direct item qualifiers | Parser and transform store `is_unsafe` for function/method items; DB fixture coverage proves `call_unsafe_function -> unsafe_target` marks the unsafe target and not the safe direct caller in node metadata and target impact | RAG exact impact preserves the same unsafe target and safe direct caller metadata | `code_item_lookup` and exact `code_item_edges` expose the same `call_impact.target.is_unsafe` and safe direct-caller flag | local fixture fallback | Switch buckets; unsafe blocks, unsafe trait obligations, unsafe impl metadata, inherited trait-method unsafety, and FFI boundary semantics remain separate future modeling work. |
