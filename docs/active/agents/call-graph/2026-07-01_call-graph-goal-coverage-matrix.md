...
- RAG and `request_code_context` now preserve the closure owner as the incoming
  caller for fixture-backed `|| local_target()` rows and can expose projected
  proof facts for that owner.
...
| Closures / executable-local body ownership | Ordinary closure-owner propagation met for now; async/local executable shapes remain future | Ordinary and `move` closure body calls are parser-owned by `CallBodyOwnerId::Executable`; `call_body_owner` projects closure metadata, validates as a call owner, keeps `local_target()` off the enclosing function, exposes `callers_for_target(local_target)` and one-hop paths from the closure owner, and snippet-materializes closure owners for RAG expansion; async body calls and function-local const initializer calls remain intentionally absent rather than flattened into enclosing owners | RAG collection, incoming expansion, and projected proof-context tests preserve fixture-backed closure owners as callers for `local_target()` | `request_code_context` tests materialize the closure owner with call context and projected proof rows; outer function is not fabricated as the caller | axum `parse_attrs` closure-body rows remain future until registered axum fixtures are regenerated/reviewed with executable owners; fixture-backed ordinary closure case is positive | Switch buckets unless the next planned slice explicitly targets async blocks or function-local executable owners. |
