...
| Closures / executable-local body ownership | Named closure-binding and fixture-backed async-block traversal met for now; async closures/local executable shapes remain future | Ordinary and `move` closure body calls plus async block body calls are parser-owned by `CallBodyOwnerId::Executable`; `call_body_owner` projects closure and async block metadata, validates both as call owners, keeps `local_target()` off enclosing function owners, exposes `callers_for_target(local_target)` and one-hop paths from executable owners, resolves named closure binding calls to the closure owner, proves an outer function -> closure body -> `local_target` two-hop path, and snippet-materializes executable owners for RAG expansion; function-local const initializer calls remain intentionally absent rather than flattened into enclosing owners | RAG collection, incoming expansion, and projected proof-context tests preserve fixture-backed closure and async block owners as callers for `local_target()` and preserve the outer `closure()` binding call to the closure owner | `request_code_context` tests materialize closure and async block owners with call context and projected proof rows; outer functions are not fabricated as direct `local_target()` callers | axum closure/async body rows remain future until registered axum fixtures are regenerated/reviewed with executable owners; fixture-backed ordinary closure and async block cases are positive | Switch buckets unless the next planned slice explicitly targets async closures, closure literals without a named binding, or function-local executable owners. |
