...
executable-local shape such as async closures, closure literals without a
named binding, or function-local executable items. Otherwise switch buckets;
ordinary closure-owner and async-block-owner DB/RAG/TUI propagation are met for
now.
...
| Dynamic callable values | Stronger fixture-backed subset plus real-corpus targetless fallback | Fixture-backed direct, alias, same-target branch/match including guarded same-target match arms, single-expression block initialized callable values, exact `Box<dyn Fn()> = Box::new(local_target)` path/dynamic calls, named local closure binding `(closure)()` calls, and pathless non-async closure literal calls resolve; mixed-target branches remain targetless; memchr function-pointer field rows remain unsupported and targetless by owner/source-line fanout | RAG preserves resolved direct/branch/block/guarded-match initialized rows, exact boxed dyn Fn initializer rows, named and literal dynamic-closure rows, blocker rows, and now preserves the two memchr function-pointer field blockers with argument counts 4 and 2 | `code_item_lookup` covers resolved dynamic-function callable rows, including block-initialized and guarded same-target match forms; DB owner/caller/path queries now cover resolved `DynamicClosure` rows including closure literals; lookup and edges targetless matrix covers axum callable-field rows plus memchr function-pointer field rows and blocked proof rows | local fixture; memchr real-corpus fallback | Switch buckets; broader callable trait objects without exact initializer proof, async closure literals, and returned closures still need binding/body ownership work. |
...
| Closures / executable-local body ownership | Named closure-binding, non-async closure-literal, and fixture-backed async-block traversal met for now; async closures/local executable shapes remain future | Ordinary and `move` closure body calls plus async block body calls are parser-owned by `CallBodyOwnerId::Executable`; `call_body_owner` projects closure and async block metadata, validates both as call owners, keeps `local_target()` off enclosing function owners, exposes `callers_for_target(local_target)` and one-hop paths from executable owners, resolves named closure binding calls and non-async closure literal dynamic calls to closure owners, proves outer function -> closure body -> `local_target` two-hop paths, and snippet-materializes executable owners for RAG expansion; function-local const initializer calls remain intentionally absent rather than flattened into enclosing owners | RAG collection, incoming expansion, and projected proof-context tests preserve fixture-backed closure and async block owners as callers for `local_target()` and preserve outer closure binding/literal calls to closure owners | `request_code_context` tests materialize closure and async block owners with call context and projected proof rows, including move closure literal traversal; outer functions are not fabricated as direct `local_target()` callers | axum closure/async body rows remain future until registered axum fixtures are regenerated/reviewed with executable owners; fixture-backed ordinary closure, non-async closure literal, and async block cases are positive | Switch buckets unless the next planned slice explicitly targets async closures or function-local executable owners. |
