...
| Dynamic callable values | Stronger fixture-backed subset plus real-corpus targetless fallback | Fixture-backed direct, alias, same-target branch/match including guarded same-target match arms, and single-expression block initialized callable values resolve; mixed-target branches remain targetless; memchr function-pointer field rows remain unsupported and targetless by owner/source-line fanout | RAG preserves resolved direct/branch/block/guarded-match initialized rows and blocker rows, and now preserves the two memchr function-pointer field blockers with argument counts 4 and 2 | `code_item_lookup` covers resolved dynamic-function callable rows, including block-initialized and guarded same-target match forms; lookup and edges targetless matrix now covers axum callable-field rows plus memchr function-pointer field rows and blocked proof rows | local fixture; memchr real-corpus fallback | Switch buckets; broader callable trait objects/returned closures still need binding/body ownership work. |
