...
- Extend the dependency-root proof carrier only with source oracles for new
  ...
  `FromRef::from_ref` dependency-root rows and the axum-core
  `TestClient::new` workspace-glob dependency-root row; broader workspace
  import families still need exact source oracles before they can be promoted
  beyond existing resolved rows.
