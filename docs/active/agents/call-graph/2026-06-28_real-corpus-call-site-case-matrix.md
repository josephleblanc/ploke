...
| Glob import type call | `axum/src/json.rs:237`, `axum/src/json.rs:250` | Covered by `axum_real_target_test_client_new_high_fanout_is_documented_gap`: the `deserialize_body` owner traverses to `TestClient::new` through `test_helpers::* -> pub use test_client::*`; the same test keeps the remaining 69 unsupported `TestClient::new` rows fail-closed. |
