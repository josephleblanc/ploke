...
| Method-result-field receiver | `axum-fixture` | axum | `axum/src/routing/route.rs:51` | `self.0.clone().oneshot(req)` | Query should traverse nested tuple-field receiver expression and classify the outer method site as `MethodResultField(clone, ["0"])`. |
...
| Case | Status | Fixture / crate | File:line | Source shape | Future DB contract |
| --- | --- | --- | --- | --- | --- |
| Function-pointer callable field value | `axum-supported` | axum | `axum/src/boxed.rs:85` | `(self.into_route)(self.handler, state)` | Covered by `axum_dynamic_callable_fields_preserve_supported_and_unsupported_boundaries`: the row resolves to the unique `BoxedIntoRoute::from_handler` closure initializer with a `DynamicClosure` edge. |
| Ambiguous callable field value | `axum-ambiguous` | axum | `axum/src/boxed.rs:159,163` | `(self.layer)(self.inner.into_route(state))`; `(self.layer)(self.inner.into_route(state)).call(request)` | Covered by DB/RAG candidate tests: both rows preserve finite ambiguous `DynamicClosure` candidates from visible `MethodRouter::{layer,route_layer}` closure bindings and admit no local traversal edge. |
...
| Callable parameter without exact initializer proof | `axum-ambiguous` | axum-macros | `axum-macros/src/lib.rs:718`, `axum-macros/src/lib.rs:724` | `F: FnOnce(I) -> syn::Result<K>` then `and_then(f)` | Covered by DB/RAG tests: current fixture preserves the finite method-callback candidate set and admits no resolved traversal edge. |
| Callable parameter direct invocation | `axum-ambiguous` | axum-macros | `axum-macros/src/lib.rs:727`, `axum-macros/src/lib.rs:737` | `F: FnOnce(A, I) -> K` then `f(attr, input)` | Covered by DB/RAG/TUI tests: the closure-owned path call to captured parameter `f` is candidate-only ambiguous and non-traversable until exact callback proof selects a target. |
...
| Function-pointer field call | `fallback-source` | memchr | `src/memmem/searcher.rs:222,718` | `unsafe { (self.call)(self, prestate, haystack, needle) }` and `(self.call)(self, haystack)` | Covered in `fallback.rs`: memchr projects two ambiguous `DynamicFunction` rows owned by `find` methods, preserving cfg-visible helper candidates and omitting cfg-gated architecture helpers that are not visible in the fixture. |
...
| Function-pointer callable field | `axum/src/boxed.rs:85` | Covered by `axum_dynamic_callable_fields_preserve_supported_and_unsupported_boundaries`: the dynamic row resolves to the unique `BoxedIntoRoute::from_handler` closure initializer. |
| Ambiguous layer callable field | `axum/src/boxed.rs:159,163` | Covered by `axum_dynamic_callable_fields_preserve_supported_and_unsupported_boundaries` plus RAG and exact TUI candidate tests: both dynamic rows preserve three `DynamicClosure` candidates from `MethodRouter::{layer,route_layer}` and transparent `Router::layer` source, with no admitted traversal edge. |
