...
| Parameter receiver | `axum-fixture` | axum | `axum-core/src/extract/default_body_limit.rs:225`, `axum/src/extension.rs:184` | `req.extensions_mut().insert(...)` on `mut req: Request<_>` | Covered in `receivers.rs`: direct external parameter receiver proof classifies these rows as external targetless frontiers; borrowed/generic receiver rows remain unresolved until richer receiver proof exists. |
