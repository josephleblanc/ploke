...
| Parameter receiver | `axum-fixture` | axum | `axum-core/src/extract/default_body_limit.rs:183,225`, `axum/src/extension.rs:184` | `req.extensions_mut().insert(...)` on `req: &mut Request<_>` and `mut req: Request<_>` | Covered in `receivers.rs`: direct and single-reference external parameter receiver proof classifies these rows as external targetless frontiers; remaining generic receiver rows stay unresolved until richer receiver proof exists. |
...
| Callable trait object field | `fallback-source` | memchr | `src/tests/substring/mod.rs:68`, `src/tests/substring/mod.rs:94` | `Box<dyn FnMut...>` field then `fwd(...)` | Covered in `fallback.rs`: `Runner::run` preserves the boxed callable calls as targetless unsupported path rows without guessing boxed closure targets. |
