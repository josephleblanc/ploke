...
| Inherent-over-trait or explicit self dispatch | `axum-fixture` | axum | `axum/src/serve/listener.rs:41` | `match Self::accept(self).await { ... }` | Query preserves `Self::accept` as an external targetless call and avoids confusing it with recursive trait dispatch or an unrelated free function. |
