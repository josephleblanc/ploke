...
| Proc-macro attribute helper calls | `axum-fixture` | axum-macros | `axum-macros/src/lib.rs:581,637` | proc-macro attribute entrypoints call `expand_attr_with(...)` | Covered in `proc_macros.rs`: active macro owners traverse to `expand_attr_with`; inactive `__private_axum_test` remains absent under the fixture cfg. |
...
| Guarded match arm | `fallback-source` | generic-array | `src/lib.rs:1241` | `(n, _) if n > N::USIZE => return Err(LengthError)` | Covered in `fallback.rs`: the fixture now asserts two targetless unsupported `size_hint` rows. |
...
| Glob import type call | `axum/src/json.rs:237`, `axum/src/json.rs:250` | Covered by `axum_real_target_test_client_new_high_fanout_is_documented_gap`: the `deserialize_body` owner traverses to `TestClient::new` through `test_helpers::* -> pub use test_client::*`; the same test now asserts 167 projected `TestClient::new` rows resolve, includes a routing child-module inherited-glob traversal from `fallback.rs:10`, and keeps the axum-core `request_parts.rs:193` row targetless. |
