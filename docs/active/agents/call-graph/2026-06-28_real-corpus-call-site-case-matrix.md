...
| Ambiguous layer callable field | `axum/src/boxed.rs:159,163` | Covered by `axum_dynamic_callable_fields_preserve_supported_and_unsupported_boundaries` plus RAG and exact TUI candidate tests: both dynamic rows preserve two `DynamicClosure` candidates and no admitted traversal edge. |
