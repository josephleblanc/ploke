...
| CGQ-5 | P1 | Open | External proof projection reports `externally_summarized` while also using blocker reason `external_dependency_summary_missing`. | Fix semantics so external rows are blocked/missing-summary until a real external summary fact exists, or document and test a stricter alternative. |
