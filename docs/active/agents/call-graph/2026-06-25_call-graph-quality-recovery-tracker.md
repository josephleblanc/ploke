...
- Recent proof context test cleanup:
  ...
  - `f349e631 Expose proof build metadata`
  - `64cb8528 Expose proof environment metadata`
  ...
  - Expansion-boundary IDs/kinds and expanded-item linkage metadata now flow
    ...
    relation shape.
  - Selected build-domain target and proof-policy metadata now flows through
    DB/RAG/TUI proof context from the same JSON-backed path, leaving summary
    and discharge semantics unchanged.
  - Cfg-domain hashes and rustc-invocation evidence metadata now flow through
    DB/RAG/TUI proof context from the same JSON-backed path, leaving proof
    invariant evaluation and discharge semantics unchanged.
  - Effect-seed IDs, confidence, and unresolved-blocker flags now flow through
    DB/RAG/TUI proof context from the same JSON-backed path. This exposes
    existing strict `effect_seed` proof metadata without changing the
...
- `ploke-rag/src/core/unit_tests/tests/call_context/public.rs` is now a thin
  ...
  child files.
- `ploke-rag/src/core/unit_tests/tests/call_context/expansion.rs` is now a
...
- `ploke-rag/src/core/unit_tests/tests/call_context/collection/helpers.rs` is
...
- RAG public sparse `get_context` call-context tests now live in a dedicated
  `unit_tests/tests/call_context/public.rs` concern module while retaining the
  shared call-expansion assertion helpers in the parent test module for the next
  expansion split.
...
- RAG call-context collection module split
  - `cargo test -p ploke-rag --features call_graph call_context_collection -- --nocapture`
    passed: 7 passed, 0 failed.
  - `cargo test -p ploke-rag --features call_graph call_context_sparse_get_context -- --nocapture`
    passed: 8 passed, 0 failed.
