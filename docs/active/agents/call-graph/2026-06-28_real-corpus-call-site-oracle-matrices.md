...
| --- | --- | --- |
| `TestClient::new` owner-per-callsite matrix | Exact 172 file/line callsites are listed. `ploke-db` now asserts the current 167 projected structural rows by module, source-file, and exact source-line fanout from parser facts, with all rows unsupported, targetless, and non-traversable. The five missing source rows are `extract/multipart.rs:{383,423,449}`, closure-body `routing/tests/mod.rs:1073`, and macro-template `routing/tests/nest.rs:371`. | Keep the current 167-row module/file/source-line fanout assertion as the fail-closed DB contract until projection coverage changes; when multipart, closure-body, or macro-template ownership lands, tighten this to full 172-callsite parity. |
