...
| --- | --- | --- | --- | --- |
| fixture_nodes | generic self-field receiver blockers | `src/impls.rs:77` `self.value.len()`; `src/impls.rs:103` `self.value.into()` | `GenericStruct<T>::value` at `src/impls.rs:58`; `impl<'a> GenericStruct<&'a str>::get_str_len` at `:75`; `impl<T> SimpleTrait for GenericStruct<T>` where-bound at `:98-100` | `fixture_nodes` projects both rows as targetless `SelfField(["value"])` method calls with `Unsupported` status. DB and RAG tests assert that no traversal edge or target is fabricated until generic self-field receiver typing and trait-dispatch proof are implemented. |
