...
assert the split high-fanout contract: 105 nested/direct re-export import rows
resolve to `TestClient::new`, while 62 currently projected other import rows
remain unsupported, targetless, and non-traversable. A fixture-backed
current-source regression now proves the routing-test-shaped `use super::*`
case where a child inherits a parent glob import and resolves the associated
function call. RAG exact call context, `code_item_lookup`, and
`code_item_edges` still preserve the 105 registered-backup resolved
target-centered caller rows and their `TestClient::new` associated-function
callee shape. The five absent text rows remain tied to multipart,
...
| --- | --- | --- |
| `TestClient::new` owner-per-callsite matrix | Exact 172 file/line callsites are listed. `ploke-db` now asserts the registered axum backup's 167 projected structural rows with a split contract: 105 nested/direct re-export import rows resolve to `TestClient::new`, while 62 other import rows remain unsupported, targetless, and non-traversable by module, source-file, and exact source-line fanout. Current-source fixture coverage now proves the inherited `use super::*` parent-glob shape that accounts for the routing-test subset, but the axum backup has not been regenerated/reviewed. The five missing source rows are `extract/multipart.rs:{383,423,449}`, closure-body `routing/tests/mod.rs:1073`, and macro-template `routing/tests/nest.rs:371`. | Keep the 105 resolved / 62 fail-closed DB contract until registered axum fixture regeneration/review updates the persisted evidence; when multipart, closure-body, macro-template ownership, or broader import evidence lands, tighten this toward full 172-callsite parity. |
