...
| chrono | `DateTime...?.naive_utc()` try receiver | `src/format/parsed.rs:836,953`; macro-equivalent `src/naive/datetime/mod.rs:140,157,174,195` | `DateTime<Tz>::naive_utc` at `src/datetime/mod.rs:563`; constructors return `Option<Self>` at `:768,:803` | `corpus_chrono_call_graph` currently projects two `TryMethodCallResult(ok_or)` receiver method rows for `naive_utc` with `Unsupported` status and no traversal edge to the method. |
