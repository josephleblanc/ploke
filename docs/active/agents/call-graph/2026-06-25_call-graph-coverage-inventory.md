...
| Proof facts | Partial | Proof graph projection covers resolved call edges, blockers, source provenance, candidate preservation, and invariant checks for the current DB surfaces. Proof context lookups match candidate IDs and external summary IDs from stored proof-fact JSON without changing the `proof_fact` relation shape; `externally_summarized` call-resolution facts now require `external_summary_id`. | External dependency summary artifacts, macro/build-domain summaries, admission semantics, and proof-authoritative artifacts remain open. |
