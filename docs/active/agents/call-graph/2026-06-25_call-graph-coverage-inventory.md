...
| TUI call context | Partial | Formatter/overlay/tool-carrier coverage exists for call summaries, target payloads, status/resolution, and selected targetless/proof shapes. Proof context payloads now include resolved target IDs and ambiguous candidate IDs in model-facing text/serde carriers. | Final interactive call-context UX and proof-facing presentation are not complete. |
| Proof facts | Partial | Proof graph projection covers resolved call edges, blockers, source provenance, candidate preservation, and invariant checks for the current DB surfaces. Proof context lookups match candidate IDs from stored proof-fact JSON without changing the `proof_fact` relation shape. | External dependency summaries, macro/build-domain summaries, admission semantics, and proof-authoritative artifacts remain open. |
