...
| Structural parser model | Green | `CodeGraph` stores structural `call_sites` and `call_site_relations`; `BodyContainsCall` links `CallBodyOwnerId` to `AnyCallSiteId`. Semantic call target/status facts are report-owned, not dormant parser graph fields. Closure, async block, async closure, function-local const/static, local `fn`, and local impl method bodies now have executable call owners instead of being flattened into enclosing functions. | Keep new body-owner families explicit; broader generated, desugared, or effect-resume owners remain future work. |
| Structural extraction | Partial | Path calls, method calls, dynamic/non-path calls, macro calls, const/static initializer calls, associated const initializer calls, closure/async block bodies, selected function-local item bodies, and selected no-arg local macro-generated path-call statements are extracted for the covered fixtures. Closure/async/local-item inner calls are attributed to typed executable owners, not to enclosing function owners. | Desugared/implicit calls, broad macro expansion, build-script generated code, arbitrary generated owners, and broader async poll/resume effect modeling. |
...
| Method resolution | Partial | Exact inherent `self.method()`, selected non-self local receivers, result/await/try receivers, exact local `self.field.method()` receivers, field/tuple receivers, alias-backed receivers, selected trait impl dispatch, generic/impl-trait/trait-object receiver methods, trait-associated calls, and exact private `and_then(f)` method-callback function arguments are covered conservatively. | Broader trait dispatch, autoderef/autoref depth, generic/nested self-field receivers, dynamic dispatch, blanket impl breadth, arbitrary callback-body traversal, and method lookup parity with rustc remain open. |
| Dynamic calls | Partial | Structural dynamic calls and exact local function-item binding calls are modeled. Covered positives include direct/alias/branch/block local function-item bindings, same-target guarded match arms, boxed `dyn Fn` values with exact `Box::new(local_target)` initializer proof, named and literal closure calls, closure-binding deref calls, direct returned function/closure/local-binding/alias calls, immediately awaited async-closure literal calls, bounded same-block awaited async-closure future binding/alias/tuple-field/named-field proofs, and private complete-single-caller parameter proofs for bare function pointers, bounded generic callable parameters, and selected constructed holder fields. Ambiguous or mixed-target candidates preserve provenance without becoming resolved. | Full dynamic dispatch, arbitrary interprocedural callable value flow, callable trait objects without exact initializer or complete private caller proof, returned future flow, general async poll/resume edges, and runtime trait-object call edges. |
...
| DB query API | Partial | Owner call context, target callers, target call sites, target-centered full call context, node-scoped outgoing/incoming context, expansion, receiver decoding, relation decoding, schema/invariant checks, and fixture-backed contexts have focused coverage. Target-centered full context preserves every target/candidate for the matched call site instead of collapsing ambiguous dynamic sibling candidates to the matched target, and now has fixture-backed target-family coverage for function, method, method-callback function, associated-function, tuple-struct constructor, and enum-variant constructor targets. Nested typed-reference method callers and exact private result method-callback function callers are covered through DB target callers and downstream context expansion. | Finish inventory-driven table matrices before adding new query cases; keep large helper files split by concern. |
| RAG call context | Partial | Call-context collection and expansion are baseline and tested for populated DBs, degraded DBs, owner seeds, direct target seeds, dynamic callers, constructors, const/static and associated-const initializer owners, method targets including nested typed-reference and exact local self-field receivers, raw identifier path/method payloads, associated functions, trait dispatch, selected resolved path-family owner seeds (`self::`, import alias, glob, re-export, module alias, grouped import), resolved special-form owners (generic turbofish function/method calls, unsafe local function calls, and direct returned-function dynamic calls), targetless special-form owners (extern C external path calls and qualified dyn Any external frontiers), selected result/field receiver owners (path-call, method-call, await path-call, and tuple-field method receivers), resolved field-dynamic owners, and public context payloads. Direct RAG hit collection now uses DB node context, so target seed parts carry incoming caller rows while owner seed parts retain outgoing rows. RAG proof-context expansion also attaches projected method proof rows to required expanded callers, including nested-reference, self-field, method-as-associated-function, and concrete trait-dispatch callers; direct projected proof collection now covers the selected resolved path family, constructor targets, method-family targets (inherent method, associated-function, trait-dispatch, and imported trait-associated-function targets), const/static and associated-const initializer owners, raw identifier path/method owners, resolved special-form owners including direct returned-function dynamic calls, targetless special-form owners, fixture blocker owners (external, macro, ambiguous, callable path, and dynamic callable blockers), selected result/field receiver owners, and resolved field-dynamic owners. The final prompt header now states the call-graph proof policy: resolved rows are local edges, while external/unsupported/unresolved/ambiguous rows are blockers or candidates and must not be treated as traversal edges. | Broader live/provider/tool-matrix coverage remains open. |
| TUI call context | Partial | Formatter/overlay/tool-carrier coverage exists for call summaries, target payloads, status/resolution, and selected targetless/proof shapes. Proof context payloads now include resolved target IDs, ambiguous candidate IDs, external summary IDs, and effect-seed metadata in model-facing text/serde carriers. `request_code_context` surfaces proof-context degradation when proof facts are absent, reports context carrier and blocker counts in its UI payload and compact summary, returns projected resolved proof rows for direct owners and expanded method callers, preserves nested-reference, self-field, method-as-associated-function, and trait-dispatch method caller proof payloads, preserves target-centered ambiguous candidate proof rows, preserves constructor target proof rows, preserves method-family target proof rows, preserves const/static and associated-const initializer proof rows, preserves targetless special-form blocker proof rows, preserves fixture-backed external, macro, ambiguous, callable path, and dynamic callable blocker proof rows, and now preserves selected result/field receiver plus resolved field-dynamic proof rows in the tool payload. Request-path caller coverage now includes local and imported trait associated-function targets, method-as-associated-function callers for method targets, concrete trait-dispatch method callers, constructor targets, const/static and associated-const initializer owners, raw identifier path/method callers, resolved special-form callers, targetless special-form callers, selected result/field receiver callers, resolved field-dynamic callers, ordinary/dynamic function callers with incoming expansion provenance, direct target-seed incoming call-context payloads, and selected resolved path-family owner payloads for `self::`, import alias, glob, re-export, module alias, and grouped import paths. Exact lookup/edge tools now accept `node_kind=variant` for enum-variant constructor targets by using the owning enum snippet anchor and variant ID for call/proof context. Context-plan overlay titles now surface call blocker and proof blocker counts for collapsed RAG rows, and context-plan RAG metadata now preserves/renders bounded outgoing/incoming call-path carriers. TUI startup config now maps `rag.call_context` and `rag.proof_context` user limits into `RagConfig`, so dense target-centered context/proof retrieval can be tuned without changing hard-coded RAG defaults. | Final interactive call-context UX and proof-facing presentation are not complete. |
| Proof facts | Partial | Proof graph projection covers resolved call edges, node-scoped proof projection, blockers, source provenance, candidate preservation, external summary artifact storage, authority terms, and invariant checks for the current DB surfaces. Proof context lookups match candidate IDs, external summary IDs, authority terms, external-summary artifact metadata, expansion-boundary IDs/kinds, expanded-item linkage metadata, selected build-domain target/policy metadata, cfg-domain hashes, rustc-invocation evidence metadata, and effect-seed IDs/confidence/unresolved-blocker metadata from stored proof-fact JSON without changing the `proof_fact` relation shape; every known stored proof fact kind now requires explicit `evidence_use`, and projection no longer silently defaults missing evidence to `proof_only`. `externally_summarized` call-resolution and expansion-boundary facts now require `external_summary_id`, and `external_summary` artifacts require scoped summary metadata. Incoherent admitted opaque summaries are rejected before storage, and admitted linked external summaries discharge derived external-dependency missing-summary blockers only when they authorize `external_summary_boundary` and match the direct or call-site build-domain scope; call-site scoped resolutions without a linked `call_site` domain fact fail closed. Admitted linked proc-macro/build-script summaries discharge their specific expansion blockers only with dedicated `proc_macro_summary_boundary` or `build_script_summary_boundary` authority. Proof blockers now also fail closed when proof facts reference a missing `build_domain`, and when a stored `build_domain` lacks admitted matching cfg-domain and rustc-invocation evidence. `proof_blockers()`, `proof_checker_edges()`, and DB/RAG/TUI proof-context surfaces expose invariant-derived blocker reasons, including multiple derived blockers for one proof fact. | Broader proof-authoritative artifact semantics and downstream presentation policy remain open. |
| Rollout gate | Green | `CALL_GRAPH_GATE:db-projection` has been removed from production code. Call graph projection and downstream DB/RAG/TUI consumers are baseline. The Cargo feature name remains only as a no-op compatibility alias for old commands. Default fixture imports are not loosened. Active checkout-local backup fixtures were reviewed/regenerated on 2026-07-06, refreshed on 2026-07-07, and regenerated again on 2026-07-08 with no tracked fixture drift. The 2026-07-08 `verify-backup-dbs`, DB `real_target_matrix`, RAG `real_corpus`, and TUI integration `real_corpus` checkpoints passed. Focused real-corpus effect propagation checks also passed through DB, RAG, `code_item_lookup`, and `code_item_edges`. | Keep backup fixture behavior strict; regenerate/review fixtures rather than adding permissive import behavior if future schema changes land. |

## Recent downstream slice

- 2026-07-11: Exact private result method-callback function proof now covers
  `call_single_result_callback(f) -> Ok::<i32, ()>(1).and_then(f)` when every
  local caller passes `local_result_target`. Parser assertions expect the
  `MethodCallbackFunction` relation, DB asserts target callers and a two-hop
  caller -> helper -> callback target traversal, RAG collection preserves the
  resolved callback row, and exact `code_item_lookup` / `code_item_edges`
  preserve the method-callback target and proof context.
...
- 2026-07-06: Owner reach summaries now split the remaining nonresolved
  ...
  `top_level_handler_fn!(post, POST)` generated function item. The sibling
  `top_level_service_fn!` generated service functions are now DB-pinned with
  generated-body `on_service(...)` edges and real-corpus `*_service(...)`
  source-line fanout. Focused
  verification:
...
- 2026-06-28: Exact TUI lookup and edge tools now address enum-variant
  ...
  `with_position.rs:92` calls `Position::First(item)`. `code_item_lookup` and
  `code_item_edges` now traverse from the exact variant target to the incoming
  real-corpus callsite and proof row using the existing real-corpus tool matrix.
  Regression coverage:
  `cargo test -p ploke-tui --test integration call_graph_tool_remaining_matrix -- --nocapture`
  and `cargo test -p ploke-tui --test integration tool_node_kind_schema -- --nocapture`.
- 2026-06-28: The real-corpus call-site oracle matrix is covered in
  ...
  53 tests, 0 failed.
- 2026-06-28: `request_code_context` compact UI summaries now include nonzero
  call and proof blocker counts in addition to the structured UI fields. This
  keeps collapsed tool output from hiding fail-closed call/proof context.
  Regression coverage:
  `request_code_context_returns_targetless_special_form_call_context`,
  `request_code_context_preserves_multiple_proof_blockers_for_one_fact`, and
  `request_code_context::gat_tests`.
- 2026-06-28: Real-corpus `parse_attrs` traversal is now covered across DB,
  ...
  `axum_real_target_parse_attrs_projects_proof_facts`,
...
- 2026-06-27: DB query helpers now include `call_sites_for_target`, a strict
...
- 2026-06-27: `code_item_lookup` and `code_item_edges` UI payloads now expose
  ...
  unchanged.
- 2026-06-27: Proof blockers now recognize dedicated
