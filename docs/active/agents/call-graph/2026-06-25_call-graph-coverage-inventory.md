...
| Proof facts | Partial | Proof graph projection covers resolved call edges, blockers, source provenance, candidate preservation, external summary artifact storage, and invariant checks for the current DB surfaces. Proof context lookups match candidate IDs and external summary IDs from stored proof-fact JSON without changing the `proof_fact` relation shape; `externally_summarized` call-resolution facts now require `external_summary_id`, and `external_summary` artifacts require scoped summary metadata. | External summary admission/discharge semantics, macro/build-domain summaries, and proof-authoritative artifacts remain open. |
