...
| DB query API | Partial | Owner call context, target callers, target-centered full call context, expansion, receiver decoding, relation decoding, schema/invariant checks, and fixture-backed contexts have focused coverage. Target-centered full context preserves every target/candidate for the matched call site instead of collapsing ambiguous dynamic sibling candidates to the matched target. | Finish inventory-driven table matrices before adding new query cases; keep large helper files split by concern. |
...
| Rollout gate | Gated | `CALL_GRAPH_GATE:db-projection` remains active; the Cargo feature is still named `call_graph`, but the active rollout gate is DB projection plus downstream consumers, not parser-side structural modeling. Default fixture imports are not loosened. Active checkout-local backup fixtures were reviewed/regenerated on 2026-06-27 in both default and `xtask/call_graph` profiles, and default workspace verification is green. | The fixture review/regeneration precondition is satisfied for this checkout. Before making DB projection baseline, decide whether the Cargo `call_graph` rollout gate should be removed or kept for a later cross-machine/seed-fixture review. |
...
1. Use this inventory plus the detailed call-site matrix to choose the next DB
   ...
   assertions.
2. Fixture review/regeneration and broad default workspace verification are
   green in this checkout; keep the rollout gate active until the baseline
   feature decision is made explicitly.
