...
- `RunRecordSet.forest_input.passive_evidence.run_attempts`
- `RunRecordSet.forest_input.passive_evidence.agent_turns`
- `RunRecordSet.forest_input.passive_evidence.attempt_runner_results`
...
| `tool-calls-results` | partial `evidence` / `loaded-not-ingested` | Agent-turn trace/summary records have typed `ploke_records::agent_turn` ownership, direct run-root `RunRecordSet` loading, passive summary + artifact metadata attachment, and node/branch joins when the path identifies a scheduler node. Remaining gap: tool request/result arguments and step-level decomposition are still not indexed as graph relations or answer objects. Do not resolve that by copying tool DTOs into UI-local or records-local mirrors. |
...
| `llm-attempts` | `not-loaded` | Provider attempts, timeouts, retries, and full response logs are still outside the loaded graph input families. Agent-turn evidence carries tool/request counts, not typed provider-attempt provenance. |
...
| `tool.call.record.arguments` | `loaded-not-ingested` | Typed agent-turn trace/summary artifacts already load into `RunRecordSet`, but `Graph::from_records` only indexes summary/artifact metadata evidence today. Tool-call argument/detail decomposition still needs graph-owned relations or answer objects. |
| `tool.request.arguments.capture` | `loaded-not-ingested` | Typed agent-turn records already carry outbound tool request facts. The remaining gap is graph indexing of request argument detail, not passive loading. |
...
| `tool.result.trace.projection` | `loaded-not-ingested` | Typed agent-turn records already load result/terminal event evidence. The missing layer is graph decomposition for tool-result traces and a projection that borrows from that graph-owned detail. |
| `prototype1.scheduler_json` | `evidence` | Loaded by `RunForestInput`; graph uses it for labels/status evidence only, not ordering authority. Frontier/completed/failed lane ids are not yet preserved as typed graph relations. |
...
| `prototype1.metrics_projection` | `loaded-not-ingested` | Typed evaluation artifacts already load through `PassiveEvidence.evaluations`, and `Graph::from_records` attaches branch evaluation evidence by artifact path. Separate graph indexing of metric fields is still missing. |
...
| `prototype1.agent_turn_trace` | `evidence` / `loaded-not-ingested` | Passive owner is `ploke_records::agent_turn`; `ploke-tree` loads direct run-root trace/summary files into `RunRecordSet`; `Graph::from_records` attaches summary/artifact metadata evidence and joins node-scoped artifacts back to candidate branches when the path identifies a scheduler node. Tool-call/provider decomposition still remains unresolved. |
...
4. Tool call/result operation nodes remain pending. Current agent-turn graph
   ingestion reaches loaded summaries, artifact metadata, and node/branch joins,
   but it still stops short of first-class tool-call/tool-result graph
   decomposition.
...
9. Scheduler frontier/completed/failed lane membership still arrives only as
   scheduler state evidence. If UI needs a candidate-frontier answer, add a
   graph-owned lane relation instead of rebuilding frontier sets in the view.
