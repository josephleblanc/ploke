...
`prototype1-state` does not hard-code one model for the full loop. Eval calls use the resolved campaign model: `ResolvedCampaignConfig.model_id`, with `provider_slug` and `route_source` resolved from the campaign manifest and fallbacks.[^campaign-resolved] Protocol calls use the campaign tuple unless `ProtocolCampaignPolicy` supplies a protocol-specific model/route/provider override. The automatic broad headless-TUI parent patcher path is separate again: it currently reads the parent-patcher model selection, falling back to the active model selection if the parent-patcher file is missing.[^parent-patcher-selection]
...
4. Protocol JSON calls use `protocol_llm_config()` to build `JsonLlmConfig` from the effective protocol model/route/provider plus protocol policy values.[^protocol-config] `JsonAdjudicator` calls `adjudicate_json()`, which parses `cfg.model_id`, builds either a Google or OpenRouter JSON chat request, and sends it through `chat_step()`.[^json-adjudicator] [^json-call]
...
- Setup writes the campaign model: `prepare_prototype1_loop_campaign()` selects CLI/profile/default eval model, route, and provider, then writes `manifest.model_id`, `manifest.provider_slug`, and `manifest.route_source`. It also stores a protocol model override in campaign `protocol` when `--protocol-*` flags or `[protocol.model]` differ from eval.[^setup-model]
...
- Runtime run shape comes from the admitted profile when present. The live eval model comes from resolved campaign config; the live protocol model comes from campaign protocol policy with campaign config as fallback, not by re-reading profile `[model]` at each turn.[^run-shape]
...
- `JsonLlmConfig::default()` has `moonshotai/kimi-k2`, but the `prototype1-state` protocol closure path passes the effective campaign/protocol-policy model explicitly. Treat the default as a generic protocol fallback, not the model chosen by a normal campaign-backed Prototype 1 state turn.[^json-default] [^protocol-advance]
