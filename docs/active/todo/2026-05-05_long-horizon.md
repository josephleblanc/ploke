› capabilities to pull together the first real self-improvement loop now that we have proven we
  ...
  Update recorded 2026-05-06: the selection/archive plan should be
  blockchain-shaped rather than a separate off-chain Archive. `History` is the
  authenticated authority/evidence substrate. An `ArchiveView` or
  `CandidateArchive` is a query/index/projection over History-admitted
  candidate, child-evaluation, judgment, validator, import, and selection
  records plus content-addressed evidence refs. Official child evaluations by a
  `Parent<Ruling>` are History-shaped events inside that Parent epoch; the
  lineage-advancing authority transition is still the admitted selection plus
  Crown lock/handoff. HyperAgents-style traversal should sample from an
  admissible Archive view under a recorded scope, sampling policy, validator
  evidence, and limitations, not from mutable scheduler/report state.
  ...
  The output of this survey should be shaped as the first `ArchiveView`
  inventory: what candidate/evaluation/validator evidence can be admitted into
  History, what remains content-addressed evidence, and which projections are
  only indexes over that substrate.
  ...
  The HyperAgents mechanism should be adapted as an archive traversal policy
  over History-backed candidate records: sample from immediate children, prior
  rejected children, ancestors, siblings, or cross-lineage imports only through
  an explicit admissibility and verification policy.
  ...
  This registry should eventually consume a bounded `ArchiveView`, not only
  generation-local `SelectionInput`, and its decision record should commit to
  candidate-set refs, evidence-set refs, validator score distributions when
  used, and known exclusions from the sampled view.
  ...
  records and Archive projections without treating another Parent's claims as
  trusted truth. Cross-ruler use should pass through admission policy:
