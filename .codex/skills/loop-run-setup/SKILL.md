...
Do not skip a live preflight or gated `live_api_tests` check solely to save API
spend when provider behavior is part of the setup contract.
...
- if child-plan fanout produces a completed child attempt before the whole step
  exits, dispatch a run-review sub-agent for that single attempt and keep
  monitoring the active loop locally
...
For child-plan fanout, do not wait for every sibling attempt before starting
review work. Each completed child attempt should get its own run-review handoff;
the orchestrator tracks those reviews separately and keeps the loop moving
unless a review identifies a blocker that invalidates the next transition.

When a review or live step exposes a source-code issue and the operator says to
clean it up in the main codebase, delegate that cleanup to a repair worker with
bounded file ownership. The orchestrator should continue setup/step monitoring
locally and only stop the loop when the controller cannot continue or the
operator withdraws the continue override.
