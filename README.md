...
##### ❗ WARNING ❗
Under development, this may crash if it tries to parse a crate with an error,
or in certain [known
limitations](./docs/plans/uuid_refactor/02c_phase2_known_limitations.md) or
unknown limitations.

The biggest current limitation is that we only supply semantic code edits
(regular search/replace block edits soon(TM)), so if the target crate is
malformed (like a function is missing a closing bracket), Ploke will fail to
parse the target directory and may crash.
...
<details>
 ...
 None of the text descriptions in this document were generated by LLMs.
  ...
  If you are a contributor and run across LLM-generated documentation, note
  that it *should* always include the name of the human responsible for
  reviewing it for accuracy, and if the docs are confusing or incorrect please
  tell them! If no one has signed off on the documentation, Joseph LeBlanc is
  responsible for it and you should yell at him (me).
</details>
...
<details>
  <summary>It's (mostly) ready!</summary>
  Some features might occasionally break, but if something seems weird (model
  picker not loading, UI feels lame, you want a feature) feel free to submit an
  issue or a PR. We are welcoming to contributors, and aim to make this a
  community-friendly project for beginners and experienced developers.
</details>

### Quickstart

Go ahead and clone the repo, and you can compile and run it with the following commands:
```bash
...
under OPENROUTER_API_KEY (support for other endpoints coming soon), for example
before using `cargo run`, enter
```
export OPENROUTER_API_KEY="your_key_here"
```

This will open the terminal user interface. From here you can start chatting
with the LLM, but to really take advantage of the differentiating features like
vector embedding/semantic search + edits you'll want to point the application
at a Rust crate's directory.
...
<!-- ``` -->
<!---->
<!-- Then inside the running instance of Ploke in your terminal: -->
<!---->
<!-- ``` -->
<!-- /index ../hello-ploke -->
<!-- /save db -->
<!-- ``` -->
<!---->
<!-- Now your new rust project is parsed, vector indexed, and saved for later use, -->
<!-- so you can safely chat with the LLM about your project. See more details on -->
<!-- Usage below. -->
<!-- TODO: Verify that the above works correctly before adding the above section -->

#### Usage

To begin using `ploke` on a Rust project, you will likely use the following steps:

1. Index the target Rust crate
2. Save the indexed code graph
3. Chat with the LLM
4. Possibly review + approve code edits

To index a target rust crate, run the application with `cargo run` and then you can enter `/index start path/to/crate` for whichever crate you want to work on. If you want to see a test example working with our local test fixture, you can run this command inside the application:

```
/index start tests/fixture_crates/fixture_nodes
```

This will parse an example test fixture that has variations of common rust
code, and expose tools to allow the LLM to more efficiently search through and
edit the target crate.

<details>
  <summary>For Contributors and the Curious: What this does</summary>

  The target directory will be parsed into a Cozo database stored in-memory,
  and the contents of each node (e.g. a `FunctionNode` body or `StructNode`
  name and fields) will be processed with a vector embedding model. Then the
  vector values are indexed using Cozo's native HNSW index, allowing for fast
  lookup of closely related nodes. If you have enabled the BM25 search with the
  `/bm25 rebuild` command, then BM25 keyword search will be enabled as well.

  Once the target crate has been indexed, user responses will be processed into
  vector embeddings as well, so the next time you send a message to the LLM, a
  search automatically runs and includes related code snippets 
  in the conversation. Additionally, if the LLM used is tool-capable, tools to
  allow the LLM to perform the same semantic and keyword search are made
  available to the LLM, allowing it to efficiently search and submit edit
  proposals for the target Rust crate.

</details>

<br>

Now you can chat with the LLM about the code base, and it will be able to call
tools to find relevant code as context for your questions or requests. However,
to avoid needing to re-index the target rust crate (which can take minutes),
you probably want to save the indexed code graph.

After the target crate is indexed, you can also save the indexed code graph
using `/save db`, and the code graph will be saved to your default config
directory in `ploke/data/<crate-name>`. This way, next time you want to chat
with the LLM about your rust project, you can avoid re-indexing the crate
again, and can quickly load it with `/load crate <crate-name>`.

After indexing and saving the target rust crate, you might ask the LLM to make
some code edits. By default, the LLM's proposed changes will not be applied to
your local files, but will be staged for review and approval. To review and
approve these proposed changes, you can use `e` to bring up the edits overlay,
select a proposed change, and press `Enter` to accept and apply the change.

<details>
  <summary>For Contributors and the Curious: What this does</summary>

  <b>Semantic Code Edits</b>

  The LLM can submit edit proposals through a `code-edit` tool we expose to the model. This tool provides the model with the ability to make semantic code edits to the code base, using the module path of the target code snippet to identify where the changes should be applied. 

  For example, if the user sends a message asking something like "Change the foo function to print, 'hello world'", the following happens:

  1. The code graph is automatically searched with the contents of the user's message using semantic search and (optionally) bm25 keyword search.
  2. Code snippets are sent along with the original user message to the LLM.
    - Note: These code snippets include the module path, e.g. `crate::module_a::foo`, for the code snippet.
  3. The LLM can either search for more context (using the `request_code_context` tool) or submit an edit.
  4. The edit the LLM submits, using the `crate::module_a::foo` from the earlier code snippet. The LLM code edit only requires the new code and the module path of the target item to edit 
    - This helps keep token costs low and helps avoid mistakes inherent in the commonly used search/replace based approach.
  5. If the LLM's code edit proposal is correctly formed and the target item exists, the proposal is "staged".
  6. Now the user can open the edit overlay from normal mode with `e` and see a
     list of code edits, including the status of the edit (proposed/applied)
     and a preview of the code changes (before/after)
  7. If the staged edit is approved by the user, then it is applied to the local files using the location of the code item for that module path, stored as the filepath and byte offset in the node in the database.
  <!--   - Note that if the target file has changed in between when the edit is -->
  <!-- proposed and when the edit is applied, the attempt to apply the code change -->
  <!-- will prompt a re-indexing of the code base to update the code graph, and the -->
  <!-- edit will not be applied. -->
  <!-- TODO: Verify that the above note is correct before including it in the documentation here -->

</details>

#### Selecting a model

When you first start `ploke`, it will default to using `kimi-k2` via OpenRouter. If you would like to browse different available models, you can use the following command:

```
/model search <model-name>
```
<!-- TODO: Confirm the following works as described. I'm fairly sure that is all -->
<!-- correct, but need to double-check. -->

For example, to see some popular models you might use `/model search kimi`,
`/model search gpt`, `/model search claude`. This will bring up an overlay with
all available endpoints that serve this model. You can navigate up and down the
list, and see more information on each (for example price per 1M tokens) by
pressing `l` on the selected model. Then simply press `Enter` and the model
will be chosen for the following conversation.

To confirm the model is the expected selection, you can see the model name
shown in the top right of the UI, or you can use the `/model info` command.

Note that you can pick different models at any time, and your conversation will
carry over to the new model.

#### Navigating Conversation History

| Keybind | Behavior |
| -------------- | --------------- |
| `k` or UpArrow | Select previous message |
| `j` or DownArrow | Select next message |
| `a` | Enter `Insert` mode with selected message as "latest" message |
| `gg` | Go to first message |
| `G` | Go to last message |
| `PageUp` | Scroll one page up |
| `PageDown` | Scroll one page down |

To navigate the conversation history with keybinds, you can enter Normal mode
by pressing `Esc`, then using the above keybinds. Once you're done you can go back to Insert mode with `i`.

By default the conversation with the LLM is sequential - each message forms a
natural chain, with the LLM occasionally calling tools to research the code
base or make edits.

However, if you would like to regenerated a response, return to an earlier
point in the conversation and explore a different conversation path, or just
navigate to earlier points quickly, the above keybinds are available. These
keybinds will likely be familiar if you use nvim, but you can also point and
click to select earlier messages or scroll through the conversation history
with your mouse or up/down arrows.

## 🚧 Under Construction 🚧

Ploke is in rapid development, and can be considered in a prototype or alpha state. The core functionality works right now:

- [x] Terminal-based LLM chat
- [x] nvim-friendly conversation navigation
- [x] Model-agnostic OpenRouter Support
  - [x] Model browser for OpenRouter LLM models
- [x] Semantic Search of parsed code graph
  - [x] HNSW semantic search of target rust project
  - [x] BM25 keyword search of target rust project
- [x] Agentic mode (multi-turn tool use)
  - [x] Semantic Code edits
  - [x] Edit overlay approval
- [x] Code graph infrastructure with support for:
  - [x] All Rust Items
  - [x] Edges for heirarchical containment relationships (e.g. module contains function, module contains module)
  - [/] Type Resolution with associated edges (partial)
  - [ ] Trait Resolution
- [/] Integration of Cozo features
  - [x] "time-travel" building versioning history per code-item
  - [x] fast hnsw search
  - [ ] Graph analysis tools (integrate cozo builtins like community detection, shortest path, etc)
  - [ ] cozo-native full-text search (requires upstream contributions to CozoDB)

These features are in-progress or will likely be added soon:
- [ ] (priority) Support for remote vector indexing + user-selected vector indexing models
- [/] (in progress) WebAssembly build for read-access to pre-indexed popular libraries (e.g. tokio, std, serde)
- [/] (in progress) Native plugin support for MCP
- [ ] Initial "reflection mode" for LLM to add notes on preferred user interaction (private, stored locally)
- [ ] Expanded provider support beyond OpenRouter (e.g. direct from OpenAI, Anthropic, Google Vertex, etc)
- [ ] Response sampling (multiple responses generated concurrently from varying models/providers)
- [ ] Ongoing QoL improvements (e.g. clear conversation history, save history, running token cost, etc)
- [ ] native cargo parsing (e.g. database includes passed/failed tests + makes test history searchable)
- [ ] native git integration (LLM tool + inclusion in code graph)

More ambitious/long term features:
- [ ] Static analysis tooling with LLM-facing interface for "ambient agent" behavior and background analysis
  - [ ] Search for code items by churn intensity, connectedness, etc
- [ ] Framework for user-defined tools + agentic behavior
- [ ] Interactive visual code graph
- [ ] Private (only stored locally) user behavior vector embeddings for coding proficiency profiling
  - [ ] Use for focused user improvement plans + goals (e.g. "I want to get better at functional programming")
- [ ] Traditional ML integration
- [ ] Rust HIR/MIR integration with code graph for multiple graph views + LLM-facing tooling
...
| ploke-embed | ✅ Alpha | Vector embeddings for code snippets |
...
| ploke-io | ✅ Alpha | I/O handler for reads/writes to file, including during vector embedding |
| context | ✅ Alpha | Context building for LLM prompts (partial, within `ploke-tui`) |
| llm | ✅ Alpha  | LLM integration for code generation<br>(Full semantic edits, needs search/replace edits for when parsing fails) |
| prompt | 👷 Partial | Prompt engineering of user prompt<br>(Basic setup for now, needs user-facing config) |
| ploke-tui | ✅ alpha prototype | User interface (primary TUI) |
| ploke-gui | ✅ Dev tool only<br>See ploke-ui branches | Dev tool to experiment with `Datalog` queries on underlying `cozo` database |
| analyze | 🚧 Planned<br>  Post-MVP | Static analysis of parsed code |
| ploke-lsp | 🚧 Planned<br>  Post-MVP   | User interface (CLI/GUI) |
| ploke-mcp | 🚧 Planned<br>  Post-MVP   | MCP protocol integration |
