...
pub enum Commands {
    ...
    /// - debug: Manifest, discovery, workspace probe, and corpus triage helpers
    #[command(subcommand)]
    Parse(Parse),
...
EXAMPLES:
    ...
    cargo xtask parse debug corpus-show run-1774750473411 --target Amanieu/parking_lot --member src --backtrace-full
...
PARSE DEBUG SUBCOMMANDS (see also `cargo xtask parse debug --help`):
    ...
    corpus-triage      Build a failure index and pending report stubs from a saved corpus run
    corpus-show        Re-open a saved corpus run and inspect one target / member / backtrace
"#
    );
}
